...
test('style gate rejects attribution, dramatic, meta, long sentences and filler; CLIP_TOPIC_STYLE_GATE=0 disables it; prompt carries the contract', () => {
  ...
  try {
    ...
    bad.mainStory = 'ลุงทูบอกว่าจะขายต่อ\nคลิปปิดท้ายด้วยรอยยิ้ม\n' + bad.mainStory;
    const gate = composeQualityGate(scoreTopicDoc(bad));
    assert.equal(gate.pass, false);
    const text = gate.reasons.join(' | ');
    for (const term of ['stories[0](s1).style: อ้างที่มา', 'บอกว่า', 'คำเปรย', 'ถึงกับ', 'เล่ากล้อง', 'คลิปเริ่มจาก', 'mainStory.style: อ้างที่มา', 'mainStory.style: เล่ากล้อง']) assert.ok(text.includes(term), 'missing ' + term + ' in ' + text);
    assert.equal(text.includes('คำเฟ้อ'), false, 'filler is soft: not a gate reason');
    // วงตรวจ 9 ก.ย.: ประโยคข้อเท็จจริงที่ใช้คำบอกเวลา/ระดับ และคำแทนคนไร้ชื่อของระบบ ต้องไม่ตกด่านแข็ง
    const factual = doc();
    factual.stories[0].story = 'ชายในคลิปเสียชีวิตทันที\nกล้องวงจรปิดเห็นทะเบียนรถชัดเจน\nลุงทูขายได้วันละ 300 บาทอย่างมาก\nเขาบอกตรงๆ ในใจ\n' + factual.stories[0].story;
    const factualGate = composeQualityGate(scoreTopicDoc(factual));
    assert.equal(factualGate.reasons.some((r) => r.includes('.style:')), false, 'neutral factual words never trip the hard gate: ' + JSON.stringify(factualGate.reasons));
    ...
    delete process.env.CLIP_TOPIC_STYLE_GATE;
    const prompt = buildComposePrompt({ evidencePack: pack() });
    for (const term of ['ใจความล้วน', 'ไม่อ้างที่มาในเนื้อเรื่อง', 'ลบสิ่งที่ไม่มีหลักฐาน', 'ลบประโยคเร้าอารมณ์', 'ห้ามเล่ากล้อง', 'ห้ามซ้ำ', 'ไม่เกิน 25 คำ', 'เล่าว่า / บอกว่า', 'ถึงกับ / เหนียวแน่น', 'ห้ามลบชื่อคน', 'ห้ามอ้างที่มาแฝง', 'ห้ามประโยคสรุป ประเมิน ตีความ อุปมา', 'ห้ามขยายหรือย่อจำนวน', 'เลี่ยงประโยคกรรม', 'identityLeads ตามข้อ 6', 'kind=speaker_statement', 'ห้ามคัดลอกประโยคเดิมคำต่อคำ', 'ห้ามคมกว่าหลักฐาน', 'ห้ามตัดคำกำกวมของหลักฐาน', 'ห้ามเติมรายละเอียดจำเพาะที่หลักฐานไม่ระบุ', 'ห้ามใส่ชื่อบุคคลจากความรู้ทั่วไป', 'ทุกประเด็น timeline ที่มี',
      // ★ 9 ก.ย. 69 มุมคนเล่า (เจ้าของชี้เคสออย-บีม): เล่าแบบคนใน ไม่ใช่นักข่าวรายงานถึงเขา
      'เล่าให้เพื่อนฟัง', 'มุมคนเล่า', 'ห้ามเปิดด้วยป้ายบทบาทซ้อนชื่อ', 'ออยกับบีม กวี คบหาดูใจกันมา 13 ปีก่อนแต่งงาน', 'ฝ่ายชาย / ฝ่ายหญิง', 'ประโยคเหตุการณ์ ไม่ใช่ประโยคนิยาม',
      // ★ 9 ก.ย. 69 รอบสอง (เจ้าของ: "ทั้งคู่" ถี่ไป เล่าเป็นทางการ): จังหวะลื่น + กันซ้ำคำ
      'จังหวะลื่น ไม่ใช่ประโยคข่าวเรียงแถว', 'ไม่เกิน 2 ครั้งต่อท่อน', 'ละประธาน', 'อย่าขึ้นต้นประโยคด้วยคำเดิมติดกัน', 'อย่าตัดจนสำนวนแข็ง']) assert.ok(prompt.includes(term), 'prompt missing ' + term);
