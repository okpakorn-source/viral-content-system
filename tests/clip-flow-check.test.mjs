...
test('🎬 ประโยคแรกขึ้นด้วยคำเล่า ต้องถูกจับ (เจ้าของทัก 11 ส.ค.: รอบที่สวยกับไม่สวยต่างกันแค่จุดนี้)', () => {
  const bad = 'เอิร์น ไดเม่ เล่าชีวิตวัยเด็กว่า เติบโตมากับยายสองคนในครอบครัวยากจน ได้เงินไปโรงเรียนวันละ 2 บาท';
  const good = 'เอิร์น ไดเม่ เติบโตมาในต่างจังหวัดกับยายสองคน ได้เงินไปโรงเรียนวันละ 2 บาท';
  assert.ok(inspectField('rawData', bad, 240).some((i) => i.type === 'tell_open'), 'แบบมีคำเล่านำต้องติดธง');
  assert.ok(!inspectField('rawData', good, 240).some((i) => i.type === 'tell_open'), 'แบบเข้าเหตุการณ์ตรงต้องไม่ติดธง');
  // คำเล่ากลางเรื่องยังใช้ได้ ห้ามจับ
  const midOk = `${good} ต่อมาเธอบอกแม่ว่าอยากย้ายโรงเรียน`;
  assert.ok(!inspectField('rawData', midOk, 240).some((i) => i.type === 'tell_open'), 'คำเล่ากลางเรื่องต้องไม่ติดธง');
});

test('เฟส B+: กฎห้ามขึ้นต้นด้วยคำเล่าต้องอยู่ในพรอมต์จริง', () => {
  fclear();
  process.env.CLIP_FLOW = '1';
  const p = ins._buildVideoInsightPromptForTest({});
  assert.ok(p.includes('ห้ามขึ้นต้นประโยคแรกด้วยคำเล่า'), 'กฎต้องอยู่ในพรอมต์');
  assert.ok(p.includes('คำเล่าใช้ได้ตามปกติในประโยคถัดๆ ไป'), 'ต้องระบุว่าห้ามเฉพาะประโยคแรก ไม่ใช่ห้ามทั้งเนื้อ');
  fclear();
});
