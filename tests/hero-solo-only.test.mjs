...
test('(a) OFF (MEGA_HERO_SOLO_ONLY=0): พูลไม่มีเดี่ยว → synth crop เดิม (_heroFaceCrop) = byte-parity + ไม่แตะ findImagesByPerson', async () => {
  await withSoloOnly('0', async () => {
    ...
    const r = await s6_slots(mkJob(), {
      origin: 'http://mock',
      _deps: { ...brainPicksCouple(couple), fetchJson: fetchStub([couple]), findImagesByPerson: async (a) => { calls.push(a); return [borrowedSolo()]; }, checkUrlAlive: async () => true },
    });
...
test('(g) ON + solo สกปรก (clean:false) → hasSolo ไม่บล็อก borrow → ใช้ borrowed solo (audit Finding 2)', async () => {
  await withSoloOnly(null, async () => {
    ...
    const r = await s6_slots(mkJob(), {
      origin: 'http://mock',
      _deps: { ...brainPicksCouple(couple), fetchJson: fetchStub([couple, dirtySoloImg]), findImagesByPerson: async (a) => { calls.push(a); return [borrowedSolo()]; }, checkUrlAlive: async () => true },
    });
    assert.equal(r.status, 'done');
    const hero = r.dossierPatch.pickImages.slots.hero;
...
// (d) rev.2 (ผู้ใช้เลือก fallback=ครอปคู่): ON + ไม่มีเดี่ยว + ยืมไม่ได้ → synth crop ภาพคู่ในเคส (ปกออก ไม่ HOLD)
test('(d) ON + พูลไม่มีเดี่ยว + ยืมไม่ได้ (mock คืน []) → synth crop ภาพคู่ในเคส (done + _heroFaceCrop) ไม่ HOLD', async () => {
  await withSoloOnly(null, async () => {
    const calls = [];
    const couple = coupleBig();
    const r = await s6_slots(mkJob(), {
    ...
    assert.equal(r.status, 'done', 'ยืมไม่ได้ → fallback synth crop → ปกออก (ไม่ HOLD)');
    assert.equal(r.dossierPatch.pickImages.slots.hero?.id, 'COUPLE-1');
    assert.ok(r.dossierPatch.pickImages.slots.hero?._heroFaceCrop, 'ต้องครอปหน้า hero เดี่ยวจากภาพคู่ (_heroFaceCrop)');
    assert.equal(calls.length, 1, 'ต้องพยายามยืมก่อน ถึงจะ fallback synth');
  });
});

// (e) ON + ไม่มีเดี่ยว + ยืมได้แต่เป็นภาพคู่ → กรองทิ้ง (รับเฉพาะเดี่ยว) → fallback synth crop
test('(e) ON + พูลไม่มีเดี่ยว + ยืมได้แต่เป็น "ภาพคู่" → กรองทิ้ง (รับเฉพาะ faceCount===1) → synth crop ในเคส (done)', async () => {
  await withSoloOnly(null, async () => {
    const couple = coupleBig();
    const r = await s6_slots(mkJob(), {
      origin: 'http://mock',
    ...
    assert.equal(r.status, 'done', 'ยืมมาแต่เป็นภาพคู่ → ไม่รับ → fallback synth crop → ปกออก');
    assert.ok(r.dossierPatch.pickImages.slots.hero?._heroFaceCrop);
  });
});

// (h) 🆕 ON + ยืมได้เดี่ยว แต่ URL ตาย (checkUrlAlive=false) → ทิ้ง → fallback synth crop (บั๊ก AC-0163)
test('(h) ON + ยืมเดี่ยวได้ แต่ URL ยืมตาย (checkUrlAlive=false) → ทิ้งภาพตาย → fallback synth crop ในเคส (done)', async () => {
  await withSoloOnly(null, async () => {
    const calls = [];
    const couple = coupleBig();
    const r = await s6_slots(mkJob(), {
      origin: 'http://mock',
      _deps: { ...brainPicksCouple(couple), fetchJson: fetchStub([couple]), findImagesByPerson: async () => [borrowedSolo()], checkUrlAlive: async (u) => { calls.push(u); return false; } },
    });
    assert.equal(r.status, 'done', 'ภาพยืม URL ตาย → ทิ้ง → fallback synth (ไม่เอาภาพตายขึ้น hero)');
    assert.equal(r.dossierPatch.pickImages.slots.hero?.id, 'COUPLE-1', 'hero ต้องไม่ใช่ภาพยืมที่ URL ตาย');
    assert.ok(r.dossierPatch.pickImages.slots.hero?._heroFaceCrop);
    assert.ok(calls.length >= 1, 'ต้องเช็ค URL ยืมว่ามีชีวิต');
  });
});
...
test('(f) ON + brain ล่ม + พูลไม่มีเดี่ยว + ยืมได้ → fallback เลือก borrowed solo (done, ไม่ HOLD)', async () => {
  await withSoloOnly(null, async () => {
    ...
    const r = await s6_slots(mkJob(), {
      origin: 'http://mock',
      _deps: { ...NO_BRAIN, fetchJson: fetchStub([couple]), findImagesByPerson: async () => [borrowedSolo()], checkUrlAlive: async () => true },
    });
    assert.equal(r.status, 'done');
