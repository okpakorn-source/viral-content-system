...
function mk({ score, confidence = 'ok', reproducible = true, worstOffsetPx = 10, duplicateOf, dropFidelity, dropTemplate, humanVerified } = {}) {
  const dna = structuredClone(BASE_DNA);
  delete dna._templateGrade;
  delete dna._duplicateOf; // กันค่าติดมาจาก BASE_DNA
  delete dna._humanVerified; // กันค่าติดมาจาก BASE_DNA (คลังจริงตอนนี้มีใบ human-verified ปนอยู่ — R6 floor test)
...
test('regression: ใบซ้ำจริงในคลัง (record._duplicateOf) → F + ถูกตัดจากพูล ON', { skip: 'ล้าสมัย: คลัง ref รีเซ็ตเหลือ top3 (3 ใบ structure-only) — fixture อ้าง ID/คลัง 33 ใบเก่า รอเขียนใหม่ด้วย synthetic fixture' }, () => {
...
test('F ทันที: ไม่มี imagePath → F', { skip: 'ล้าสมัย: structure-only redesign (f486c72) ตัดเงื่อนไข imagePath ออกแล้ว — assertion เดิมไม่จริงอีก' }, () => {
...
// ── R6 (19 ก.ค.): grade floor สำหรับใบ human-verified ──
//   ★ เดิม (ก่อน R6) _humanVerified ไม่มีผลต่อเกรดเลย — ตอนนี้เปลี่ยนเจตนา: ตาคนยืนยัน geometry แล้ว
//     แต่ auto-fidelity ยังต่ำ ไม่ควรถูกกักที่ C/F → floor เป็น B (default เปิด, ปิดได้ด้วย env='0')
test('R6: human-verified + score 59 (C) → floor เป็น B', () => {
  const g = computeTemplateGrade(mk({ score: 59, humanVerified: true }));
  assert.equal(g.grade, 'B');
  assert.ok(g.reasons.some((r) => /human-verified.*floor/.test(r)));
});
test('R6: human-verified + score 40 (F) → floor เป็น B', () => {
  const g = computeTemplateGrade(mk({ score: 40, humanVerified: true }));
  assert.equal(g.grade, 'B');
});
test('R6: human-verified + score 92 (A อยู่แล้ว) → ยังเป็น A (ไม่ลดขั้น)', () => {
  const g = computeTemplateGrade(mk({ score: 92, humanVerified: true }));
  assert.equal(g.grade, 'A');
  assert.ok(!g.reasons.some((r) => /human-verified/.test(r)), 'ไม่ควรมีเหตุผล floor ติดมาเมื่อไม่ได้ floor จริง');
});
test('R6: ไม่ human-verified + score 59 → ยัง C เหมือนเดิม (ไม่ floor)', () => {
  assert.equal(computeTemplateGrade(mk({ score: 59, humanVerified: false })).grade, 'C');
});
test('R6: _duplicateOf + human-verified → ยัง F (ประตู duplicate มาก่อน ไม่ถูก floor)', () => {
  const rec = mk({ score: 95, humanVerified: true, duplicateOf: 'REF-orig' });
  const g = computeTemplateGrade(rec);
  assert.equal(g.grade, 'F');
  assert.ok(g.reasons.some((r) => /ใบซ้ำ/.test(r)));
});
test('R6: dnaToTemplateSpec ไม่ผ่าน + human-verified → ยัง F (structural fail ไม่ถูก floor)', () => {
  const g = computeTemplateGrade(mk({ score: 95, humanVerified: true, dropTemplate: true }));
  assert.equal(g.grade, 'F');
});
test('R6: ไม่มี _fidelity + human-verified → ยัง F (ไม่ถูก floor)', () => {
  const g = computeTemplateGrade(mk({ humanVerified: true, dropFidelity: true }));
  assert.equal(g.grade, 'F');
});
test('R6 kill-switch: REF_HUMAN_VERIFIED_FLOOR=0 → ปิด floor กลับเป็นพฤติกรรมเดิม', () => {
  const rec = mk({ score: 40, humanVerified: true });
  assert.equal(computeTemplateGrade(rec, { REF_HUMAN_VERIFIED_FLOOR: '0' }).grade, 'F');
  assert.equal(computeTemplateGrade(rec, {}).grade, 'B', 'ไม่ตั้ง/ค่าอื่น = floor เปิด (default)');
});
...
test('gate OFF: refPoolGateOpen === ตัวกรองเดิม (dna+imagePath+!_derived+_reproducible!==false) ทุกใบในคลังจริง', { skip: 'ล้าสมัย: structure-only redesign (f486c72) ตัดเงื่อนไข imagePath ออกจาก gate แล้ว — assertion เดิมไม่จริงอีก' }, () => {
...
test('gate OFF: pool ที่ป้อน pickBestRef เท่าเดิม (จำนวน+สมาชิกตรง)', { skip: 'ล้าสมัย: คลัง ref รีเซ็ตเหลือ top3 (3 ใบ structure-only) — fixture อ้างคลัง 33 ใบเดิม รอเขียนใหม่ด้วย synthetic fixture' }, () => {
  const envOff = {};
...
test('gate OFF: refPoolGateOpen(variant, {}) === false ทุก variant _derived ในคลัง', { skip: 'ล้าสมัย: คลัง ref รีเซ็ตเหลือ top3 (3 ใบ structure-only ไม่มี variant _derived) — รอเขียนใหม่ด้วย synthetic fixture' }, () => {
  const envOff = {};
...
test('gate ON: รับเฉพาะเกรด A/B และไม่มี _duplicateOf', { skip: 'ล้าสมัย: คลัง ref รีเซ็ตเหลือ top3 (3 ใบ structure-only) — fixture อ้างคลัง 33 ใบเดิม รอเขียนใหม่ด้วย synthetic fixture' }, () => {
  const envOn = { REF_TEMPLATE_GRADE_GATE: '1' };
...
test('stored _templateGrade ในคลังจริงตรงกับ recompute ทุกใบ (รวม duplicate = F)', { skip: 'ล้าสมัย: คลัง ref รีเซ็ตเหลือ top3 (3 ใบ structure-only) — stored grade เดิมคำนวณก่อน R6 floor/structure-only รอเขียนใหม่ด้วย synthetic fixture' }, () => {
  const dups = ['REF-mrbq76vs-652r', 'REF-mrbqalpo-h1r1'];
  // ★ R6 (19 ก.ค.): เทียบด้วย env='0' (ปิด floor) — stored ในคลังจริงคำนวณจากเอนจินก่อนมี floor นี้
  //   ใบ human-verified ที่ยังไม่ได้รัน scripts/grade-ref-library.mjs ซ้ำ จะได้ B จาก floor (recompute ปกติ)
  //   แต่ stored ยังเป็น C/F เดิม — ด่านนี้ตรวจ "ลืมรันสคริปต์ regrade หรือไม่" ไม่ใช่ตรวจ floor feature
  //   จึงเทียบแบบปิด floor ให้ตรงกับตอนที่ค่า stored ถูกคำนวณไว้
  for (const rec of library) {
    const stored = rec?.dna?._templateGrade?.grade;
    const fresh = computeTemplateGrade(rec, { REF_HUMAN_VERIFIED_FLOOR: '0' }).grade;
...
test('PIN: ตั้ง REF_POOL_PIN = เหลือเฉพาะ id นั้น (ใบอื่นแม้เกรด A ก็ไม่ผ่าน)', { skip: 'ล้าสมัย: คลัง ref รีเซ็ตเหลือ top3 (3 ใบ) — fixture อ้าง ID เก่าที่ลบไปแล้ว รอเขียนใหม่ด้วย synthetic fixture' }, () => {
