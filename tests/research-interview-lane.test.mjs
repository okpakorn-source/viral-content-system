...
// ── 🔒 F2 (Fable audit): needsContext — ชื่อกำกวมต้องมีหลักฐานแข็งกว่าหัวข้อ ──
test('F2: needsContext=true + ชื่อในหัวข้อ/snippet เท่านั้น → expected (ไม่ matched, ไม่เดาตัวตน)', () => {
  const r = confirmObservedName({ expectedName: 'คิว', title: 'คิว เผยน้ำตากลางรายการ', needsContext: true });
  assert.equal(r.nameStatus, 'expected');
  assert.equal(r.observedName, null);
  const r2 = confirmObservedName({ expectedName: 'คิว', snippet: 'คิว เปิดใจ', needsContext: true });
  assert.equal(r2.nameStatus, 'expected');
});

test('F2: needsContext=true แต่มีชื่อใน speakers (หลักฐานแข็ง) → matched', () => {
  const r = confirmObservedName({ expectedName: 'คิว', title: 'สัมภาษณ์พิเศษ', speakers: ['คิว', 'พิธีกร'], needsContext: true });
  assert.equal(r.nameStatus, 'matched');
  assert.equal(r.nameEvidence, 'speaker');
});

test('F2: needsContext=false (คนทั่วไป) + ชื่อในหัวข้อ → matched เหมือนเดิม (ไม่กระทบ)', () => {
  const r = confirmObservedName({ expectedName: 'แพท ณปภา', title: 'แพท ณปภา เผยจุดเปลี่ยน', needsContext: false });
  assert.equal(r.nameStatus, 'matched');
  assert.equal(r.nameEvidence, 'title');
});

test('F2: classifyInterviewCandidate ส่ง needsContext ต่อ → คนกำกวมในหัวข้อได้ expected', () => {
