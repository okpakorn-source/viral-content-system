import assert from 'node:assert/strict';
import { test, beforeEach, afterEach, after } from 'node:test';
import { register } from 'node:module';
import { spawnSync } from 'node:child_process';
import { inflateSync } from 'node:zlib';
import { deserialize } from 'node:v8';
import { fileURLToPath } from 'node:url';

const SRC = new URL('../src/', import.meta.url).href;
const PIPE_URL = new URL('../src/lib/services/clipBrain/clipBrainPipeline.js', import.meta.url);
const SERVICE_URL = new URL('../src/lib/services/clipInsightService.js', import.meta.url);
const VERIFY_URL = new URL('../src/lib/services/clipBrain/clipVerify.js', import.meta.url);
const SCHEMA_URL = new URL('../src/lib/services/clipBrain/topicSchema.js', import.meta.url);
const COMPOSE_URL = new URL('../src/lib/services/clipBrain/composeTopics.js', import.meta.url);
const KEY = 'clip-verify-readiness.state';
const state = { responses: [], calls: [], packs: [], fetches: [] };
globalThis[Symbol.for(KEY)] = state;
after(() => { delete globalThis[Symbol.for(KEY)]; });
const moduleData = (s) => 'data:text/javascript,' + encodeURIComponent(s);
const brainModule = moduleData(`
  export async function runBrain(opts) {
    const s = globalThis[Symbol.for(${JSON.stringify(KEY)})];
    s.calls.push(opts);
    if (opts.label === 'วางแผนผ่า') return { ok: true, json: { segments: s.plan }, costUSD: 0.02 };
    if (opts.label.startsWith('clip-compose-')) {
      if (!s.composeReplies.length) throw new Error('Unexpected composer call');
      const reply = s.composeReplies.shift();
      if (reply instanceof Error) throw reply;
      return reply;
    }
    if (opts.label === 'ผู้ตรวจ') return { ok: true, json: { verdict: s.findings.length ? 'ต้องตรวจ' : 'สะอาด', findings: s.findings } };
    if (opts.label === 'ตัวซ่อม') return s.repairReply || { ok: false, errorType: 'MOCK_REPAIR_UNAVAILABLE' };
    throw new Error('Unexpected brain label');
  }
`);
const composeModule = moduleData(`
  import { composeTopics as real } from ${JSON.stringify(COMPOSE_URL.href)};
  export async function composeTopics(opts) {
    const s = globalThis[Symbol.for(${JSON.stringify(KEY)})];
    s.packs.push(structuredClone(opts.evidencePack));
    if (s.composeCrash) throw new Error('synthetic composer crash');
    return real(opts);
  }
`);
register(moduleData(`
  const hasExt = (s) => /\\.[a-zA-Z0-9]{1,5}$/.test(s);
  export async function resolve(spec, ctx, next) {
    if (spec === './topicMetrics.js' && ctx.parentURL === ${JSON.stringify(VERIFY_URL.href)} && process.env.CLIP_READINESS_BLOCK_METRICS === '1') throw new Error('Legacy readiness imported metrics');
    if (spec === './brainRunner.js') return { url: ${JSON.stringify(brainModule)}, shortCircuit: true };
    if (spec === './composeTopics.js' && ctx.parentURL?.startsWith(${JSON.stringify(PIPE_URL.href)})) return { url: ${JSON.stringify(composeModule)}, shortCircuit: true };
    if (spec === '../../ai/usageLogger.js') return { url: 'data:text/javascript,export async function logApiUsage() {}', shortCircuit: true };
    if (spec === '@/lib/services/clipAI/openai') return { url: 'data:text/javascript,export function callAI() { throw new Error("Unexpected paid AI call"); }', shortCircuit: true };
    if (spec.startsWith('@/')) return next(new URL(spec.slice(2) + (hasExt(spec) ? '' : '.js'), ${JSON.stringify(SRC)}).href, ctx);
    if ((spec.startsWith('./') || spec.startsWith('../')) && !hasExt(spec)) {
      try { return await next(spec + '.js', ctx); } catch { /* Default resolution. */ }
    }
    return next(spec, ctx);
  }
  export async function load(url, ctx, next) {
    const result = await next(url, ctx);
    const isPipe = url === ${JSON.stringify(PIPE_URL.href)};
    const isSchema = url === ${JSON.stringify(SCHEMA_URL.href)};
    let source = String(result.source);
    if (url === ${JSON.stringify(VERIFY_URL.href)}) {
      source = source.replace("export async function assessReadiness(insight, { truth = '' } = {}) {",
        "export async function assessReadiness(insight, { truth = '' } = {}) { if (globalThis[Symbol.for('clip-verify-readiness.state')]?.readinessCrash) throw new Error('synthetic readiness crash');");
      return { ...result, source };
    }
    const patches = {
      crash: [isPipe, "brain.degradations.push({ type: 'readiness-crashed', why: String(e?.message || e) });", 'throw e;'],
      readiness: [isPipe, 'const readiness = await assessReadiness(insight, { truth: truthText });',
        'const readiness = { findings: [], stories: insight.topicsV2.stories.map((s) => ({ id: s.id, issues: [] })), mainStory: { issues: [] } };'],
      low: [isPipe, "f?.severity === 'สูง' || f?.severity === 'กลาง'", "!!f"],
      stale: [isSchema, "if (repairedMain) set(doc, 'mainStoryStale', true, 'topicsV2.mainStoryStale');",
        "if (repairedMain) { doc.mainStory = doc.stories.find((s) => s.id === doc.mainTopicId).story; result.mainStory = doc.mainStory; }"],
      quote: [isPipe, "quote.verification = quoteCoverage(quote.text, truthText) >= 0.6 ? 'verified' : 'unverified';",
        "quote.verification = 'verified';"],
    };
    const patch = patches[process.env.CLIP_READINESS_MUTATION];
    if (patch?.[0]) {
      if (!source.includes(patch[1])) throw new Error('Mutation anchor missing');
      source = source.replace(patch[1], patch[2]);
      return { ...result, source };
    }
    return result;
  }
`));

const initialUncapped = process.env.CLIP_UNCAPPED;
process.env.CLIP_UNCAPPED = '1';
const { runClipBrainPipeline } = await import(PIPE_URL.href);
const { normalizeInsight } = await import(SERVICE_URL.href);
process.env.CLIP_UNCAPPED = '0';
const { normalizeInsight: normalizeCapped } = await import(SERVICE_URL.href + '?capped-p3-test');
if (initialUncapped === undefined) delete process.env.CLIP_UNCAPPED;
else process.env.CLIP_UNCAPPED = initialUncapped;
const { emptyTopicDoc, toLegacyInsight, syncTopicsV2FromLegacy } = await import('../src/lib/services/clipBrain/topicSchema.js');
const { countThaiWords } = await import('../src/lib/services/clipBrain/topicMetrics.js');

const ENV_KEYS = ['CLIP_TOPIC_V2', 'CLIP_TOPIC_MODEL', 'CLIP_TOPIC_EFFORT', 'CLIP_TOPIC_FALLBACK_MODEL',
  'CLIP_TOPIC_FALLBACK_EFFORT', 'CLIP_TOPIC_TIMEOUT_MS', 'GEMINI_VIDEO_API_KEY', 'GEMINI_API_KEY',
  'CLIP_SAFE_TEXT', 'CLIP_USAGE_LOG', 'CLIP_GEMINI_MAX_ATTEMPTS', 'CLIP_GEMINI_FALLBACK_MODELS', 'CLIP_TOPIC_STYLE_GATE'];
let saved;
function setup() {
  saved = { env: Object.fromEntries(ENV_KEYS.map((k) => [k, process.env[k]])), fetch: globalThis.fetch,
    Date: globalThis.Date, log: console.log, warn: console.warn };
  for (const key of ENV_KEYS) delete process.env[key];
  process.env.CLIP_TOPIC_STYLE_GATE = '0'; // ★ 9 ก.ย. 69: fixture จริงมีคำอ้างที่มา — ด่านสำนวนมีเทสของตัวเอง
  Object.assign(process.env, { GEMINI_VIDEO_API_KEY: 'offline-test-key', CLIP_SAFE_TEXT: '0',
    CLIP_USAGE_LOG: '0', CLIP_GEMINI_MAX_ATTEMPTS: '1', CLIP_GEMINI_FALLBACK_MODELS: '' });
  const fixed = 1788652800000;
  globalThis.Date = class extends saved.Date {
    constructor(...args) { super(...(args.length ? args : [fixed])); }
    static now() { return fixed; }
  };
  console.log = () => {};
  console.warn = () => {};
  globalThis.fetch = async (url, opts) => {
    state.fetches.push({ url, body: JSON.parse(opts.body) });
    assert.ok(state.responses.length, 'No unexpected Gemini calls');
    const data = state.responses.shift();
    if (data?.mockHttpFailure) return { ok: false, status: 400, json: async () => ({ error: { message: 'offline failure' } }) };
    return { ok: true, status: 200, json: async () => ({ candidates: [{
      content: { parts: [{ text: JSON.stringify(data) }] }, finishReason: 'STOP',
    }] }) };
  };
}
function teardown() {
  globalThis.fetch = saved.fetch; globalThis.Date = saved.Date;
  console.log = saved.log; console.warn = saved.warn;
  for (const key of ENV_KEYS) {
    if (saved.env[key] === undefined) delete process.env[key]; else process.env[key] = saved.env[key];
  }
}
beforeEach(setup);
afterEach(teardown);

const ORIGINAL_TEXT = 'คนในชุมชนช่วยกันซ่อมบ้านและจัดเตรียมร้านก่อนเปิดขายของในตลาด '.repeat(10);
const BASE = { clipType: 'monologue', headline: 'ชุมชนเปิดร้าน', rawData: ORIGINAL_TEXT,
  overview: 'คนในชุมชนเตรียมเปิดร้าน', speakers: [], quotes: [], subStories: [] };
const TRUTH = { transcription: [{ time: '00:00–00:30', speaker: '', text: ORIGINAL_TEXT }], onScreenText: ['เปิดร้านในชุมชน'] };
const PLAN = [1, 2, 3].map((no) => ({ no, startSec: (no - 1) * 300, endSec: no * 300, topics: [] }));
function words(n, variant = 0) {
  const tokens = [['คน', 'รถ', 'บ้าน', 'น้ำ', 'งาน'], ['แมว', 'นก', 'ปลา', 'ช้าง', 'ม้า']][variant % 2];
  const value = Array.from({ length: n }, (_, i) => tokens[i % tokens.length]).join(' ');
  assert.equal(countThaiWords(value), n);
  return value;
}
function doc() {
  const d = emptyTopicDoc();
  d.mainTopicId = 's1'; d.mainStory = words(120, 1);
  d.stories = [{ id: 's1', topic: 'ชุมชนซ่อมบ้าน', highlight: 'คนในชุมชนช่วยกันซ่อมบ้าน', story: words(120),
    timeRanges: [{ startSec: 0, endSec: 30 }], sharePct: null,
    facts: [{ id: 'f1', text: 'คนในชุมชนช่วยกันซ่อมบ้าน', kind: 'speaker_statement', evidenceIds: ['t1'] }],
    quotes: [], standalone: true, overlaps: [], quality: { status: 'not_checked', issues: [] } }];
  return d;
}
async function execute(flag, options = {}) {
  if (flag === undefined) delete process.env.CLIP_TOPIC_V2; else process.env.CLIP_TOPIC_V2 = flag;
  Object.assign(state, { calls: [], packs: [], fetches: [], plan: PLAN,
    findings: options.findings || [], repairReply: options.repairReply,
    composeCrash: !!options.composeCrash, readinessCrash: !!options.readinessCrash,
    composeReplies: options.composeReplies || [{ ok: true, json: doc(), costUSD: 0.15 }] });
  const durationSec = options.long ? 900 : 60;
  const map = { headline: 'MAP_ONLY_FAKE_NAME', timeline: [], clipDurationSec: durationSec };
  state.responses = [map, ...(options.long
    ? [BASE, options.missing ? { mockHttpFailure: true } : BASE, BASE] : [options.extracted || BASE]), options.truth ?? TRUTH];
  const result = await runClipBrainPipeline({ url: 'https://www.youtube.com/watch?v=offline-test',
    isYouTube: true, durationSec, model: 'gemini-3.7-flash', usageLogger: () => {} });
  assert.equal(result.ok, true, result.error);
  assert.equal(state.responses.length, 0);
  return { result, calls: structuredClone(state.calls), packs: structuredClone(state.packs), fetches: structuredClone(state.fetches) };
}
const freeze = (v) => { if (v && typeof v === 'object') { Object.values(v).forEach(freeze); Object.freeze(v); } return v; };


// Immutable pre-P3 V8 snapshots, reused unchanged for legacy compatibility.
const SNAPSHOTS = deserialize(inflateSync(Buffer.from('', 'base64')));

const { assessReadiness, checkAgainstTruth, quoteCoverage } = await import(VERIFY_URL.href);
const { bureaucraticRate, longSentenceCount } = await import('../src/lib/services/clipBrain/topicMetrics.js');
const codes = (issues) => issues.map((issue) => issue.code);
const assess = (d, truth = ORIGINAL_TEXT) => assessReadiness({ topicsV2: d }, { truth });
const storyIssues = async (d, truth) => (await assess(d, truth)).stories[0].issues;
const paragraphs = (n, variant = 0) => {
  const tokens = words(n, variant).split(' ');
  return Array.from({ length: Math.ceil(n / 50) }, (_, i) => tokens.slice(i * 50, (i + 1) * 50).join(' ')).join('\n');
};

test('readiness no-v2 and wrong-version inputs are pure no-ops without importing metrics', async () => {
  const before = process.env.CLIP_READINESS_BLOCK_METRICS;
  process.env.CLIP_READINESS_BLOCK_METRICS = '1';
  try {
    for (const input of [undefined, null, {}, { topicsV2: { schemaVersion: 1 } }]) {
      const snapshot = structuredClone(input);
      assert.deepEqual(await assessReadiness(freeze(input), { truth: ORIGINAL_TEXT }),
        { findings: [], stories: [], mainStory: { issues: [] } });
      assert.deepEqual(input, snapshot);
    }
  } finally {
    if (before === undefined) delete process.env.CLIP_READINESS_BLOCK_METRICS;
    else process.env.CLIP_READINESS_BLOCK_METRICS = before;
  }
});

test('readiness word boundaries 99/100/170/171 use real Thai segmentation for stories and mainStory (no ceiling by default)', async () => {
  for (const [n, expected] of [[99, 'length-short'], [100, null], [170, null], [171, null]]) {
    const d = doc();
    d.stories[0].story = paragraphs(n);
    d.mainStory = paragraphs(n, 1);
    assert.equal(countThaiWords(d.stories[0].story), n);
    assert.equal(countThaiWords(d.mainStory), n);
    const r = await assess(d);
    for (const issues of [r.stories[0].issues, r.mainStory.issues]) {
      const lengths = issues.filter((i) => i.code.startsWith('length-'));
      assert.deepEqual(lengths.map((i) => i.code), expected ? [expected] : []);
      if (expected) assert.equal(lengths[0].value, n);
    }
  }
});

test('readiness highlights, meaningful facts, missing main narrative and Thai finding contracts', async () => {
  const d = doc();
  d.stories[0].highlight = ' ';
  d.stories[0].facts = [{ text: ' ' }];
  d.mainStory = '';
  const r = await assess(d);
  assert.ok(codes(r.stories[0].issues).includes('no-highlight'));
  assert.ok(codes(r.stories[0].issues).includes('no-facts'));
  assert.deepEqual(codes(r.mainStory.issues), ['missing']);
  for (const finding of r.findings) {
    assert.equal(finding.side, 'ความพร้อม');
    assert.equal(finding.severity, 'ข้อสังเกต');
    assert.match(finding.where, /^(topicsV2\.stories\[0\]|mainStory)$/);
    assert.equal(finding.kind, 'ความพร้อมใช้งาน');
    assert.match(finding.fix, /[ก-๙]/);
    assert.match(finding.detail, /[ก-๙]/);
  }
  assert.equal(r.findings.length, r.stories[0].issues.length + r.mainStory.issues.length);
});

test('readiness bureaucratic cutoff is inclusive 0.8 per 1000 Unicode chars in story and main', async () => {
  for (const [chars, expected] of [[1251, false], [1250, true], [1249, true]]) {
    const d = doc();
    const term = 'ดังกล่าว';
    const body = term + ' '.repeat(chars - Array.from(term).length);
    assert.equal(Array.from(body).length, chars);
    assert.ok(Math.abs(bureaucraticRate(body) - 1000 / chars) < 1e-12);
    d.stories[0].story = body; d.mainStory = body;
    const r = await assess(d);
    for (const issues of [r.stories[0].issues, r.mainStory.issues]) {
      assert.equal(codes(issues).includes('bureaucratic'), expected);
    }
  }
});

test('readiness long sentence uses >60 words and respects paragraph boundaries', async () => {
  for (const [n, expected] of [[60, false], [61, true]]) {
    const d = doc();
    d.stories[0].story = words(n) + '\n' + words(100 - n, 1);
    assert.equal(countThaiWords(d.stories[0].story), 100);
    assert.equal(longSentenceCount(d.stories[0].story), expected ? 1 : 0);
    const issues = await storyIssues(d);
    assert.equal(codes(issues).includes('long-sentence'), expected);
    if (expected) assert.equal(issues.find((i) => i.code === 'long-sentence').value, 1);
  }
});

test('readiness overlap uses actual matching text and identifies both peer story IDs', async () => {
  const d = doc();
  d.stories.push({ ...structuredClone(d.stories[0]), id: 'second' });
  d.stories[0].overlaps = [];
  d.stories[1].overlaps = [];
  let r = await assess(d);
  assert.deepEqual(r.stories.map((s) => s.issues.filter((i) => i.code === 'overlap').map((i) => i.value)), [['second'], ['s1']]);
  assert.match(r.stories[0].issues.find((i) => i.code === 'overlap').detail, /second/);
  d.stories[1].story = words(120, 1);
  d.stories[0].overlaps = [{ storyId: 'second', kind: 'duplicate' }];
  r = await assess(d);
  assert.ok(r.stories.every((s) => !codes(s.issues).includes('overlap')), 'Declared overlaps alone do not fabricate metric matches');
});

test('readiness quotes require coverage >=0.6, short normalized bodies never verify, metadata is not evidence', async () => {
  const prefix = 'abcdefghijklmnopqrstu';
  const quote = prefix + 'VWXYZABCDEFGHI';
  assert.equal(quote.length, 35);
  assert.equal(quoteCoverage(quote, prefix), 0.6);
  assert.ok(quoteCoverage(quote, prefix.slice(0, -1)) < 0.6);
  const d = doc();
  d.stories[0].quotes = [
    { text: quote, speaker: '', evidenceIds: [], verification: 'pending' },
    { text: 'never matched quotation text', speaker: '', evidenceIds: ['t1'], verification: 'verified' },
    { text: 'ผู้พูดชื่อยาว: ดีมาก', speaker: '', evidenceIds: ['t1'], verification: 'verified' },
  ];
  const r = await assess(d, prefix + ' ดีมาก');
  const issues = r.stories[0].issues.filter((i) => i.code.startsWith('quote-'));
  assert.deepEqual(codes(issues), ['quote-unverified', 'quote-short']);
  assert.equal(issues[0].value, 0);
  assert.equal(issues[1].value, 2);
  assert.equal((await storyIssues({ ...d, stories: [{ ...d.stories[0], quotes: [d.stories[0].quotes[0]] }] }, prefix)).some((i) => i.code.startsWith('quote-')), false);
  const noTruth = await assess(d, '');
  assert.equal(codes(noTruth.stories[0].issues).filter((code) => code === 'quote-unverified').length, 2);
});

test('readiness is pure for a frozen v2 document and never drops or rewrites its text', async () => {
  const input = toLegacyInsight(doc(), BASE);
  const snapshot = structuredClone(input);
  await assessReadiness(freeze(input), { truth: ORIGINAL_TEXT });
  assert.deepEqual(input, snapshot);
});

test('truth findings gain only side while retaining severity, detail, location, fixes and low code diagnostics', () => {
  const r = checkAgainstTruth({ ...BASE, speakers: ['สมชาย ใจดี'], rawData: 'ข้อมูล 9999', quotes: ['ดีมาก'] }, ORIGINAL_TEXT);
  assert.deepEqual(r.findings.map((f) => f.severity), ['สูง', 'กลาง', 'ต่ำ']);
  for (const f of r.findings) {
    assert.equal(f.side, 'ความจริง');
    for (const key of ['kind', 'where', 'detail', 'fix']) assert.equal(typeof f[key], 'string');
  }
});

test('readiness crash stays ok, records degradation and continues truth status calculation', async () => {
  for (const severity of [null, 'กลาง', 'สูง']) {
    const findings = severity ? [{ side: 'ความจริง', severity, kind: 'fixture', where: 'rawData', detail: 'synthetic truth finding', fix: 'review' }] : [];
    const { result } = await execute('1', { readinessCrash: true, findings });
    assert.equal(result.ok, true);
    assert.ok(result.insight.topicsV2);
    assert.deepEqual(result.brain.degradations.filter((d) => d.type === 'readiness-crashed'),
      [{ type: 'readiness-crashed', why: 'synthetic readiness crash' }]);
    assert.equal(result.brain.status, severity === 'สูง' ? 'ต้องตรวจ' : severity === 'กลาง' ? 'มีข้อสังเกต' : 'สะอาด');
    assert.equal(Object.hasOwn(result.brain.check, 'readiness'), false);
    assert.ok(result.brain.check.code);
  }
});

test('readiness pipeline diagnostics stay out of repair and status; quality survives real normalization by story ID', async () => {
  const d = doc();
  d.stories[0].quotes = [{ text: 'คน', speaker: '', evidenceIds: ['t1'], verification: 'pending' }];
  const actual = await execute('1', { composeReplies: [{ ok: true, json: d }] });
  const { insight, brain } = actual.result;
  assert.ok(brain.topicsV2?.ok, JSON.stringify(brain.topicsV2));
  assert.equal(brain.status, 'สะอาด', 'Readiness issues and low quotes must not change truth status');
  assert.equal(brain.check.repair, null);
  assert.equal(actual.calls.filter((c) => c.label === 'ตัวซ่อม').length, 0);
  assert.equal(brain.check.lowCount, 1);
  assert.ok(brain.check.readiness.findings.length > 0);
  assert.ok(brain.check.readiness.findings.every((f) => f.side === 'ความพร้อม' && f.severity === 'ข้อสังเกต'));
  { const { 'style-soft': soft, ...byCode } = brain.check.readiness.counts.byCode; // ★ 9 ก.ย. 69: สำนวนระดับอ่อนเป็นข้อสังเกตเพิ่ม ไม่กระทบ code เดิม
    assert.deepEqual({ ...brain.check.readiness.counts, byCode }, { stories: 1, withIssues: 1, byCode: { 'long-sentence': 1, 'quote-short': 1 } });
    assert.ok(soft >= 1, 'fixture prose carries soft style observations'); }
  assert.equal(insight.topicsV2.stories[0].quotes[0].verification, 'unverified');
  assert.equal(insight.topicsV2.mainStoryQuality.status, 'checked');
  assert.deepEqual(insight.subStories[0].quality, insight.topicsV2.stories[0].quality);
  assert.notEqual(insight.subStories[0].quality, insight.topicsV2.stories[0].quality);
  const normalized = normalizeInsight(insight, 'clip-brain');
  assert.deepEqual(normalized.topicsV2, insight.topicsV2);
  assert.deepEqual(normalized.subStories[0].quality, insight.subStories[0].quality);
  assert.equal(normalized.subStories[0].rawData, d.stories[0].story);
});

test('readiness pipeline verifies quotes against truth, including a quote supported only by extraction', async () => {
  const d = doc();
  const exact = 'คนในชุมชนช่วยกันซ่อมบ้านและจัดเตรียมร้านก่อนเปิดขายของในตลาด';
  d.stories[0].quotes = [
    { text: exact, speaker: '', evidenceIds: ['t1'], verification: 'pending' },
    { text: 'This quotation is not part of the recording', speaker: '', evidenceIds: ['q1'], verification: 'pending' },
    { text: 'คน', speaker: '', evidenceIds: ['t1'], verification: 'pending' },
  ];
  const actual = await execute('1', { composeReplies: [{ ok: true, json: d }],
    extracted: { ...BASE, quotes: [d.stories[0].quotes[1].text] } });
  assert.ok(actual.result.brain.topicsV2?.ok, JSON.stringify(actual.result.brain.topicsV2));
  assert.deepEqual(actual.result.insight.topicsV2.stories[0].quotes.map((q) => q.verification), ['verified', 'unverified', 'unverified']);
  assert.equal(actual.result.brain.status, 'ต้องตรวจ', 'Unsupported long quote still uses existing factual repair');
  assert.ok(actual.calls.some((c) => c.label === 'ตัวซ่อม'));
  assert.deepEqual(codes(actual.result.insight.topicsV2.stories[0].quality.issues).filter((code) => code.startsWith('quote-')),
    ['quote-unverified', 'quote-short']);
});

test('low-only code and AI findings keep clean status with accurate lowCount, medium stays observed', async () => {
  for (const severity of ['ต่ำ', 'กลาง', 'สูง']) {
    const finding = { kind: 'อื่นๆ', severity, where: 'ข้อความ', detail: 'ทบทวน', fix: 'ตรวจต้นทาง' };
    const actual = await execute('0', { findings: [finding], extracted: { ...BASE, quotes: ['เปิดร้าน'] } });
    assert.equal(actual.result.brain.status, severity === 'ต่ำ' ? 'สะอาด' : severity === 'กลาง' ? 'มีข้อสังเกต' : 'ต้องตรวจ');
    assert.equal(actual.result.brain.check.lowCount, severity === 'ต่ำ' ? 2 : 1);
    assert.equal(actual.result.brain.check.ai.findings[0].side, 'ความจริง');
    assert.equal(actual.result.brain.check.ai.findings[0].severity, severity);
    assert.equal(actual.calls.filter((c) => c.label === 'ตัวซ่อม').length, severity === 'สูง' ? 1 : 0);
    assert.equal(actual.result.brain.check.code.findings[0].kind, 'คำพูดสั้นตรวจไม่ได้');
    assert.equal(Object.hasOwn(finding, 'side'), false, 'Do not mutate AI response');
  }
});

test('readiness recomputes quote verification from accepted repaired quotes, with no second AI review', async () => {
  const d = doc();
  const bad = 'This quotation is not part of the recording';
  const exact = 'คนในชุมชนช่วยกันซ่อมบ้านและจัดเตรียมร้านก่อนเปิดขายของในตลาด';
  d.stories[0].quotes = [{ text: bad, speaker: '', evidenceIds: ['q1'], verification: 'pending' }];
  const actual = await execute('1', { composeReplies: [{ ok: true, json: d }], extracted: { ...BASE, quotes: [bad] },
    repairReply: { ok: true, json: { patch: { quotes: [exact], subStories: [{ no: 1, quotes: [exact] }] }, changed: [], unfixed: [] } } });
  const { insight, brain } = actual.result;
  assert.ok(brain.topicsV2?.ok, JSON.stringify(brain.topicsV2));
  assert.equal(brain.check.code.findings.filter((f) => f.severity === 'สูง').length, 2);
  assert.equal(brain.recheck.high, 0);
  assert.equal(brain.status, 'ซ่อมแล้ว');
  assert.deepEqual(insight.topicsV2.stories[0].quotes, [{ text: exact, speaker: '', evidenceIds: [], verification: 'verified' }]);
  assert.equal(codes(insight.topicsV2.stories[0].quality.issues).some((code) => code.startsWith('quote-')), false);
  assert.equal(actual.calls.filter((c) => c.label === 'ผู้ตรวจ').length, 1);
});

test('readiness binds legacy quality by storyId across different orders and preserves unmatched stories', async () => {
  const d = doc();
  d.stories[0].story = paragraphs(120);
  d.stories.push({ ...structuredClone(d.stories[0]), id: 'second', story: paragraphs(120, 1), highlight: '' });
  const extracted = toLegacyInsight(d, BASE);
  extracted.subStories.reverse();
  extracted.subStories.push({ topic: 'เรื่องเก่า', rawData: ORIGINAL_TEXT, storyId: 'unmatched',
    quality: { status: 'not_checked', issues: ['sentinel'] } });
  const { result } = await execute('0', { extracted });
  assert.equal(result.insight.subStories[0].storyId, 'second');
  const noSoft = (issues) => issues.filter((i) => i.code !== 'style-soft'); // ★ 9 ก.ย. 69: สำนวนระดับอ่อนเป็นข้อสังเกตเพิ่ม ไม่กระทบการผูก quality ตาม storyId
  assert.deepEqual(codes(noSoft(result.insight.subStories[0].quality.issues)), ['no-highlight']);
  assert.equal(result.insight.subStories[1].storyId, 's1');
  assert.equal(result.insight.subStories[1].quality.status, 'checked');
  assert.deepEqual(noSoft(result.insight.subStories[1].quality.issues), []);
  assert.deepEqual(result.insight.subStories[2].quality, { status: 'not_checked', issues: ['sentinel'] });
  { const { 'style-soft': soft, ...byCode } = result.brain.check.readiness.counts.byCode; // ★ 9 ก.ย. 69: ตัด style-soft ออกก่อนเทียบ code เดิม
    assert.deepEqual(byCode, { 'no-highlight': 1 });
    assert.equal(result.brain.check.readiness.counts.stories, 2);
    assert.ok(result.brain.check.readiness.counts.withIssues >= 1); void soft; }
  for (const normalize of [normalizeInsight, normalizeCapped]) {
    const normalized = normalize(result.insight, 'clip-brain');
    assert.deepEqual(normalized.topicsV2, result.insight.topicsV2);
    assert.deepEqual(normalized.subStories.map((s) => s.quality), result.insight.subStories.map((s) => s.quality));
  }
});

test('readiness does not enter the capped repair list or clear unresolved high AI findings', async () => {
  const d = doc();
  const findings = Array.from({ length: 14 }, (_, i) => ({ kind: 'ของงอก', severity: 'สูง',
    where: 'จุดตรวจ ' + i, detail: 'ตรวจหลักฐาน', fix: 'แก้ตามต้นทาง' }));
  const actual = await execute('1', { composeReplies: [{ ok: true, json: d }], findings });
  const { brain } = actual.result;
  assert.equal(brain.status, 'ต้องตรวจ');
  assert.equal(brain.check.repair.unverifiedAi.length, 14);
  assert.deepEqual(brain.degradations.find((x) => x.type === 'repair-capped'), { type: 'repair-capped', got: 12, want: 14 });
  const prompt = actual.calls.find((c) => c.label === 'ตัวซ่อม').prompt;
  assert.ok(prompt.includes('จุดตรวจ 11'));
  assert.equal(prompt.includes('จุดตรวจ 12'), false);
  assert.equal(prompt.includes('[ความพร้อมใช้งาน]'), false);
  assert.ok(brain.check.readiness.findings.length);
});

test('stale main narrative remains intact after actual accepted stage 6 repair and readiness sees repaired text', async () => {
  const d = doc();
  const bad = 'The course takes twenty years.';
  const good = 'The course takes six months.';
  d.stories[0].story = words(110) + '\n' + bad;
  const corrected = d.stories[0].story.replace(bad, good);
  const actual = await execute('1', { composeReplies: [{ ok: true, json: d }],
    findings: [{ kind: 'ของงอก', severity: 'สูง', where: bad, detail: 'ระยะเวลาเรียนผิด', fix: good }],
    repairReply: { ok: true, json: { patch: { subStories: [{ no: 1, rawData: corrected }] }, unfixed: [],
      changed: [{ fromFinding: 1, summary: 'แก้เวลาเรียน', edits: [{ path: 'subStories.1.rawData', before: bad, after: good }] }] } } });
  const { insight, brain } = actual.result;
  assert.equal(insight.topicsV2.stories[0].story, corrected);
  assert.equal(insight.mainStory, d.mainStory);
  assert.equal(insight.topicsV2.mainStory, d.mainStory);
  assert.equal(insight.topicsV2.mainStoryStale, true);
  assert.equal(insight.topicsV2.mainStoryQuality.status, 'checked');
  assert.ok(codes(insight.topicsV2.mainStoryQuality.issues).includes('main-story-stale'));
  assert.equal(brain.check.readiness.counts.byCode['main-story-stale'], 1);
  assert.equal(brain.status, 'ซ่อมแล้ว');
  assert.equal(brain.recheck.high, 0);
  assert.equal(brain.recheck.unverifiedAi, 0);
  assert.ok(brain.topicsV2.syncedAfterRepair.includes('topicsV2.mainStoryStale'));
});

test('sync stale follows mainTopicId through reordering, persists on repeated sync and never overwrites mainStory', async () => {
  const d = doc();
  d.stories.push({ ...structuredClone(d.stories[0]), id: 'second', story: words(120, 1) });
  let input = toLegacyInsight(d, BASE);
  input.subStories.reverse();
  input.subStories[0].rawData = words(130, 1);
  let synced = syncTopicsV2FromLegacy(freeze(input));
  assert.equal(Object.hasOwn(synced, 'mainStoryStale'), false, 'Secondary repairs do not stale main narrative');
  assert.equal(synced.insight.mainStory, d.mainStory);
  input = structuredClone(synced.insight);
  input.subStories[1].rawData = words(130);
  const snapshot = structuredClone(input);
  synced = syncTopicsV2FromLegacy(freeze(input));
  assert.deepEqual(input, snapshot);
  assert.equal(synced.mainStoryStale, true);
  assert.equal(synced.insight.mainStory, d.mainStory);
  assert.equal(synced.insight.topicsV2.mainStory, d.mainStory);
  const repeated = syncTopicsV2FromLegacy(synced.insight);
  assert.deepEqual(repeated.changed, []);
  assert.equal(repeated.mainStoryStale, true);
  assert.ok(codes((await assessReadiness(repeated.insight, { truth: ORIGINAL_TEXT })).mainStory.issues).includes('main-story-stale'));
});

test('readiness missing composer output adds no metadata; legacy off stays deep-equal immutable pre-P3 snapshots', async () => {
  for (const flag of [undefined, '0', '', 'true', '01', ' 1 ']) assert.deepEqual(await execute(flag), SNAPSHOTS.clean);
  assert.deepEqual(await execute('0', { long: true, missing: true }), SNAPSHOTS.partial);
  assert.deepEqual(await execute(undefined, { truth: { transcription: [] } }), SNAPSHOTS.noTruth);
  const failed = await execute('1', { composeCrash: true });
  assert.equal(Object.hasOwn(failed.result.brain.check, 'readiness'), false);
  assert.equal(Object.hasOwn(failed.result.insight, 'topicsV2'), false);
});

test('readiness pipeline timeout default is 1200000 while valid env override survives', async () => {
  assert.equal((await execute('1')).calls.find((c) => c.label.startsWith('clip-compose')).timeoutMs, 1200000);
  process.env.CLIP_TOPIC_TIMEOUT_MS = '1500000';
  assert.equal((await execute('1')).calls.find((c) => c.label.startsWith('clip-compose')).timeoutMs, 1500000);
});

for (const [mutation, pattern] of [
  ['crash', '^readiness crash stays ok'],
  ['readiness', '^readiness pipeline diagnostics'],
  ['low', '^low-only code'],
  ['stale', '^stale main narrative'],
  ['quote', '^readiness pipeline verifies quotes'],
]) {
  test('mutation ' + mutation + ': real assertions reject the defective source', { skip: !!process.env.CLIP_READINESS_MUTATION }, () => {
    const env = { ...process.env, CLIP_READINESS_MUTATION: mutation };
    delete env.NODE_TEST_CONTEXT;
    const child = spawnSync(process.execPath, ['--test', '--test-name-pattern', pattern, fileURLToPath(import.meta.url)],
      { env, encoding: 'utf8', timeout: 30000, maxBuffer: 4 * 1024 * 1024 });
    assert.equal(child.error, undefined);
    assert.equal(child.status, 1, child.stdout + '\n' + child.stderr);
    assert.match(child.stdout + child.stderr, /ERR_ASSERTION|AssertionError/);
  });
}

// ★ 9 ก.ย. 69 (Fable): สำนวนระดับอ่อน (ประโยคยาว/คำเฟ้อ) เป็นข้อสังเกตความพร้อม ไม่ใช่เหตุตกด่าน
test('readiness reports soft style issues (long sentence, filler) as observations with a fix hint', async () => {
  const d = doc();
  d.stories[0].story = Array.from({ length: 30 }, (_, i) => ['คน', 'รถ', 'บ้าน', 'น้ำ', 'งาน'][i % 5]).join(' ') + '\nทั้งนี้ ดังกล่าว พยายาม กลายเป็น เป็นการ\n' + d.stories[0].story;
  const r = await assess(d);
  const soft = r.findings.filter((f) => f.detail && (f.detail.includes('ประโยคยาวเกิน') || f.detail.includes('คำเฟ้อ')));
  assert.ok(soft.length >= 2, 'both soft issues reported: ' + JSON.stringify(r.findings.map((f) => f.detail)));
  assert.ok(soft.every((f) => f.side === 'ความพร้อม' && f.severity === 'ข้อสังเกต' && f.fix && f.fix.includes('ปรับสำนวน')));
});
