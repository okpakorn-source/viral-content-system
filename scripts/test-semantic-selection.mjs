...
// AC-0099 LANE-B: import the canonical strict-latch key list directly (refSlotContract is a
// PURE module with zero imports, so this static import needs no @/ loader hook and is safe
// above register()). STRICT_KEYS below derives from + asserts against this single source.
import { STRICT_LATCH_KEYS } from '../src/lib/refSlotContract.js';
...
// ★ AC-0099 LANE-B: derive from the contract's canonical STRICT_LATCH_KEYS so this harness's
//   key list can NEVER drift from the producer/renderer authority. A drift-guard test below
//   assert-equals it against the literal expected list.
const STRICT_KEYS = [...STRICT_LATCH_KEYS];
...
// ★ AC-0099 LANE-B drift guard: the harness key list MUST equal the canonical contract list.
//   Adding/removing/renaming a strict latch in refSlotContract.js will fail HERE until this
//   harness is updated — the two lists are structurally prevented from silently diverging.
await test('Checkpoint C: STRICT_KEYS derives from canonical STRICT_LATCH_KEYS (no drift)', async () => {
  assert.deepEqual(STRICT_KEYS, STRICT_LATCH_KEYS, 'STRICT_KEYS must equal the contract STRICT_LATCH_KEYS');
  assert.deepEqual(STRICT_KEYS, ['MEGA_SEMANTIC_SELECTION', 'MEGA_SELECTION_SPEC', 'MEGA_STRICT_PRODUCER', 'MEGA_STRICT_RENDER', 'MEGA_REF_SHOT_AUTHORITY'],
    'canonical strict latch list changed — update every strict consumer in lockstep');
});
