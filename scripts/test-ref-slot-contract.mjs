...
// ═══════════ 🎯 D3-B1 — REF SHOT AUTHORITY (template_v1) PURE FOUNDATION ═══════════
// template.slots = แกน · DNA semantic merged by role+occurrence เท่านั้น (ไม่มี env — mode explicit)

// fixture: DNA "reorder + shot conflict + cross-wire trap"
//   template = [hero(closeup), context(blank shot,no subject), moment-circle(blank shot)]
//   dna       = [moment, hero(medium,'ตัวเอก-dna'), context(wide,'สถานที่')]  ← เรียงคนละลำดับ template
const D3_DNA = {
  template: {
    slots: [
      { role: 'hero', shape: 'rect', xPct: 0, yPct: 0, wPct: 55, hPct: 100, shot: 'closeup', pos: 'left', faceSizePct: 60, subject: 'ตัวเอก' },
      { role: 'context', shape: 'rect', xPct: 55, yPct: 0, wPct: 45, hPct: 60, shot: '', pos: 'right' },
      { role: 'moment', shape: 'circle', xPct: 60, yPct: 60, wPct: 30, hPct: 30, shot: '' },
    ],
  },
  slots: [
    { role: 'moment', shot: '', emotion: 'ช็อก', subject: 'เหตุการณ์', facing: 'front' },
    { role: 'hero', shot: 'medium', emotion: 'เศร้า', subject: 'ตัวเอก-dna', facing: 'side' },
    { role: 'context', shot: 'wide', emotion: 'สงบ', subject: 'สถานที่' },
  ],
};

test('D3B-1 legacy parity (canonical input เดียว): default / legacy / "0" / junk / unknown-key → deepEqual golden · ไม่มี authority field', () => {
  const golden = buildRefSlotContract({ refDNA: D3_DNA });
  for (const opts of [
    { refDNA: D3_DNA },
    { refDNA: D3_DNA, mode: 'legacy' },
    { refDNA: D3_DNA, mode: '0' },
    { refDNA: D3_DNA, mode: 'junk' },
    { refDNA: D3_DNA, mode: 'template_v2' }, // อะไรที่ไม่ใช่ 'template_v1' = legacy
    { refDNA: D3_DNA, refShotAuthority: true }, // ★ P0-1: key ที่ถูกลบแล้ว = ignored → legacy (ไม่มี dual API)
  ]) {
    const c = buildRefSlotContract(opts);
    assert.deepStrictEqual(c, golden, `legacy path ต้องเท่า golden (${JSON.stringify(opts.mode ?? 'refShotAuthority')})`);
    assert.ok(!('authority' in c), 'legacy contract ห้ามมี field authority');
    assert.ok(!c.slots.some((s) => 'refShotProvenance' in s || 'semanticMatched' in s), 'legacy slot ห้ามมี per-slot metadata ใหม่');
  }
  // golden = พฤติกรรม cross-wire เดิมจริง (context ได้ subject ของ hero-dna ตาม index)
  const ctx = golden.slots.find((s) => s.refRole === 'context');
  assert.equal(ctx.subject, 'ตัวเอก-dna', 'legacy cross-wire (index merge) ต้องคงอยู่เป๊ะ');
});

test('D3B-2 template_v1 reorder + unequal role match: axis=template · semantic role+occurrence · ไม่ cross-wire', () => {
  const c = buildRefSlotContract({ refDNA: D3_DNA, mode: 'template_v1' });
  assert.equal(c.source, 'template.slots');
  assert.deepEqual(c.slots.map((s) => s.refRole), ['hero', 'context', 'moment'], 'axis ตาม template order');
  const by = Object.fromEntries(c.slots.map((s) => [s.refRole, s]));
  // hero: shot template ชนะ (closeup ไม่ใช่ medium) · subject template ('ตัวเอก')
  assert.equal(by.hero.refShot, 'closeup');
  assert.equal(by.hero.subject, 'ตัวเอก');
  // context: template shot ว่าง → fallback dna context ('wide') · subject template ว่าง → dna context ('สถานที่')
  //   ★ พิสูจน์ไม่ cross-wire: legacy จะได้ 'ตัวเอก-dna' (hero-dna by index) — template_v1 ต้องได้ 'สถานที่'
  assert.equal(by.context.refShot, 'wide');
  assert.equal(by.context.subject, 'สถานที่');
  assert.notEqual(by.context.subject, 'ตัวเอก-dna', 'context ห้ามได้ subject ของ hero-dna (cross-wire)');
  // moment: shot ทั้งคู่ว่าง → null · subject dna moment ('เหตุการณ์')
  assert.equal(by.moment.refShot, null);
  assert.equal(by.moment.subject, 'เหตุการณ์');
  // resolver ยืนยัน emotion role-matched (ไม่ cross-role)
  const view = resolveRefSlotView(D3_DNA, { mode: 'template_v1' });
  const vBy = Object.fromEntries(view.views.map((v) => [v.role, v]));
  assert.equal(vBy.hero.emotion, 'เศร้า');
  assert.equal(vBy.context.emotion, 'สงบ');
  assert.equal(vBy.moment.emotion, 'ช็อก', 'moment ต้องได้ emotion ของตัวเอง ไม่ใช่ hero');
});

test('D3B-3 duplicate roles nth-to-nth deterministic · ไม่ reuse DNA slot ซ้ำ', () => {
  const dna2 = {
    template: { slots: [
      { role: 'context', shape: 'rect', xPct: 0, yPct: 0, wPct: 50, hPct: 100 },
      { role: 'context', shape: 'rect', xPct: 50, yPct: 0, wPct: 50, hPct: 100 },
    ] },
    slots: [
      { role: 'context', subject: 'A', shot: 'wide' },
      { role: 'context', subject: 'B', shot: 'medium' },
    ],
  };
  const c = buildRefSlotContract({ refDNA: dna2, mode: 'template_v1' });
  assert.equal(c.slots.length, 2);
  assert.deepEqual(c.slots.map((s) => s.id), ['context', 'context_2'], 'id unique ต่อ role ซ้ำ');
  assert.equal(c.slots[0].subject, 'A'); // ตัวที่ 1 ↔ dna ตัวที่ 1
  assert.equal(c.slots[1].subject, 'B'); // ตัวที่ 2 ↔ dna ตัวที่ 2 (ไม่ reuse A)
  assert.equal(c.slots[0].refShot, 'wide');
  assert.equal(c.slots[1].refShot, 'medium');
  // dna มี context เดียว template 2 → ตัวที่สอง semantic null (ไม่ reuse ตัวแรก)
  const dna1 = { template: dna2.template, slots: [{ role: 'context', subject: 'ONLY', shot: 'wide' }] };
  const c1 = buildRefSlotContract({ refDNA: dna1, mode: 'template_v1' });
  assert.equal(c1.slots[0].subject, 'ONLY');
  assert.equal(c1.slots[1].subject, null, 'context ตัวที่ 2 ไม่มี dna คู่ → subject null ห้าม reuse');
  assert.equal(c1.slots[1].refShot, null);
});

test('D3B-4 template-only role อยู่+semantic null · DNA-only role ถูกละ (danglingDnaRoles)', () => {
  const dna = {
    template: { slots: [
      { role: 'hero', shape: 'rect', xPct: 0, yPct: 0, wPct: 55, hPct: 100, shot: 'closeup' },
      { role: 'context', shape: 'rect', xPct: 55, yPct: 0, wPct: 45, hPct: 100 },
    ] },
    slots: [
      { role: 'hero', shot: 'medium', subject: 'ฮีโร่' },
      { role: 'evidence', subject: 'หลักฐาน' }, // DNA-only role — ห้ามสร้าง slot
    ],
  };
  const c = buildRefSlotContract({ refDNA: dna, mode: 'template_v1' });
  assert.deepEqual(c.slots.map((s) => s.refRole), ['hero', 'context'], 'มีแค่บท template · evidence(dna-only) ไม่โผล่');
  const ctx = c.slots.find((s) => s.refRole === 'context');
  assert.equal(ctx.subject, null, 'template-only role → semantic null');
  assert.equal(ctx.refShot, null);
  const view = resolveRefSlotView(dna, { mode: 'template_v1' });
  assert.deepEqual(view.diagnostics.danglingDnaRoles, [{ index: 1, role: 'evidence' }], 'dna-only role รายงานใน dangling');
});

test('D3B-5 shot resolution: template ชนะ · blank template → dna fallback · both blank → null + provenance', () => {
  const view = resolveRefSlotView(D3_DNA, { mode: 'template_v1' });
  const by = Object.fromEntries(view.views.map((v) => [v.role, v]));
  assert.equal(by.hero.shot, 'closeup');
  assert.equal(by.hero.shotProvenance, 'template');
  assert.equal(by.context.shot, 'wide');
  assert.equal(by.context.shotProvenance, 'dna');
  assert.equal(by.moment.shot, null);
  assert.equal(by.moment.shotProvenance, 'none');
  // subject provenance เช่นกัน
  assert.equal(by.hero.subjectProvenance, 'template');
  assert.equal(by.context.subjectProvenance, 'dna');
});

test('D3B-6 order role-safe: index ผิด role ห้าม bind · same-role occurrence binds · order.shot ไม่ override template', () => {
  // order i:0 แต่ role='context' ในขณะ template slot0 = hero → ห้าม bind (want/personHint ต้อง null)
  const c1 = buildRefSlotContract({
    refDNA: D3_DNA, mode: 'template_v1',
    artBriefOrders: [{ i: 0, role: 'context', want: 'ผิดช่อง', personHint: 'คนผิด', shot: 'wide' }],
  });
  const h1 = c1.slots.find((s) => s.refRole === 'hero');
  assert.equal(h1.eventIntent, null, 'order role ผิดที่ index 0 ห้ามผูก hero');
  assert.equal(h1.wantPerson, null);
  assert.equal(h1.refShot, 'closeup', 'order.shot ไม่ override template effective shot');
  // order role ตรง (ไม่มี i) → bind by occurrence
  const c2 = buildRefSlotContract({
    refDNA: D3_DNA, mode: 'template_v1',
    artBriefOrders: [{ role: 'hero', want: 'ถูกช่อง', personHint: 'ตัวเอก', shot: 'wide' }],
  });
  const h2 = c2.slots.find((s) => s.refRole === 'hero');
  assert.equal(h2.eventIntent, 'ถูกช่อง');
  assert.equal(h2.wantPerson, 'ตัวเอก');
  assert.equal(h2.refShot, 'closeup', 'แม้ order role ตรง order.shot ก็ไม่ override');
});

// local mirror ของ cleanRole (ไม่ export) — เทียบ role กับ template ตรงๆ
const _norm = (v, fb) => (String(v || '').trim().toLowerCase().replace(/[^a-z0-9]+/g, '_').replace(/^_+|_+$/g, '') || fb);

test('D3B-7 ทุก refs template_v1: clean role/count/order = template · full geometry (5 ฟิลด์) · effectiveHash นิ่ง 2 รอบ', () => {
  const refs = JSON.parse(fs.readFileSync(new URL('../data/ref-cover-library.json', import.meta.url), 'utf8'));
  let checked = 0;
  for (const rec of refs) {
    const tpl = rec.dna?.template?.slots;
    if (!Array.isArray(tpl) || !tpl.length) continue;
    checked++;
    const a = buildRefSlotContract({ refDNA: rec.dna, mode: 'template_v1' });
    const b = buildRefSlotContract({ refDNA: rec.dna, mode: 'template_v1' });
    assert.equal(JSON.stringify(a), JSON.stringify(b), `${rec.id}: deterministic 2 รอบ`);
    assert.equal(a.authority.effectiveViewHash, b.authority.effectiveViewHash, `${rec.id}: effectiveViewHash byte-identical 2 รอบ`);
    assert.equal(a.authority.mode, 'template_v1');
    assert.equal(a.authority.axis, 'template.slots');
    assert.equal(a.authority.axisReady, true);
    assert.equal(a.slots.length, tpl.length, `${rec.id}: count = template`);
    a.slots.forEach((s, i) => {
      assert.equal(s.sourceIndex, i, `${rec.id}: axis order sequential`);
      assert.equal(s.refRole, _norm(tpl[i]?.role ?? tpl[i]?.id, `slot_${i + 1}`), `${rec.id}[${i}]: clean role = template`);
      const g = s.geometry;
      const t = tpl[i];
      const gt = [t.xPct, t.yPct, t.wPct, t.hPct].every((n) => Number.isFinite(Number(n)));
      if (gt) {
        // full geometry: xPct/yPct/wPct/hPct/zIndex
        assert.equal(g.xPct, Number(t.xPct), `${rec.id}[${i}]: geometry.xPct`);
        assert.equal(g.yPct, Number(t.yPct), `${rec.id}[${i}]: geometry.yPct`);
        assert.equal(g.wPct, Number(t.wPct), `${rec.id}[${i}]: geometry.wPct`);
        assert.equal(g.hPct, Number(t.hPct), `${rec.id}[${i}]: geometry.hPct`);
        assert.equal(g.zIndex, Number(t.zIndex) || 0, `${rec.id}[${i}]: geometry.zIndex`);
      }
    });
  }
  assert.ok(checked >= 15, `ต้องตรวจ ref ที่มี template อย่างน้อย 15 (ได้ ${checked})`);
});

test('D3B-9 main↔hero alias สองทิศ + exact-first + duplicate (ไม่ pool ก่อน exact, ไม่ reuse)', () => {
  // (a) template hero, DNA main → alias
  const A = { template: { slots: [{ role: 'hero', shape: 'rect', xPct: 0, yPct: 0, wPct: 50, hPct: 100, shot: 'closeup' }] }, slots: [{ role: 'main', subject: 'ตัวเอก', shot: 'medium', emotion: 'ยิ้ม' }] };
  const ca = buildRefSlotContract({ refDNA: A, mode: 'template_v1' });
  assert.equal(ca.slots[0].refRole, 'hero', 'preserve template refRole (ไม่กลายเป็น main)');
  assert.equal(ca.slots[0].subject, 'ตัวเอก', 'hero ได้ semantic จาก main (alias)');
  assert.equal(ca.slots[0].refShot, 'closeup', 'template shot ยังชนะ');
  assert.equal(ca.slots[0].aliasUsed, true);
  assert.equal(ca.slots[0].semanticIndex, 0);
  assert.deepEqual(ca.authority.viewDiagnostics.aliasMatches, [{ index: 0, role: 'hero', fromRole: 'main', semanticRole: 'main', semanticIndex: 0 }], 'aliasMatches พก fromRole/semanticRole สำหรับ D1');
  // (b) template main, DNA hero → alias (ทิศกลับ)
  const B = { template: { slots: [{ role: 'main', shape: 'rect', xPct: 0, yPct: 0, wPct: 50, hPct: 100 }] }, slots: [{ role: 'hero', subject: 'ฮ' }] };
  const cb = buildRefSlotContract({ refDNA: B, mode: 'template_v1' });
  assert.equal(cb.slots[0].refRole, 'main');
  assert.equal(cb.slots[0].subject, 'ฮ');
  assert.equal(cb.slots[0].aliasUsed, true);
  // (c) exact-first: template [main, hero], DNA [hero, main] → exact-to-exact, ไม่มี alias
  const C = { template: { slots: [
    { role: 'main', shape: 'rect', xPct: 0, yPct: 0, wPct: 50, hPct: 100 },
    { role: 'hero', shape: 'rect', xPct: 50, yPct: 0, wPct: 50, hPct: 100 },
  ] }, slots: [{ role: 'hero', subject: 'H' }, { role: 'main', subject: 'M' }] };
  const cc = buildRefSlotContract({ refDNA: C, mode: 'template_v1' });
  const cBy = Object.fromEntries(cc.slots.map((s) => [s.refRole, s]));
  assert.equal(cBy.main.subject, 'M', 'main ← DNA main (exact) ไม่ pool');
  assert.equal(cBy.hero.subject, 'H', 'hero ← DNA hero (exact)');
  assert.ok(cc.slots.every((s) => !s.aliasUsed), 'ไม่มี alias เมื่อ exact ครบ');
  assert.deepEqual(cc.authority.viewDiagnostics.aliasMatches, []);
  // (d) duplicate: template [hero, hero], DNA [main, main] → alias nth↔nth ไม่ reuse
  const D = { template: { slots: [
    { role: 'hero', shape: 'rect', xPct: 0, yPct: 0, wPct: 50, hPct: 100 },
    { role: 'hero', shape: 'rect', xPct: 50, yPct: 0, wPct: 50, hPct: 100 },
  ] }, slots: [{ role: 'main', subject: 'M1' }, { role: 'main', subject: 'M2' }] };
  const cd = buildRefSlotContract({ refDNA: D, mode: 'template_v1' });
  assert.equal(cd.slots[0].subject, 'M1');
  assert.equal(cd.slots[1].subject, 'M2', 'hero ตัวที่ 2 ← main ตัวที่ 2 (ไม่ reuse M1)');
  assert.deepEqual(cd.slots.map((s) => s.semanticIndex), [0, 1]);
});

test('D3B-10 order preassign no-reuse: reversed/sparse duplicate + extra/missing + unique binding', () => {
  const dna = { template: { slots: [
    { role: 'context', shape: 'rect', xPct: 0, yPct: 0, wPct: 50, hPct: 100 },
    { role: 'context', shape: 'rect', xPct: 50, yPct: 0, wPct: 50, hPct: 100 },
  ] }, slots: [] };
  // orders สอง context ตัว (สลับลำดับ i) — ต้องผูก 1:1 ไม่ reuse
  const c = buildRefSlotContract({
    refDNA: dna, mode: 'template_v1',
    artBriefOrders: [
      { i: 1, role: 'context', want: 'ช่องขวา', personHint: 'B' },
      { i: 0, role: 'context', want: 'ช่องซ้าย', personHint: 'A' },
    ],
  });
  assert.equal(c.slots[0].eventIntent, 'ช่องซ้าย', 'passA exact index+role: slot0 ← order i:0');
  assert.equal(c.slots[1].eventIntent, 'ช่องขวา', 'slot1 ← order i:1');
  assert.equal(c.slots[0].wantPerson, 'A');
  assert.equal(c.slots[1].wantPerson, 'B');
  // sparse: order เดียวสำหรับ 2 context → ผูกได้ตัวเดียว อีกตัว null (ไม่ reuse)
  const c2 = buildRefSlotContract({ refDNA: dna, mode: 'template_v1', artBriefOrders: [{ role: 'context', want: 'เดียว' }] });
  assert.equal(c2.slots[0].eventIntent, 'เดียว');
  assert.equal(c2.slots[1].eventIntent, null, 'order 1 ตัวห้ามผูก 2 slots');
  // extra order (role ไม่มีใน template) → ไม่ผูกที่ไหน; missing → slot null
  const c3 = buildRefSlotContract({ refDNA: dna, mode: 'template_v1', artBriefOrders: [{ role: 'evidence', want: 'ผี' }, { role: 'context', want: 'จริง' }] });
  assert.equal(c3.slots[0].eventIntent, 'จริง');
  assert.equal(c3.slots[1].eventIntent, null);
  assert.ok(!c3.slots.some((s) => s.eventIntent === 'ผี'), 'order role นอก template ห้ามผูก slot ใด');
});

test('D3B-11 P0-2 template_v1 ไม่มี template.slots → axisReady=false + viewDiagnostics สะท้อน DNA จริง (ไม่ silent downgrade)', () => {
  const heroDna = { slots: [{ role: 'hero', subject: 'x', shot: 'closeup' }] };
  const c = buildRefSlotContract({ refDNA: heroDna, mode: 'template_v1' });
  assert.equal(c.source, 'template.slots', 'ห้ามตก DNA axis (source ต้องเป็น template.slots)');
  assert.deepEqual(c.slots, [], 'slots ว่าง — ไม่สร้างจาก DNA');
  assert.equal(c.authority.axisReady, false);
  assert.equal(c.authority.viewDiagnostics.templateCount, 0);
  assert.equal(c.authority.viewDiagnostics.dnaCount, 1);
  assert.deepEqual(c.authority.viewDiagnostics.danglingDnaRoles, [{ index: 0, role: 'hero' }], 'DNA ทุกตัว = dangling');
  const c2 = buildRefSlotContract({ refDNA: heroDna, mode: 'template_v1' });
  assert.equal(c.authority.effectiveViewHash, c2.authority.effectiveViewHash, 'not-ready hash นิ่ง 2 รอบ');
  // ★ DNA ต่างกัน → effectiveViewHash ต่าง แม้ slots=[] ทั้งคู่
  const ctxDna = { slots: [{ role: 'context', subject: 'x', shot: 'closeup' }] };
  const cc = buildRefSlotContract({ refDNA: ctxDna, mode: 'template_v1' });
  assert.notEqual(c.authority.effectiveViewHash, cc.authority.effectiveViewHash, 'hero-DNA vs context-DNA hash ต้องต่าง');
  assert.deepEqual(cc.authority.viewDiagnostics.danglingDnaRoles, [{ index: 0, role: 'context' }]);
  // ★ legacy/no-template ยังคง DNA fallback exact เดิม (source 'slots')
  const legacy = buildRefSlotContract({ refDNA: heroDna });
  assert.equal(legacy.source, 'slots');
  assert.equal(legacy.slots.length, 1, 'legacy no-template = DNA fallback เดิม');
  assert.ok(!('authority' in legacy));
});

test('D3B-12 effectiveViewHash: semantic field เปลี่ยน (geometry/shot เดิม) ต้องขยับ · แต่ artBrief order ห้ามขยับ', () => {
  const base = { template: { slots: [{ role: 'hero', shape: 'rect', xPct: 0, yPct: 0, wPct: 50, hPct: 100, shot: 'closeup' }] }, slots: [{ role: 'hero', emotion: 'ยิ้ม' }] };
  const variant = { template: base.template, slots: [{ role: 'hero', emotion: 'เศร้า' }] };
  const h1 = buildRefSlotContract({ refDNA: base, mode: 'template_v1' }).authority.effectiveViewHash;
  const h2 = buildRefSlotContract({ refDNA: variant, mode: 'template_v1' }).authority.effectiveViewHash;
  assert.notEqual(h1, h2, 'emotion ต่าง (geometry/shot เดิม) → effectiveViewHash ขยับ');
  const v2 = { template: base.template, slots: [{ role: 'hero', emotion: 'ยิ้ม', facing: 'side' }] };
  assert.notEqual(h1, buildRefSlotContract({ refDNA: v2, mode: 'template_v1' }).authority.effectiveViewHash, 'facing เปลี่ยน → hash ขยับ');
  // ★ B1.2 boundary: artBrief order เปลี่ยน (eventIntent/wantPerson) ห้ามขยับ effectiveViewHash
  const hOrderA = buildRefSlotContract({ refDNA: base, mode: 'template_v1', artBriefOrders: [{ role: 'hero', want: 'A', personHint: 'X' }] }).authority.effectiveViewHash;
  const hOrderB = buildRefSlotContract({ refDNA: base, mode: 'template_v1', artBriefOrders: [{ role: 'hero', want: 'ZZZ', personHint: 'YYY' }] }).authority.effectiveViewHash;
  assert.equal(h1, hOrderA, 'มี order ก็ไม่กระทบ effectiveViewHash (view-only)');
  assert.equal(hOrderA, hOrderB, 'order ต่างกัน → effectiveViewHash เท่ากัน (order อยู่คนละ diagnostic)');
});

test('D3B-14 axisReady boundary: มี template (แม้ missing shot/unmatched semantic/geometry เพี้ยน) = true + diagnostics', () => {
  // template มี hero(no shot, no dna match) + geometry เพี้ยน (ไม่มี xPct)
  const dna = { template: { slots: [
    { role: 'hero', shape: 'rect', shot: '' }, // ไม่มี geometry, shot ว่าง, ไม่มี dna match
    { role: 'context', shape: 'rect', xPct: 0, yPct: 0, wPct: 50, hPct: 100 },
  ] }, slots: [{ role: 'evidence', subject: 'ผี' }] };
  const c = buildRefSlotContract({ refDNA: dna, mode: 'template_v1' });
  assert.equal(c.authority.axisReady, true, 'มี template axis = axisReady true (ไม่ใช่ strict-ready)');
  assert.equal(c.slots.length, 2, 'ยังสร้าง slots ตาม template');
  const vd = c.authority.viewDiagnostics;
  assert.deepEqual(vd.missingShotSlots, [{ index: 0, role: 'hero' }, { index: 1, role: 'context' }], 'shot ว่างทั้งคู่ = missing');
  assert.deepEqual(vd.unmatchedTemplateRoles, [{ index: 0, role: 'hero' }, { index: 1, role: 'context' }], 'ไม่มี dna match = unmatched');
  assert.deepEqual(vd.danglingDnaRoles, [{ index: 0, role: 'evidence' }]);
  assert.equal(c.slots[0].geometry, null, 'geometry เพี้ยน (ไม่มี xPct) = null แต่ axisReady ยัง true');
});

test('D3B-15 order provenance/no-reuse: matchMode + orderDiagnostics (alias 2 ทิศ, exact-first, extra reported)', () => {
  // hero template ← order role 'main' (alias) — order hero-family 2 ทิศ
  const dna = { template: { slots: [
    { role: 'hero', shape: 'rect', xPct: 0, yPct: 0, wPct: 55, hPct: 100 },
    { role: 'context', shape: 'rect', xPct: 55, yPct: 0, wPct: 45, hPct: 100 },
  ] }, slots: [] };
  const c = buildRefSlotContract({
    refDNA: dna, mode: 'template_v1',
    artBriefOrders: [
      { role: 'main', want: 'ตัวเอก', personHint: 'H' }, // alias → hero
      { i: 1, role: 'context', want: 'บริบท', personHint: 'C' }, // exact_index → context
      { role: 'evidence', want: 'ส่วนเกิน' }, // extra
    ],
  });
  const hero = c.slots.find((s) => s.refRole === 'hero');
  const ctx = c.slots.find((s) => s.refRole === 'context');
  assert.equal(hero.eventIntent, 'ตัวเอก');
  assert.equal(hero.orderMatchMode, 'hero_alias');
  assert.equal(hero.orderIndex, 0);
  assert.equal(hero.orderRole, 'main');
  assert.equal(ctx.eventIntent, 'บริบท');
  assert.equal(ctx.orderMatchMode, 'exact_index');
  const od = c.authority.orderDiagnostics;
  assert.deepEqual(od.orderAliasMatches, [{ index: 0, role: 'hero', orderIndex: 0, orderRole: 'main' }]);
  assert.deepEqual(od.unmatchedOrderIndices, [2], 'order ส่วนเกินต้องรายงาน index');
  assert.deepEqual(od.unmatchedOrderRoles, ['evidence']);
  // no-reuse: order เดียว role hero + 2 hero slots → ผูกตัวเดียว
  const dna2 = { template: { slots: [
    { role: 'hero', shape: 'rect', xPct: 0, yPct: 0, wPct: 50, hPct: 100 },
    { role: 'hero', shape: 'rect', xPct: 50, yPct: 0, wPct: 50, hPct: 100 },
  ] }, slots: [] };
  const c2 = buildRefSlotContract({ refDNA: dna2, mode: 'template_v1', artBriefOrders: [{ role: 'hero', want: 'เดียว' }] });
  assert.equal(c2.slots[0].eventIntent, 'เดียว');
  assert.equal(c2.slots[1].eventIntent, null, 'order 1 ตัวห้ามผูก 2 slots');
  assert.equal(c2.slots[1].orderMatchMode, null);
});

test('D3B-16 (P1) template role ว่าง/whitespace → ถอยไป id: matcher+view role ตรงกัน (semantic fallback ไม่หาย)', () => {
  const dna = { template: { slots: [
    { role: '', id: 'hero', shape: 'rect', xPct: 0, yPct: 0, wPct: 55, hPct: 100, shot: '' },  // role ว่าง → id 'hero'
    { role: '   ', id: 'context', shape: 'rect', xPct: 55, yPct: 0, wPct: 45, hPct: 100 },      // whitespace → id 'context'
  ] }, slots: [
    { role: 'main', subject: 'ตัวเอก', shot: 'medium', emotion: 'ยิ้ม' }, // main → alias เข้า hero (role จาก id)
    { role: 'context', subject: 'สถานที่', shot: 'wide' },
  ] };
  const c = buildRefSlotContract({ refDNA: dna, mode: 'template_v1' });
  assert.deepEqual(c.slots.map((s) => s.refRole), ['hero', 'context'], 'role มาจาก id เมื่อ role ว่าง/whitespace');
  const hero = c.slots[0];
  // hero (role ว่าง→id hero) ต้องจับ DNA main แบบ alias — เดิม matcher เห็น slot_1 จะพลาด semantic
  assert.equal(hero.subject, 'ตัวเอก', 'semantic (subject) จาก DNA main ผ่าน alias — fallback ไม่หาย');
  assert.equal(hero.aliasUsed, true);
  assert.equal(hero.semanticIndex, 0);
  assert.equal(hero.refShot, 'medium', 'template shot ว่าง → fallback DNA main shot');
  assert.equal(hero.refShotProvenance, 'dna');
  const ctx = c.slots[1];
  assert.equal(ctx.subject, 'สถานที่', 'context (role whitespace→id) exact match DNA context');
  assert.equal(ctx.aliasUsed, false);
  assert.equal(ctx.refShot, 'wide');
  assert.deepEqual(c.slots.map((s) => s.semanticIndex), [0, 1], 'exact-first + no-reuse ยังถูกต้องกับ role จาก id');
  assert.deepEqual(c.authority.viewDiagnostics.aliasMatches, [{ index: 0, role: 'hero', fromRole: 'main', semanticRole: 'main', semanticIndex: 0 }]);
});

test('D3B-17 (P1) real-library REF-mrbqalpo-h1r1 template_v1: hero refShot=closeup (DNA medium ห้าม override) + provenance/semantic จริง', () => {
  const dna = loadLibraryRef('REF-mrbqalpo-h1r1');
  const c = buildRefSlotContract({ refDNA: dna, mode: 'template_v1' });
  const hero = c.slots.find((s) => s.refRole === 'hero');
  assert.ok(hero, 'ต้องมี hero slot');
  assert.equal(hero.sourceIndex, 0, 'hero occurrence 0 = template index 0 (axis template)');
  assert.equal(hero.refShot, 'closeup', 'template closeup ต้องชนะ DNA medium (authority contradiction แก้แล้ว)');
  assert.equal(hero.refShotProvenance, 'template');
  assert.equal(hero.semanticMatched, true);
  assert.equal(hero.semanticIndex, 0, 'DNA hero index 0 (exact same-role)');
  assert.equal(hero.aliasUsed, false, 'exact match ไม่ใช่ alias');
  // axis/order template-driven: 5 slots ตาม template order
  assert.deepEqual(c.slots.map((s) => s.refRole), ['hero', 'context', 'action', 'moment', 'reaction']);
  assert.equal(c.authority.axisReady, true);
  const view = resolveRefSlotView(dna, { mode: 'template_v1' });
  assert.equal(view.views[0].semanticRole, 'hero', 'semantic role = hero จริง (ไม่ cross-role)');
});

test('D3B-13 specHash identity-only: authority metadata ใหม่ห้ามขยับ specHash', () => {
  const dna = { template: { slots: [
    { role: 'hero', shape: 'rect', xPct: 0, yPct: 0, wPct: 55, hPct: 100, shot: 'closeup' },
    { role: 'context', shape: 'rect', xPct: 55, yPct: 0, wPct: 45, hPct: 100 },
    { role: 'moment', shape: 'circle', xPct: 8, yPct: 60, wPct: 30, hPct: 24 },
  ] }, slots: [{ role: 'hero', emotion: 'a' }] };
  const contract = buildRefSlotContract({ refDNA: dna, mode: 'template_v1' });
  const P = (cid) => ({ candidateId: cid, imageUrl: `https://cdn.test/${cid}.jpg`, backups: [] });
  const planned = { hero: P('H'), context: P('C'), moment: P('M') };
  const specA = buildSelectionSpec({ contract, realizedTemplate: dnaToTemplateSpec(dna), plannedByRefSlot: planned, refId: 'R' });
  // สร้าง contract ที่ strip authority + per-slot metadata ออก (identity เท่าเดิม)
  const stripped = { ...contract, slots: contract.slots.map((s) => ({ id: s.id, refRole: s.refRole, solverRole: s.solverRole, legacySlot: s.legacySlot, sourceIndex: s.sourceIndex, shape: s.shape, subject: s.subject, eventIntent: s.eventIntent, wantPerson: s.wantPerson, refShot: s.refShot, position: s.position, geometry: s.geometry })) };
  delete stripped.authority;
  const specB = buildSelectionSpec({ contract: stripped, realizedTemplate: dnaToTemplateSpec(dna), plannedByRefSlot: planned, refId: 'R' });
  assert.equal(specA.specHash, specB.specHash, 'specHash ต้องเท่ากัน — authority/metadata ไม่เข้า identity hash');
});

test('D3B-8 PURITY: source refSlotContract.js ห้ามมี process.env / import / IO', () => {
  const src = fs.readFileSync(new URL('../src/lib/refSlotContract.js', import.meta.url), 'utf8');
  assert.ok(!/process\s*\.\s*env/.test(src), 'ห้ามอ่าน process.env');
  assert.ok(!/\brequire\s*\(/.test(src), 'ห้าม require()');
  assert.ok(!/^\s*import\s/m.test(src), 'ห้าม import');
  assert.ok(!/\b(readFile|writeFile|fetch\s*\()/.test(src), 'ห้าม IO');
  assert.ok(!/Math\s*\.\s*random|Date\s*\.\s*now|new\s+Date/.test(src), 'ห้าม time/random');
});
