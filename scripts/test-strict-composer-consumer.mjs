...
await test('1) activation: OFF/absent = legacy parity เต็มผลลัพธ์ (deepEqual) · ON+present = strict ตื่น', async () => {
  // ★ รอบ 4 (P1-3): baseline = call ที่ "ไม่มี own selectionSpec" — ทุกทาง legacy ต้องได้ผลเต็มเท่ากันทุก field
  ...
  // unset + payload strict เต็ม → ผลลัพธ์เต็มเท่ากับ baseline (ห้ามเทียบแค่ errorType)
  const rOff = await composeAndVerify(mkArgs({ slotPlan: [] }));
  assert.deepEqual(rOff, baseline, 'switch unset + payload = consumer หลับสนิท ผลเต็มตรง baseline');
  process.env.MEGA_STRICT_RENDER = '0';
  try {
    const rZero = await composeAndVerify(mkArgs({ slotPlan: [] }));
    assert.deepEqual(rZero, baseline, "ค่า '0' + payload = ผลเต็มตรง baseline");
  ...
  await withStrict(async () => {
    ...
    assert.deepEqual(rAbsent, baseline, 'ON + absent spec = ผลเต็มตรง baseline');
...
await test('11) source guard: route preserve own-property · validator ก่อน core IO · strict ไม่มี template fallback · invariant หลัง Eye ก่อน manifest', async () => {
  ...
  // route: ส่งผ่านแบบ own-property เท่านั้น ห้ามสร้าง payload เอง
  assert.ok(routeSrc.includes("hasOwnProperty.call(body, 'selectionSpec')"), 'route ต้องเช็ค own-property selectionSpec');
  assert.ok(routeSrc.includes("hasOwnProperty.call(body, 'realizedTemplate')"), 'route ต้องเช็ค own-property realizedTemplate');
  assert.ok(routeSrc.includes('payload.selectionSpec = body.selectionSpec'), 'pass-through ตรงๆ ไม่แปลงร่าง');
  // service: validator (ใน _strictPrepare) ต้องถูกเรียกก่อน composeCore เสมอ
  const iCompose = svcSrc.indexOf('export async function composeAndVerify');
  const iGate = svcSrc.indexOf('_strictPrepare({ carrier: args, slotPlan })', iCompose);
  const iCore = svcSrc.indexOf('composeCore({ slotPlan, refDNA, stableOrder, strictCtx })', iCompose);
  assert.ok(iCompose > 0 && iGate > iCompose && iCore > iGate, 'ลำดับ: composeAndVerify → strict gate → composeCore');
  ...
  // ★ รอบ 2 (P0-1): validator ต้องรับ carrier ต้นฉบับ — ห้ามประกอบ {selectionSpec,...} ใหม่ก่อนส่ง
  assert.ok(svcSrc.includes('validateStrictRenderActivation(carrier)'), 'validator รับ carrier ตรง');
  assert.ok(!svcSrc.includes('validateStrictRenderActivation({ selectionSpec'), 'ห้าม reconstruct carrier');
  // ★ รอบ 2 (โครง): strict dispatch ต้องมาก่อน legacy guard ทุกตัวใน composeCore
  ...
  // ★ รอบ 2 (P0-3): postcondition ยืนกั้น "ทันทีก่อน" success return ของ composeAndVerify
...
await test('22) (P1-A) SUCCESSFUL legacy OFF parity: เส้นสำเร็จเต็ม (โหลด→FD→template→assign→render→manifest) byte-identical + snapshot เป๊ะ', async () => {
  ...
  // ★ FINAL AUDIT: ห้าม JSON clone ใน proof — JSON strip undefined + normalize -0/NaN/Infinity
  //   ใช้ structuredClone (แม่นตาม structured clone algorithm) แล้วเทียบกับ object จริงตรงๆ
  const prevEnv = process.env.MEGA_STRICT_RENDER;
  delete process.env.MEGA_STRICT_RENDER;
  try {
    // ── baseline: legacy success เต็มเส้น (env unset · ไม่มี payload strict) ──
    const b0 = ioDelta();
    ...
    const baseResp = structuredClone(baseline);                       // capture complete response (exact clone)
    const baseSnap = structuredClone(globalThis.__EXEC_SNAP || []);   // capture complete executor snapshot (exact clone)
    assert.ok(baseSnap.length >= 3 && baseSnap.every((s) => s.crop && Number.isInteger(s.idx)), 'snapshot ต้องมี slot/idx/bytes/crop ครบ');

    // เทียบ "ทั้ง response และ snapshot" แบบ deepStrictEqual บน representation จริง — ไม่มี serialization/normalizer
    const compare = async (label, args) => {
      const d0 = ioDelta();
      ...
      assert.deepStrictEqual(globalThis.__EXEC_SNAP, baseSnap, `${label}: __EXEC_SNAP (slot/idx/bytes/crop) ต้องเท่ากันเป๊ะ — จับ assignment/crop drift ที่ response คงที่ซ่อนได้`);
    };
    // payload พิษ: ถ้า OFF แอบอ่านแม้ field เดียว (slots/canvas) ผลจะเพี้ยนทันที — ต้องถูกเมินทั้งก้อน
    const mkPoisonPayload = () => ({
      selectionSpec: { __offPoison: true, slots: 'INVALID_IF_READ' },
      realizedTemplate: { __offPoison: true, canvasW: -999, canvasH: -999 },
    });

    // A. env unset + own selectionSpec/realizedTemplate (fresh spec/realized ทุก call)
    await compare('A: unset+payload', { ...mkLegacySuccessArgs(), selectionSpec: mkSpec(), realizedTemplate: mkRealized() });
    await compare('A-poison: unset+payload พิษ', { ...mkLegacySuccessArgs(), ...mkPoisonPayload() });
    // B. MEGA_STRICT_RENDER='0' + payload strict เดียวกัน + payload พิษ
    process.env.MEGA_STRICT_RENDER = '0';
    try {
      await compare("B: '0'+payload", { ...mkLegacySuccessArgs(), selectionSpec: mkSpec(), realizedTemplate: mkRealized() });
      await compare("B-poison: '0'+payload พิษ", { ...mkLegacySuccessArgs(), ...mkPoisonPayload() });
    } finally { delete process.env.MEGA_STRICT_RENDER; }
    // C. MEGA_STRICT_RENDER='1' + args ที่ "ไม่มี" own selectionSpec และไม่มี own realizedTemplate = legacy job จริง
    process.env.MEGA_STRICT_RENDER = '1';
    try {
      await compare("C: '1'+absent", mkLegacySuccessArgs());
    } finally { delete process.env.MEGA_STRICT_RENDER; }
  } finally {
