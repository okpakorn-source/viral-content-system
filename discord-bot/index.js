    ...
    while (Date.now() - pollStartTime < maxPollTime) {
      ...
      try {
        ...
        if (st.status === 'pending' && st.status !== lastStatus) {
          ...
          // แสดง pipeline steps ตามเวลาที่ผ่านไป
          const elapsed = Math.round((Date.now() - pollStartTime) / 1000);
          const steps = [
            { at: 0,  icon: '✅', label: 'ถึงคิวของคุณแล้ว' },
            { at: 5,  icon: '✅', label: 'ตรวจจับแหล่งข้อมูล' },
            { at: 10, icon: '✅', label: 'ดึงเนื้อหา' },
            { at: 20, icon: '✅', label: 'สกัดเนื้อข่าว (AI)' },
            { at: 35, icon: '✅', label: 'แตกประเด็น (AI)' },
            { at: 50, icon: '✅', label: 'วาง Emotional Blueprint' },
            { at: 65, icon: '✅', label: 'ค้นหาข้อมูล (Google)' },
            { at: 90, icon: '✅', label: 'Multi-Angle Generate' },
            { at: 120, icon: '✅', label: 'Enhanced Generate' },
          ];

          const stepLines = steps.map(s => {
            if (elapsed >= s.at + 10) return `${s.icon} ${s.label}`;
            if (elapsed >= s.at) return `⏳ ${s.label}...`;
            return `⬜ ${s.label}`;
          }).join('\n');

          const progressMsg = `⚡ กำลังประมวลผลข่าวของคุณ... (${elapsed}s)\n\n**ขั้นตอน:**\n${stepLines}`;

          // อัพเดททุก 5 วินาที เพื่อไม่ spam Discord API
          if (elapsed % 5 === 0 || st.status !== lastStatus) {
