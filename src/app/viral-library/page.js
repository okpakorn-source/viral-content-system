...
export default function ViralLibraryPage() {
  ...
  const [batchProgress, setBatchProgress] = useState({ current: 0, total: 0, success: 0, failed: 0, running: false });
  ...
  // === AI สร้าง Prompt (รองรับ regenerate + ป้องกัน duplicate) ===
  const handleGeneratePrompt = async (item, regenerate = false) => {
    if (!item.analysis) {
    ...
    setProcessing(item.id);
    setMsg(regenerate ? '🔄 AI กำลังสร้าง Prompt ใหม่...' : '⚡ AI กำลังสร้าง Prompt เฉพาะทาง...');
    try {
      const res = await fetch('/api/viral-analyze', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (data.success && data.promptData) {
        ...
        // Check for existing prompt from this content (ป้องกัน duplicate)
        const existingRes = await fetch('/api/prompt-library');
        const existingPrompts = existingRes.ok ? await existingRes.json() : [];
        const existingPrompt = Array.isArray(existingPrompts)
          ? existingPrompts.find(p => p.sourceContentId === item.id)
          : null;
        ...
        const promptPayload = {
          ...
          narrativeArchetype: data.promptData.narrative_archetype || data.promptData.narrativeArchetype || '',
          targetCategories: data.promptData.target_categories || data.promptData.targetCategories || [],
        };

        if (existingPrompt) {
          // Update existing prompt (don't create duplicate)
          await fetch('/api/prompt-library', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id: existingPrompt.id, ...promptPayload }),
          });
          console.log(`[Prompt] Updated existing: ${existingPrompt.id}`);
        } else {
          await fetch('/api/prompt-library', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          ...
          console.log(`[Prompt] Created new for content: ${item.id}`);
        }

        setMsg(`✅ ${regenerate ? 'สร้าง Prompt ใหม่' : 'สร้าง Prompt'} สำเร็จ: ${data.promptData.prompt_name || 'สำเร็จ'}`);
        await new Promise(r => setTimeout(r, 300));
        await loadItems();
      } else {
  ...
  const handleBatchGenerate = async () => {
    ...
    if (batchFilter === 'no-prompt') {
      itemsToProcess = items.filter(i => i.status !== 'prompted' || !i.generatedPrompt);
    } else if (batchFilter === 'has-prompt') {
      itemsToProcess = items.filter(i => i.status === 'prompted' && i.generatedPrompt);
    }
    ...
    setBatchProgress({ current: 0, total: itemsToProcess.length, success: 0, failed: 0, running: true });
    setBatchLog([`🚀 เริ่ม Batch: ${itemsToProcess.length} รายการ (${batchFilter})`]);

    for (let i = 0; i < itemsToProcess.length; i++) {
      const item = itemsToProcess[i];

      setBatchLog(prev => [...prev, `⏳ [${i+1}/${itemsToProcess.length}] ${item.title || item.content?.slice(0,40) || item.id}`]);

      try {
        // Step 1: Analyze if needed
        let analysis = item.analysis;
        if (!analysis) {
          const analyzeRes = await fetch('/api/viral-analyze', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ text: item.content, mode: 'viral-analyze' }),
          });
          ...
          await fetch('/api/viral-library', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id: item.id, analysis }),
          });
        }

        // Step 2: Generate prompt
        const genRes = await fetch('/api/viral-analyze', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ text: item.content, mode: 'generate-prompt', analysis_input: analysis }),
        });
        const genData = await genRes.json();
        if (!genData.success) throw new Error(genData.error || 'Prompt generation failed');

        // Step 3: Save to viral library
        await fetch('/api/viral-library', {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ id: item.id, generatedPrompt: genData.promptData }),
        });

        // Step 4: Save to prompt library (with duplicate check)
        const existingRes = await fetch('/api/prompt-library');
        const existingPrompts = existingRes.ok ? await existingRes.json() : [];
        const existing = Array.isArray(existingPrompts) ? existingPrompts.find(p => p.sourceContentId === item.id) : null;

        const promptPayload = {
          ...genData.promptData,
          sourceContentId: item.id,
          exampleContent: item.content?.slice(0, 500),
          narrativeArchetype: genData.promptData.narrative_archetype || '',
          targetCategories: genData.promptData.target_categories || [],
        };
        ...
        if (existing) {
          await fetch('/api/prompt-library', {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ id: existing.id, ...promptPayload }),
          });
        } else {
          await fetch('/api/prompt-library', {
            method: 'POST',
        ...
        setBatchProgress(p => ({ ...p, current: i + 1, success: p.success + 1 }));
        setBatchLog(prev => [...prev, `✅ [${i+1}] ${genData.promptData?.prompt_name || 'OK'}`]);

      } catch (err) {
        setBatchProgress(p => ({ ...p, current: i + 1, failed: p.failed + 1 }));
        setBatchLog(prev => [...prev, `❌ [${i+1}] ${err.message?.slice(0, 60)}`]);
      }

      // Small delay between items to avoid API overload
      if (i < itemsToProcess.length - 1) await new Promise(r => setTimeout(r, 1000));
    }

    setBatchProgress(p => ({ ...p, running: false }));
    setBatchLog(prev => [...prev, `🏁 Batch เสร็จสิ้น!`]);
  ...
  return (
    <>
      <Header title="📚 หอสมุดไวรัล" subtitle="ป้อนข่าวปังที่คัดมา → ถอด DNA วิธีเขียน → สร้าง Prompt เข้าคลัง · อัปเดต 25 ก.ค. 69" />
      <div className="page-content">
        ...
        <div className="card" style={{ marginBottom: 16, border: '1px solid rgba(34,197,94,0.3)', background: 'linear-gradient(135deg, rgba(34,197,94,0.06), rgba(6,182,212,0.06))' }}>
          <div style={{ fontSize: 13, fontWeight: 800, color: '#22c55e', marginBottom: 8 }}>
            🧬 หอสมุดไวรัล — สถานะปัจจุบัน (อัปเดต 25 ก.ค. 69)
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 8, fontSize: 11, color: 'var(--text-muted)', lineHeight: 1.7 }}>
            <div>🔬 <b style={{ color: 'var(--text-primary)' }}>ถอด DNA 12 มิติ</b> — เรียนวิธีเขียนที่ทำให้คนอ่านจบ: วิธีเปิดเรื่อง จังหวะ ภาษา ภาพจำ โครงเรื่อง</div>
            <div>🛡️ <b style={{ color: 'var(--text-primary)' }}>ด่านคัดแบบยืดหยุ่น</b> — บล็อกจริงเฉพาะเนื้อหาโจมตี/ประจาน · ติดธงโทนอื่นแล้วยังรับเข้าได้ (ไม่ได้ปัดตก)</div>
            <div>🤖 <b style={{ color: 'var(--text-primary)' }}>โมเดล</b> — ด่านคัด + วิเคราะห์ DNA + สร้างพร้อมท์ ใช้ gpt-5.6-terra (เปลี่ยนจาก gpt-5.5 เมื่อ 25 ก.ค.)</div>
            <div>🔁 <b style={{ color: 'var(--text-primary)' }}>ส่งเข้าคลังพร้อมท์อัตโนมัติ</b> — ระบบทำข่าวหยิบไปใช้เขียนจริงทันที</div>
            <div>⚠️ <b style={{ color: 'var(--text-primary)' }}>ค้างอยู่</b> — ยังไม่มีด่านเช็กว่าใบใหม่ซ้ำแนวใบเดิมไหม · หมวดยังกระจุก 2 หมวด (60%) → อยู่ในแผนยกเครื่อง</div>
          </div>
        </div>
        ...
        <div className="card" style={{ marginBottom: 16, padding: 14 }}>
          <button onClick={() => setBatchMode(!batchMode)} style={{
            ...
            background: 'linear-gradient(135deg, #a3e635, #16a34a)', color: '#000',
          ...
          {batchMode && (
            <div style={{ marginTop: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
                {['all', 'no-prompt', 'has-prompt'].map(f => (
                  <button key={f} onClick={() => setBatchFilter(f)} style={{
                    padding: '8px 16px', borderRadius: 8,
                    background: batchFilter === f ? '#a3e635' : 'var(--bg-primary)',
                    color: batchFilter === f ? '#000' : 'var(--text-muted)',
                    border: '1px solid var(--border)', cursor: 'pointer', fontSize: 12, fontWeight: 700,
                    fontFamily: 'inherit',
                  }}>
                    {f === 'all' ? '🔄 ทั้งหมด' : f === 'no-prompt' ? '➕ ยังไม่มี Prompt' : '🔁 มี Prompt แล้ว (สร้างใหม่)'}
                  </button>
                ))}
              </div>
              ...
              {!batchProgress.running ? (
                <button onClick={handleBatchGenerate} style={{
                  ...
                  🚀 เริ่ม Batch สร้าง Prompt ({batchFilter === 'all' ? 'ทั้งหมด' : batchFilter === 'no-prompt' ? 'ยังไม่มี' : 'สร้างใหม่'})
                </button>
              ) : (
                <button onClick={() => setBatchProgress(p => ({...p, running: false}))} style={{
                  padding: '12px 24px', borderRadius: 10, border: '1px solid #ef4444',
                  background: 'rgba(239,68,68,0.1)', color: '#ef4444', fontWeight: 800, fontSize: 14,
              ...
              {batchProgress.total > 0 && (
                <div style={{ marginTop: 12 }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 6, display: 'flex', gap: 16 }}>
                    ...
                    <span>✅ {batchProgress.success} สำเร็จ</span>
                    <span>❌ {batchProgress.failed} ล้มเหลว</span>
                  </div>
                </div>
              )}
              ...
              {/* Batch log */}
              {batchLog.length > 0 && (
                <div style={{ marginTop: 8, maxHeight: 150, overflowY: 'auto', fontSize: 10, color: 'var(--text-muted)', background: 'rgba(0,0,0,0.2)', borderRadius: 8, padding: 8 }}>
                  {batchLog.slice(-20).map((log, i) => <div key={i}>{log}</div>)}
                </div>
              )}
            </div>
          )}
                    ...
                    <div style={{ display: 'flex', gap: 4 }} onClick={e => e.stopPropagation()}>
                      ...
                      {item.status === 'prompted' && (
                        <>
                          ...
                          <button onClick={() => handleGeneratePrompt(item, true)} disabled={isProcessing}
                            style={{
                              padding: '5px 10px', borderRadius: 8, border: 'none', fontSize: 10, fontWeight: 700,
                  ...
                  {isExpanded && (
                    <div style={{ padding: 14 }}>
                      ...
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                        ...
                        {item.status === 'prompted' && item.generatedPrompt && (
                          <button onClick={() => handleGeneratePrompt(item, true)} disabled={isProcessing}
                            style={{ padding: '7px 14px', borderRadius: 8, border: 'none', background: 'linear-gradient(135deg, rgba(6,182,212,0.15), rgba(59,130,246,0.15))', color: '#06b6d4', fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' }}>
                            🔄 สร้าง Prompt ใหม่
                          </button>
                        )}
