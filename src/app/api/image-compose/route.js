...
export async function POST(request) {
  ...
  try {
    ...
    if (useFAL) {
      try {
        rlog.step('fal-phase2', '⚡ FAL opt-in: calling imageEnhanceService...');
        const enhData = await enhanceImage({ layoutBase64: layoutB64, newsTitle: newsTitle || '' });
        if (enhData.success) {
    ...
    if (generateText && newsTitle && process.env.IDEOGRAM_API_KEY) {
      try {
        rlog.step('ideogram-phase3', 'adding text overlay via Ideogram /remix service');
        const textData = await addTextOverlay({
