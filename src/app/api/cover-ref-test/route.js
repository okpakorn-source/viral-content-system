...
// ------------------------------------------------------------
// LANE-A (AC-0099): ท่อนี้ "ไม่สร้าง/ไม่ประกอบใหม่" อะไรหลัง S6 อีกต่อไป —
//   หลัง S6 เรียก **โปรดิวเซอร์จริง** s7_cover() ผ่าน in-process seam แล้ว "บริโภค payload ที่ S7 ส่งจริง"
//   (sourceLinks / SLOT_ORDER / slotPlan / realizedTemplate / SelectionSpec / hashes ทั้งหมด = ของ S7 เท่านั้น
//    ท่อนี้ห้าม rebuild แม้ byte เดียว)
//
// สายเดิม: compassBrain → s5_case → s5_keywords → s5_search → s5_triage → s5_clipframe → s6_slots
//          (ยิง endpoint จริงตามเดิม — พูลภาพมาจากระบบ keyword จริง)
// สาย S7:  s7_cover(job, { origin, _deps:{ fetchJson, queueTransport:'cover_ref_test_in_process' } })
...
const _CAPTURE_HOST = 'http://127.0.0.2:3900';
...
export async function runCoverRefTest(input = {}, deps = {}) {
  ...
  const {
    ...
    composeAndVerify: _compose = composeAndVerify,
