  ...
  try {
    ...
    if (mode === 'analyze') {
      ...
      try {
        ...
        let promptLib = [];
        try { promptLib = await promptStore.getAll(); } catch (e) { console.warn('[Analyze] Supabase prompt load:', e.message); }

        // ✅ FIX: Dual-source — Supabase ว่าง → โหลดจาก local file
        if (promptLib.length === 0) {
          try {
            ...
            if (Array.isArray(_localData) && _localData.length > 0) {
              ...
              console.log('[Analyze] ✅ FALLBACK: Loaded ' + promptLib.length + ' prompts from LOCAL FILE (Supabase empty)');
            }
          } catch (fileErr) {
            console.warn('[Analyze] Local file fallback failed:', fileErr.message);
          }
        } else {
          console.log('[Analyze] ✅ Supabase: ' + promptLib.length + ' prompts loaded');
        }
        console.log('[Analyze] 📋 validPrompts: ' + promptLib.filter(p => p.promptText).length + '/' + promptLib.length);
...
{"newsType":"ประเภทข่าว","selectedIndex":เลขindex,"matchType":"exact หรือ closest","reason":"เหตุผลการเลือก"}`
          ...
          // ✅ FIX: Library ว่าง → ใช้ built-in fallback prompt (ห้าม return error)
          promptMatchReason = 'PROMPT_LIBRARY_MISSING — ใช้ built-in fallback';
          console.warn('[Analyze] ⚠️ PROMPT_LIBRARY_MISSING — activating built-in fallback prompt');
          smartPrompt = {
            id: 'fallback_builtin', promptName: 'Built-in Fallback',
            category: 'ทั่วไป', emotionalType: 'สาระน่าสนใจ', viralScore: 70,
            promptText: 'เขียนเนื้อหาข่าวสำหรับ Facebook ที่ดึงดูดความสนใจ ใช้ภาษาเป็นกันเอง เล่าเรื่องราวอย่างน่าสนใจ มีการเปิดด้วย hook ที่ดึงความสนใจ เล่ารายละเอียดสำคัญ และปิดด้วยประโยคที่ทิ้งความประทับใจ ห้ามใช้ภาษาราชการ ห้ามตั้งคำถามปิดท้าย',
            _isFallback: true,
          };
        ...
        console.warn('[Analyze] Smart Match error:', err.message);
      }

      // ✅ FIX: ยังไม่มี prompt → ใช้ built-in fallback (ห้ามพัง)
      if (!smartPrompt) {
        console.warn('[Analyze] ⚠️ No prompt matched — using built-in fallback. Reason:', promptMatchReason);
        smartPrompt = {
          id: 'fallback_builtin', promptName: 'Built-in Fallback (Auto)',
      ...
      if (smartPrompt && smartPrompt.promptText) {
        ...
        prompt = '=== 🏛️ คำสั่งเขียนจากหอสมุดไวรัล ===\n' +
          ...
          '1. ใช้สไตล์การเขียนจากคำสั่งด้านบน แต่ต้องเขียนจาก Narrative Payload (facts/quotes/research) ด้านล่างเท่านั้น\n' +
          '2. ห้ามแต่งเรื่อง ห้ามเพิ่มข้อมูลที่ไม่มีใน payload ห้ามสรุปผิด\n' +
          '3. ชื่อคน สถานที่ ตัวเลข วันที่ ต้องตรงกับ facts ที่ระบุ 100%\n' +
          '4. ห้ามดึงข้อมูลจากแหล่งอื่น ใช้เฉพาะ Narrative Payload\n\n' +
          '=== จบคำสั่งหอสมุด ===\n\n';

        // === NARRATIVE RECONSTRUCTION: Build structured payload (NO raw source) ===
        narrativePayload = buildNarrativePayload(actualNewsTitle, actualBreakdown, researchData, emotionalBlueprint);
        prompt += formatNarrativePayload(narrativePayload);
        console.log(`[Analyze] 🔄 NARRATIVE PAYLOAD built: sourceRemoved=true | facts=${narrativePayload.coreFacts.length} | research=${narrativePayload.researchContexts.length} | quotes=${narrativePayload.quoteFragments.length} | researchGrade=${narrativePayload.researchGrade} | factSufficiency=${narrativePayload.factSufficiency}`);

        // === Breakdown now inside NarrativePayload (no separate injection) ===
        console.log(`[Analyze] ✅ Breakdown merged into NarrativePayload: ${actualBreakdown?.key_points?.length || 0} points, ${actualBreakdown?.possible_angles?.length || 0} angles`);
      ...
      // === NARRATIVE RECONSTRUCTION: No raw source injection ===
      // Research + Blueprint + Breakdown are all inside NarrativePayload already
      // For non-library path: also use NarrativePayload (no raw source)
      if (promptSource !== 'library') {
        if (!prompt) prompt = '';
        narrativePayload = buildNarrativePayload(actualNewsTitle, actualBreakdown, researchData, emotionalBlueprint);
        prompt += formatNarrativePayload(narrativePayload);
        console.log(`[Analyze] 🔄 Non-library path: NarrativePayload injected (facts=${narrativePayload.coreFacts.length})`);
      }

      // === Research Grade Tagging ===
      const _researchGrade = (researchData?.items?.length || 0) >= 3 ? 'strong'
        : (researchData?.items?.length || 0) >= 1 ? 'partial' : 'missing';
      if (_researchGrade === 'missing') {
        console.warn('[Analyze] ⚠️ Research: MISSING — output quality may be lower');
      } else {
        console.log(`[Analyze] ✅ Research grade: ${_researchGrade} (${researchData?.items?.length || 0} items — embedded in NarrativePayload)`);
      }
...
`;
      ...
      // === Blueprint now inside NarrativePayload (no separate injection) ===
      if (emotionalBlueprint?.core_emotion) {
        console.log('[Analyze] 🧬 Blueprint embedded in NarrativePayload: emotion=' + emotionalBlueprint.core_emotion);
      }
      ...
      try {
        ...
        // Debug info — Narrative Reconstruction metadata
        const debugInfo = {
          ...
          aiError,
          aiWarning,
          // === Narrative Reconstruction Metadata ===
        ...
        if (result && typeof result === 'object') {
          ...
          // === NARRATIVE SIMILARITY CHECK ===
          const firstContent = versions[0]?.content || '';
          const similarity = checkNarrativeSimilarity(actualNewsBody || '', firstContent);
          console.log(`[Analyze] 📊 Similarity: score=${similarity.score} grade=${similarity.grade} pass=${similarity.pass} | nGram=${similarity.details.nGramOverlap} longMatch=${similarity.details.longMatchCount}`);
          if (!similarity.pass) {
            console.warn(`[Analyze] ⚠️ SIMILARITY TOO HIGH (${similarity.score}) — content may still resemble source`);
          }
