...
ตอบเป็น JSON (ชื่อ field คงเดิมเพื่อระบบเดิม แต่ความหมายตามมิติ v3 ข้างบน):
{
  ...
  "key_takeaways": ["บทเรียนวิธีเขียนดี1", "บทเรียน2", "บทเรียน3", "บทเรียน4", "บทเรียน5"]
      ...
      // UI compat: การ์ดผลวิเคราะห์อ่าน key รุ่นแรก (อารมณ์/Hook/โครงสร้างเคยโชว์ว่างมาตลอด)
      if (!analysis.primary_emotion) analysis.primary_emotion = analysis.emotional_core?.primary_emotion || '';
      if (!analysis.hook_analysis) analysis.hook_analysis = { hook_technique: analysis.stop_scrolling_hook?.hook_technique || analysis.stop_scrolling_hook?.hook_type || '' };
      if (!analysis.structure) analysis.structure = { flow: analysis.story_structure?.full_flow || '' };
...
- ใช้ของจริงจาก DNA ให้คุ้ม: หยิบสำนวนธรรมชาติ (natural_words) วลีอวยมีลูกเล่น (viral_words) สูตรจังหวะ และเทคนิคเปิดเรื่อง ที่วิเคราะห์ได้ มาใส่ใน prompt_text เป็น "แนวทาง+ตัวอย่าง pattern" ให้นักเขียนเห็นภาพ — ห้ามเขียนคำสั่งกว้างๆ ลอยๆ ที่ใช้กับข่าวไหนก็ได้
...
- viral_score ให้ใช้คะแนน overall จากผลวิเคราะห์ (ไม่ใช่ตัวเลขสุ่ม)
...
ตอบเป็น JSON:
{
  ...
  "conflict_tags": ["เลือก 1-3 ความท้าทายเชิงข้อเท็จจริงที่คนในเรื่องเจอ (ไม่ใช่เชื้อดราม่า) จาก: การต่อสู้, การสูญเสีย, ความผิดพลาด, ความขัดแย้ง, การตัดสิน, ความเหลื่อมล้ำ, ความอยุติธรรม, การเอาเปรียบ"],
  "narrative_archetype": "เลือก 1 ที่ตรงเรื่องที่สุด จาก: สู้ชีวิต, ฮีโร่ชาวบ้าน, น้ำใจคนไทย, ชีวิตพลิกผัน, ดราม่าครอบครัว, คนดีที่โลกลืม, ความรักข้ามขีดจำกัด (ลิสต์เดียวกับระบบจับคู่ข่าว — ห้ามสร้างคำใหม่นอกลิสต์)",
