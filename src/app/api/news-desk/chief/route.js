...
export async function POST(request) {
  ...
  try {
    ...
    const topUnclaimed = fresh.map(i => ({ ...i, _ds: decayScore(i) }))
      ...
      .map(i => {
        const pushedBefore = prevPushed.some(p => p && (String(i.title).includes(p.slice(0, 18)) || p.includes(String(i.title).slice(0, 18))));
...
- pushNow เลือกได้เฉพาะจากรายการ "ตัวท็อปที่ยังไม่มีใครหยิบ" เท่านั้น และห้ามเลือกตัวที่ติดป้าย "สั่งดันรอบก่อนแล้ว" — ถ้าตัวท็อปติดป้ายหมด ให้ใส่ orders ว่า "ข่าวที่สั่งดันรอบก่อนยังไม่มีใครหยิบ: <ชื่อ>" แทนการดันซ้ำ
