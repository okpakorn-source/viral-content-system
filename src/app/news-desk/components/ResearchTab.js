...
const HUNT = '/api/desk/research/hunt';
...
export default function ResearchTab({ onToast }) {
  ...
  // 🆕 เฟส 0 (Discovery V2): อ่าน config สาธารณะ (masterOn) ตอนเปิดแท็บ — fail-safe = ปิด (ไม่แนบ shadow sample)
  ...
  async function startHunt() {
    ...
    // 🆕 เฟส 0 (Discovery V2, shadow): สะสมตัวอย่างหลัง judge (มี fingerprint/channel/kept) — แนบเข้า logRun เฉพาะเมื่อ MASTER เปิด
    const measurementSampleAgg = [];
    ...
    for (const clusterId of clusterKeys) {
      ...
      for (const j of judged) {
        ...
        // 🆕 เฟส 0 (shadow): เก็บตัวอย่างรอบนี้ (เฉพาะเมื่อ MASTER เปิด) — url/channel/fingerprint จาก judged, kept=ผ่านเกณฑ์เก็บ
        if (discoveryMasterOn) {
          measurementSampleAgg.push({ url: j.url, channel: j.channel, fingerprint: j.fingerprint, kept: isKeeper });
        }
      }
