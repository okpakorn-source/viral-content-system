...
const CAT_COLORS = {
  ...
  'กระแสรายวัน': '#f59e0b', 'คนดัง/ดราม่าบันเทิง': '#ec4899', 'ความรัก/แต่งงาน': '#f472b6', 'กีฬา': '#0ea5e9',
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary, #0f1419)' }}>
      ...
      <div style={{ maxWidth: 1180, margin: '0 auto', padding: '16px 20px 60px' }}>
        ...
        {/* ★ 21 มิ.ย.: คำอธิบายหมวดที่เลือก — บอกชัดว่ากดแล้วหาข่าวประเภทไหน */}
        {(() => { const f = FOCUS_OPTIONS.find(o => o.key === focusSel); return f?.desc ? (
          <div style={{ marginTop: -4, marginBottom: 12, padding: '9px 13px', borderRadius: 9, background: 'rgba(99,102,241,0.08)', border: '1px solid rgba(99,102,241,0.22)', fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.6 }}>
            🔎 <b style={{ color: '#818cf8' }}>{f.label}</b> → {f.desc}
          </div>
        ) : null; })()}
                ...
                <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: 6, fontSize: 12 }}>
                      ...
                      {/* ★ 4 ก.ค.: ยอดวิวจริงของคลิป (YouTube Data API) — คลิปยิ่งวิวเยอะยิ่งน่าทำ */}
                      {it.views > 0 && <span title="ยอดวิวจริงบน YouTube" style={{ padding: '2px 9px', borderRadius: 999, background: it.views >= 100000 ? 'rgba(239,68,68,0.15)' : 'rgba(148,163,184,0.13)', color: it.views >= 100000 ? '#ef4444' : '#94a3b8', fontWeight: 800 }}>👁 {it.views >= 1e6 ? (it.views / 1e6).toFixed(1) + 'M' : it.views >= 1e3 ? Math.round(it.views / 1e3) + 'K' : it.views} วิว</span>}
                      {/* ★ 4 ก.ค.: ใบที่ บก.ใหญ่ยังไม่ตัดสิน โชว์คะแนนเบื้องต้น (classify ประเมินทุกใบแล้ว) — เดิมไม่มีคะแนนโชว์เลย */}
                      {it.judgeScore === undefined && it.prelimScore !== undefined && (
                        <span title="คะแนนเบื้องต้นจากสมองคัดรอบแรก (บก.ใหญ่จะตัดสินซ้ำเฉพาะตัวท็อป)" style={{ padding: '2px 9px', borderRadius: 999, background: it.prelimScore >= 7 ? 'rgba(34,197,94,0.13)' : 'rgba(148,163,184,0.13)', color: it.prelimScore >= 7 ? '#22c55e' : '#94a3b8', fontWeight: 700 }}>🧠 เบื้องต้น {it.prelimScore}</span>
                      )}
