...
function NewContentPageInner() {
  ...
  const handleUniversalSubmit = async (inputText, inputImages) => {
    ...
    // เริ่มด้วย 4 steps พื้นฐาน — จะ upgrade เป็น 8 steps ถ้า enhanced pipeline
    ...
    try {
      ...
      if (!data.success) {
        ...
        const errorWithStep = new Error(errMsg);
        errorWithStep.failedStep = data.failedStep || 'u_extract';
        throw errorWithStep;
      }

      // ─── Detect ว่าเป็น Enhanced Pipeline หรือ Local Pipeline ───
      const isEnhanced = !!(data.debug?.delegatedTo || data.data?.stepTimings);
      const delay = (ms) => new Promise(r => setTimeout(r, ms));

      if (isEnhanced && data.data?.stepTimings) {
        // === Enhanced Pipeline: replay 8 steps เหมือน handleAutoMode ===
        const st = data.data.stepTimings || {};
        const pi = data.data.usedPromptInfo;
        ...
        const pipelineLabel = data.detection?.pipelineLabel || 'Text Pipeline';

        // Upgrade workflow steps → 8 steps เหมือน URL pipeline
        startWorkflow(`Enhanced Pipeline — ${pipelineLabel}`, [
          { id: 'auto_detect', label: `${pipelineIcon} ตรวจจับแหล่งข้อมูล` },
          { id: 'auto_scrape', label: '📡 ดึงเนื้อหา' },
          { id: 'auto_extract', label: '📰 สกัดเนื้อข่าว (AI)' },
          { id: 'auto_breakdown', label: '🔍 แตกประเด็น (AI)' },
          { id: 'auto_blueprint', label: '🧬 วาง Emotional Blueprint' },
          ...
          { id: 'auto_enhanced', label: '🧬 Enhanced Generate' },
        ], { type: pipelineLabel, label: inputLabel });

        wfStart('auto_detect', { detail: `ตรวจจับ: ${pipelineLabel}` });
        await delay(150);
        wfComplete('auto_detect', `${pipelineIcon} ${pipelineLabel} (${Math.round((data.detection?.confidence || 0.9) * 100)}%)`);

        wfStart('auto_scrape', { detail: 'ดึงเนื้อหา...' });
        await delay(200);
        wfComplete('auto_scrape', `ดึง ${data.data?.newsData?.newsBody?.length || data.newsData?.newsBody?.length || 0} ตัวอักษร | ${st.scrape || '?'}s`);

        wfStart('auto_extract', { api: 'GPT-4o-mini', detail: `EXTRACT prompt → "${newsTitle.slice(0,30)}..."` });
        await delay(250);
        wfComplete('auto_extract', `"${newsTitle.slice(0, 40)}" | ${st.extract || '?'}s`);

        wfStart('auto_breakdown', { api: 'GPT-4o-mini', detail: 'BREAKDOWN prompt → วิเคราะห์มุมข่าว' });
        await delay(250);
        wfComplete('auto_breakdown', `${anglesCount} มุมข่าว | ${st.breakdown || '?'}s`);

        wfStart('auto_blueprint', { api: 'GPT-4o', detail: 'BLUEPRINT prompt → Emotional Architecture' });
        await delay(250);
        ...
        wfStart('auto_research', { api: 'Serper API', detail: 'ค้นหาข้อมูลจริงจาก Google...' });
        await delay(250);
        wfComplete('auto_research', resCount > 0
          ? `✅ ${resCount} แหล่งข้อมูล | ${st.research || '?'}s`
          : `⚠️ ไม่พบข้อมูลเพิ่มเติม`);

        wfStart('auto_classic', { api: `Claude ×${classicCount}`, detail: 'Multi-Angle prompts from Library' });
        await delay(250);
        wfComplete('auto_classic', `✅ ${classicCount} เวอร์ชัน | ${st.classic || '?'}s`);

        wfStart('auto_enhanced', { api: `Claude ×${enhancedCount}`, detail: 'Enhanced + Blueprint inject' });
        await delay(250);
        wfComplete('auto_enhanced', `✅ ${enhancedCount} เวอร์ชัน | ${st.enhanced || '?'}s`);
        ...
        // === Local Pipeline: แสดง steps ตามจริง ===
        const pipelineIcon = data.detection?.pipelineIcon || '⚙️';
        const pipelineLabel = data.detection?.pipelineLabel || 'Pipeline';
        const versionCount = data.analysisResult?.totalVersions || data.analysisResult?.versions?.length || 0;

        wfComplete('u_detect', `${pipelineIcon} ${pipelineLabel} (${Math.round((data.detection?.confidence || 0) * 100)}%)`);
        ...
        wfStart('u_generate', {}); wfComplete('u_generate', `✅ ${versionCount} versions`);
        finishWorkflow(`✅ ${data.debug?.durationSeconds}s | ${pipelineIcon} ${pipelineLabel}`);
      }
