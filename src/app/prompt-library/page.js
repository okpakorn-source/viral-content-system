                ...
                <div key={p.id} className="card" style={{ padding: 0, overflow: 'hidden' }}>
                  ...
                  {isExpanded && (
                    <div style={{ padding: 14 }}>
                      {/* Structure & Style Info */}
                      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 8, marginBottom: 12 }}>
                        {p.structure && (
                          <div style={{ padding: 8, background: 'rgba(59,130,246,0.06)', borderRadius: 6 }}>
                            <div style={{ fontSize: 9, fontWeight: 700, color: '#3b82f6', marginBottom: 4 }}>📐 โครงสร้าง</div>
                            <div style={{ fontSize: 11, color: 'var(--text-primary)' }}>{p.structure}</div>
                          </div>
                        )}
                        {p.ctaStyle && (
