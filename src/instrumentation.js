...
export async function register() {
  ...
  const { installCrashGuard, recoverOrphanJobsOnBoot, startOrphanRescan } = await import('./instrumentation-node.js');
  installCrashGuard();
  ...
  // ★ 29 ก.ค. 69 คืน (Opus fix #4 — บูตเร็วหลังตาย 5วิ ทำให้สแกนบูตพลาดงานที่เพิ่งกำพร้า <90วิ): ตั้งสแกนซ้ำเป็น
  //   รอบ (ไม่ await — ตั้ง interval แล้วปล่อยผ่านทันที ไม่บล็อกการบูต) มีตาข่ายกันซ้อน/kill-switch ของตัวเองแล้ว
  startOrphanRescan();
}
