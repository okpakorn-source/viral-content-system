...
export async function artBriefBrain({ refDNA, compass, deskTitle, typeMatched = false, mode = 'legacy', storyRulesOn = true, pagePlaybookOn = true, _callBrain = callBrain }) {
...
export async function slotDirectorBrain({ imagesMeta, compass, deskTitle, refDNA = null, artBrief = null, sceneInventory = '', slotContract = null, storyRulesOn = true, pagePlaybookOn = true, imgMetaBudget = 45000, _deps = {} }) {
...
(8) ★ปกทั้งใบต้องเล่าเรื่องครบ: ทุกช่องรวมกันต้องเห็น "คน → กำลังทำอะไร → หลักฐาน/สถานที่" — ห้ามเป็นพอร์ตเทรตล้วนทุกช่อง (9) ★ฉากห้ามซ้ำข้ามช่อง (เทียบจาก note — เฟรมคลิปเดียวกัน/เวทีเดิม = ฉากเดียวกัน) (10) ★${_heroInst.id} เลือกภาพ "หน้าเดี่ยว" (faces=1) หน้าใหญ่คมชัด แนวตั้ง/จัตุรัส (orient=tall/sq) ก่อนเสมอ — ห้ามภาพแนวนอนกว้าง/แบนเนอร์ถ้ามีตัวเลือกอื่นของตัวเอก${_circleIds.length ? ` (11) ★ช่องวงกลม (${_circleIds.join('/')}) ควรเป็นคนละคนกับ ${_heroInst.id} เมื่อช่องนั้นไม่ได้ระบุ "คน:" ไว้` : ''} (12) ★ภาพคนหันหลัง/ก้มกราบ/เห็นแต่แผ่นหลัง ใช้ได้เฉพาะช่องฉากกว้างเท่านั้น${_hasFaceH ? ` (13) ★faceH = สัดส่วนความสูงหน้าเทียบเฟรม (0-1) — ${_heroInst.id} เลือกภาพ faceH สูง (หน้าใหญ่เด่น) ก่อนเสมอ หลีกเลี่ยงภาพหน้าเล็กในช็อตกว้าง/มี backdrop เยอะ` : ''}${storyRulesOn ? ` (14) ★สูตรเล่าเรื่องตามเพจ (วิจัยปกยอด ≥2 หมื่นไลค์): ${_heroInst.id} เลือกใบหน้าที่มีอารมณ์ตรง "อารมณ์หลัก" ในเข็มทิศเสมอ (ร้องไห้/น้ำตาคลอ/ยิ้มกว้าง ตามโทนข่าว) ใหญ่เต็มช่อง — ห้ามภาพโพสนิ่ง/แฟชั่น/โปรโมทถ้ามีตัวเลือกอารมณ์จริงให้เลือก · ช่องย่อยอื่น (ไม่ใช่ ${_heroInst.id}${_circleIds.length ? `/${_circleIds.join('/')}` : ''}) ให้เรียงเป็นไทม์ไลน์เล่าเรื่อง: บริบท/ที่มา → โมเมนต์หัวใจ → ผลลัพธ์/จุดพลิก (อ่านจาก note/emotion แต่ละใบ)${_circleIds.length ? ` · ช่องวงกลม (${_circleIds.join('/')}) เลือกช็อตหน้าชัดโฟกัสเดี่ยวของบุคคลที่สอง (ไม่ใช่ภาพหมู่/ไกล)` : ''} · ภาพที่ note บ่งชี้ฉากรายการทีวี/สตูดิโอ/ฉากหลังจอ-ไฟเวที/ตัวอักษรใหญ่ฉากหลัง = เลี่ยงทุกช่องย่อย เว้นแต่ไม่มีตัวเลือกอื่นจริงๆ · ช่องย่อย (ไม่ใช่ ${_heroInst.id}) เลี่ยงภาพมืด/เบลอ/คุณภาพต่ำ (สังเกตจาก note เช่น "เบลอ/มืด/คุณภาพต่ำ" หรือค่า quality ต่ำ) เว้นแต่ไม่มีตัวเลือกอื่นในเรื่องเดียวกันจริงๆ · ทั้งปกควรมีช่องภาพมืด/กลางคืนไม่เกิน 1 ช่อง เมื่อมีหลายใบเข้าเรื่องพอๆ กันให้เลือกภาพสว่างชัดก่อนเสมอ (สังเกตจาก note)` : ''}
ตอบ JSON เท่านั้น:
${schemaSem}`;
  }
