...
// ★ ตรวจซ้ำ (28 ก.ค. 69 — คัมภีร์เพจฉบับเต็ม, กฎเหล็กเจ้าของ): string ล้วน ~10KB ห้ามแก้เนื้อหาข้างในไฟล์นั้นเลย
//   ต้องแทรก "ทั้งก้อน verbatim" เข้า system prompt ทุกจุด (ไม่ตัด/ย่อ/สรุป) — ดู pagePlaybookOn param ด้านล่าง
import { COVER_PAGE_PLAYBOOK } from '@/lib/coverPagePlaybook';
...
export async function artBriefBrain({ refDNA, compass, deskTitle, typeMatched = false, mode = 'legacy', storyRulesOn = true, pagePlaybookOn = true, rulesV3On = true, _callBrain = callBrain }) {
  // ★ ตรวจซ้ำ (28 ก.ค. 69 — คัมภีร์เพจฉบับเต็ม): แทรกทั้งก้อน COVER_PAGE_PLAYBOOK verbatim ก่อนบล็อกกติกาเดิม
  //   (คัมภีร์=หลักการกว้าง กติกาเดิม=ข้อบังคับเฉพาะที่ยังต้องเหนือกว่าเสมอ) — caller (megaAdapters.js) อ่าน env
  //   MEGA_PAGE_PLAYBOOK แล้วส่ง pagePlaybookOn มา (ไฟล์นี้ไม่อ่าน process.env เอง — แพทเทิร์นเดียวกับ storyRulesOn)
  //   ไม่ส่งพารามิเตอร์ = default true (ON) · pagePlaybookOn=false → prompt byte-identical กับก่อนแทรกคัมภีร์เป๊ะ
  const _pagePlaybookBlock = pagePlaybookOn ? `${COVER_PAGE_PLAYBOOK}\n\n` : '';
  // ★ TIER3 (สูตรเล่าเรื่องตามเพจ — memory igdara-cover-formula, เจ้าของอนุมัติ 27 ก.ค. 69): กติกา hero=อารมณ์พีค +
  //   ไทม์ไลน์ช่องรอง + circle=บุคคลที่สอง + เลี่ยงฉากรายการ/สตูดิโอ — caller (megaAdapters.js) อ่าน env
  //   MEGA_STORY_RULES แล้วส่ง storyRulesOn มา (ไฟล์นี้ไม่อ่าน process.env เอง) · ไม่ส่งพารามิเตอร์ = default true (ON)
  //   storyRulesOn=false → prompt byte-identical กับก่อน TIER3 (ไม่มีข้อความใหม่แม้ byte เดียว)
  // ★ MEGA_RULES_V3 (28 ก.ค. 69 — เจ้าของสั่ง: "กติกาเก่าที่เป็นตัวถ่วงกติกาใหม่ ให้ลบ/เขียนใหม่ได้ เพราะของใหม่
  //   พิสูจน์จากงานจริง 478 ใบ"): 2 จุดใน TIER3 ขัดกับคัมภีร์ v3 ตรงๆ — (4) hero=อารมณ์พีค/ร้องไห้/น้ำตาคลอ (v3 ก.3:
  //   "หน้านิ่งชนะ — ปกแสนไลค์ 18 ใบไม่มี hero ร้องไห้เลยสักใบ") · (6) circle=บุคคลที่สองเท่านั้น เลือกช็อตหน้าชัด
  //   โฟกัสเดี่ยว (v3 ง: circle ยังเป็นภาพเก่า/วัยเด็ก/โมเมนต์กอด-กราบ(มักมี 2 คน)/ขาวดำผู้ล่วงลับได้ ไม่ใช่แค่ "หน้าเดี่ยว")
  //   caller อ่าน env MEGA_RULES_V3 แล้วส่ง rulesV3On มา (ไฟล์นี้ไม่อ่าน process.env เอง) · ไม่ส่ง = default true (ON)
  //   rulesV3On=false → ข้อความ (4)+(6) กลับเป็นของเดิมก่อนแก้ทุก byte (rollback, ใช้ได้เฉพาะเมื่อ storyRulesOn=true)
  const _item4Hero = rulesV3On
    ? '★สูตรเล่าเรื่องตามเพจ (วิจัยปกจริง 478 ใบ): ช่อง hero สั่งหน้านิ่ง มองตรง/เฉียง เห็นหน้าชัดเต็ม มีเรื่องซ่อนในสายตา ใหญ่เต็มช่อง — ห้ามสั่งอารมณ์ร้องไห้/น้ำตาคลอเป็น hero (ปกแสนไลค์ 18 ใบไม่มีเลยสักใบ — น้ำตาให้ไปอยู่ช่องย่อยหรือวงกลมแทน) ไม่ใช่ภาพโพสนิ่ง/แฟชั่น/โปรโมทถ้ามีตัวเลือกจริงให้เลือก'
    : '★สูตรเล่าเรื่องตามเพจ (วิจัยปกยอด ≥2 หมื่นไลค์): ช่อง hero สั่งอารมณ์ให้ตรง "อารมณ์หลัก" ของเข็มทิศเสมอ (ร้องไห้/น้ำตาคลอ/ยิ้มกว้าง ตามโทนข่าว) ใหญ่เต็มช่อง ไม่ใช่ภาพโพสนิ่ง/แฟชั่น/โปรโมทถ้ามีตัวเลือกอารมณ์จริงให้เลือก';
  const _item6Circle = rulesV3On
    ? '★ช่องวงกลม (ถ้ามี) = ตัวที่ 2 ของเรื่อง: สั่งใบหน้าตัวละครที่ 2 / ภาพเก่า-วัยเด็ก / โมเมนต์กอด-กราบ / ขาวดำผู้ล่วงลับ+ป้ายชื่อ — ต้องเล่าสิ่งที่ hero ไม่ได้เล่า · ยังต้องมีหน้าคนในภาพ (ด่านกลไกปัจจุบันบังคับ)'
    : '★ช่องวงกลม (ถ้ามี) สั่งบุคคลที่สองของเรื่อง เลือกช็อตหน้าชัดโฟกัสเดี่ยว ไม่ใช่ภาพหมู่/ไกล';
  const _storyRulesNote = storyRulesOn ? ` (4) ${_item4Hero} (5) ★ช่องรอง (ไม่ใช่ hero/circle) สั่งให้เรียงเป็นไทม์ไลน์เล่าเรื่อง: บริบท/ที่มา → โมเมนต์หัวใจ → ผลลัพธ์/จุดพลิก (6) ${_item6Circle} (7) ★ห้ามสั่งภาพฉากรายการทีวี/สตูดิโอ/ฉากหลังจอ-ไฟเวที/ตัวอักษรใหญ่ฉากหลัง เว้นแต่ไม่มีทางเลือกอื่นจริงๆ (8) ★ช่องย่อย (ไม่ใช่ hero) เลี่ยงภาพมืด/เบลอ/คุณภาพต่ำ (สังเกตจาก note เช่น "เบลอ/มืด/คุณภาพต่ำ" หรือค่า quality ต่ำ) เว้นแต่ไม่มีตัวเลือกอื่นในเรื่องเดียวกันจริงๆ (9) ★ทั้งปกควรมีช่องภาพมืด/กลางคืนไม่เกิน 1 ช่อง — เมื่อมีหลายใบเข้าเรื่องพอๆ กัน ให้เลือกภาพสว่างชัดก่อนเสมอ (สังเกตจาก note)` : '';
...
${_pagePlaybookBlock}กฎเหล็ก: (1) hero = หน้าเดี่ยวตัวเอกของข่าวเสมอ ห้ามภาพหมู่ (2) สั่งเฉพาะภาพที่ข่าวนี้มีโอกาสมีจริง (3) ช่องไหน ref ใส่โมเมนต์/หลักฐาน ให้แปลงเป็นโมเมนต์/หลักฐานของข่าวนี้${_storyRulesNote}
...
export async function slotDirectorBrain({ imagesMeta, compass, deskTitle, refDNA = null, artBrief = null, sceneInventory = '', slotContract = null, storyRulesOn = true, pagePlaybookOn = true, rulesV3On = true, imgMetaBudget = 45000, _deps = {} }) {
  // ★ ตรวจซ้ำ (28 ก.ค. 69 — คัมภีร์เพจฉบับเต็ม): เหมือน artBriefBrain เป๊ะ — แทรกทั้งก้อน verbatim ก่อนบล็อกกติกาเดิม
  //   ทั้ง legacy `system` และ `systemSem` (สองระบบ prompt คนละโหมด) · false = byte-identical เดิมทั้งคู่
  const _pagePlaybookBlock = pagePlaybookOn ? `${COVER_PAGE_PLAYBOOK}\n\n` : '';
  const _cb = _deps.callBrain || callBrain;
  // ★ TIER3 (สูตรเล่าเรื่องตามเพจ — memory igdara-cover-formula, เจ้าของอนุมัติ 27 ก.ค. 69): กติกา hero=อารมณ์พีค +
  //   ไทม์ไลน์ช่องรอง (reaction→action→context) + circle=บุคคลที่สองโฟกัสเดี่ยว + เลี่ยงฉากรายการ/สตูดิโอ —
  //   caller (megaAdapters.js) อ่าน env MEGA_STORY_RULES แล้วส่ง storyRulesOn มา (ไฟล์นี้ไม่อ่าน process.env เอง
  //   — แพทเทิร์นเดียวกับ _hasFaceH/_hasBusy/_hasFaceFront ด้านล่าง: เติมข้อความก็ต่อเมื่อสัญญาณเป็นจริง) ·
  //   ไม่ส่งพารามิเตอร์ = default true (ON) · storyRulesOn=false → prompt byte-identical กับก่อน TIER3 ทั้งสอง system
  ...
  // ★ MEGA_CLUTTER_GUARD (มือ C, 19 ก.ค.): field busy (0-2, ระดับลายตาจาก Gemini triage) มีเฉพาะ megaAdapters.js เมื่อสวิตช์ MEGA_CLUTTER_GUARD===1
  //   เลียนแบบ _hasFaceH เป๊ะ — ตรวจจาก "ข้อมูลจริงที่ส่งมา" ไม่มีใบไหนมี busy เลย (สวิตช์ปิด/ผู้ส่งเก่า) → ไม่เติมกฎใหม่ = prompt เดิมเป๊ะ byte-parity
  const _hasBusy = Array.isArray(imagesMeta) && imagesMeta.some((m) => typeof m?.busy === 'number');
  // ★ 21 ก.ค. (MEGA_HERO_FRONTAL — เคส AC-0166 hero มุมข้าง): field faceFront 0-2 มีเฉพาะเมื่อสวิตช์เปิดฝั่ง
  //   megaAdapters — ตรวจจากข้อมูลจริงที่ส่งมา (ไฟล์นี้ไม่อ่าน process.env) · ไม่มีใบไหนมี = prompt เดิมเป๊ะ byte-parity
  const _hasFaceFront = Array.isArray(imagesMeta) && imagesMeta.some((m) => typeof m?.faceFront === 'number');
  // ★ MEGA_RULES_V3 (28 ก.ค. 69 — เจ้าของสั่งรื้อกติกาเก่าที่ขัด v3): 3 จุดในไฟล์นี้ (item 13 busy-guard circle-clause,
  //   item 15 legacy hero+circle-clause, item 14 systemSem hero+circle-clause) ขัดกับคัมภีร์ v3 — เขียนใหม่ 2 เรื่อง:
  //   (ก) hero=อารมณ์พีค/ร้องไห้/น้ำตาคลอ → v3 ก.3 หน้านิ่งชนะ (ปกแสนไลค์ 18 ใบไม่มี hero ร้องไห้เลย)
  //   (ข) circle=ต้องเป็นคนเดี่ยวโฟกัสชัดเท่านั้น → v3 ง: circle ยังเป็นภาพเก่า/วัยเด็ก/โมเมนต์กอด-กราบ (มักมี 2 คน
  //   ในเฟรม)/ขาวดำผู้ล่วงลับได้ ไม่ใช่บังคับ "คนเดี่ยว" เสมอไป — caller อ่าน env MEGA_RULES_V3 ส่ง rulesV3On มา
  //   (ไฟล์นี้ไม่อ่าน process.env เอง) · ไม่ส่ง = default true (ON) · rulesV3On=false = ข้อความเดิมทุก byte (rollback)
  const _item13Circle = rulesV3On
    ? 'ช่อง circle เลือกภาพไม่ลายตา (busy 0-1) เสมอที่เป็นไปได้ — ไม่บังคับต้องเป็นคนเดี่ยว (โมเมนต์กอด-กราบ/ภาพเก่ามีมากกว่า 1 คนได้ ขอแค่โฟกัสชัด)'
    : 'ช่อง circle ต้องเป็นคนเดี่ยวโฟกัสชัด (busy 0-1) เท่านั้น';
  const _item15Hero = rulesV3On
    ? '★สูตรเล่าเรื่องตามเพจ (วิจัยปกจริง 478 ใบ): hero เลือกใบหน้านิ่ง มองตรง/เฉียง เห็นหน้าชัดเต็ม มีเรื่องซ่อนในสายตา ใหญ่เต็มช่อง — ห้ามเลือกภาพร้องไห้/น้ำตาคลอเป็น hero (ปกแสนไลค์ 18 ใบไม่มีเลยสักใบ น้ำตาให้ไปอยู่ช่องย่อยหรือวงกลมแทน) ไม่ใช่ภาพโพสนิ่ง/แฟชั่น/โปรโมทถ้ามีตัวเลือกจริงให้เลือก'
    : '★สูตรเล่าเรื่องตามเพจ (วิจัยปกยอด ≥2 หมื่นไลค์): hero เลือกใบหน้าที่มีอารมณ์ตรง "อารมณ์หลัก" ในเข็มทิศเสมอ (ร้องไห้/น้ำตาคลอ/ยิ้มกว้าง ตามโทนข่าว) ใหญ่เต็มช่อง — ห้ามภาพโพสนิ่ง/แฟชั่น/โปรโมทถ้ามีตัวเลือกอารมณ์จริงให้เลือก';
  const _item15Circle = rulesV3On
    ? 'circle = ตัวที่ 2 ของเรื่อง: ใบหน้าตัวละครที่ 2 / ภาพเก่า-วัยเด็ก / โมเมนต์กอด-กราบ / ขาวดำผู้ล่วงลับ+ป้ายชื่อ — ต้องเล่าสิ่งที่ hero ไม่ได้เล่า · ยังต้องมีหน้าคนในภาพ (ด่านกลไกปัจจุบันบังคับ)'
    : 'circle = บุคคลที่สองของเรื่อง เลือกช็อตหน้าชัดโฟกัสเดี่ยว (ไม่ใช่ภาพหมู่/ไกล)';
...
(11) ★ภาพที่คนหันหลัง/ก้มกราบ/เห็นแต่แผ่นหลัง (สังเกตจาก note เช่น "กราบ/หันหลัง/มองจากด้านหลัง") ใช้ได้เฉพาะช่องฉากกว้าง (context) เท่านั้น — ห้ามลงช่องเล็ก/ช่องคน เพราะครอปแล้วหัวขาดง่ายและไม่เห็นว่าเป็นใคร${_hasFaceH ? '\n(12) ★faceH = สัดส่วนความสูงหน้าเทียบเฟรม (0-1, ยิ่งมากยิ่งหน้าใหญ่เด่น) — ช่อง hero เลือกภาพ faceH สูงก่อนเสมอ หลีกเลี่ยงภาพหน้าเล็กในช็อตกว้าง/มี backdrop เยอะ แม้ภาพนั้นจะดูสะอาด/คุณภาพสูง' : ''}${_hasBusy ? `\n(13) ★busy = ระดับลายตา 0-2 (0=สะอาดโฟกัสชัด, 1=มีกิจกรรมพื้นหลังบ้างแต่ยังเห็น subject ชัด, 2=ลายตา คนเยอะหาโฟกัสไม่ได้/มุมกว้างไม่มีจุดเด่น) — ช่องย่อย (reaction/action/context/circle) เลี่ยงภาพ busy=2 นอกจากไม่มีตัวเลือกอื่นจริงๆ · ${_item13Circle} · เมื่อหลายใบเข้าเรื่องพอๆ กัน ให้เลือกใบ busy ต่ำกว่าก่อนเสมอ` : ''}${_hasFaceFront ? '\n(14) ★faceFront = มุมการเห็นหน้า 0-2 (0=หันหลัง/หน้าถูกบัง, 1=มุมข้าง/ก้มหน้า/เห็นบางส่วน, 2=เห็นเต็มหน้า) — 🔴ช่อง hero เลือกเฉพาะ faceFront=2 เท่านั้น (ภาพมุมข้าง/ก้มหน้าแม้อารมณ์ตรงก็ห้ามขึ้น hero — คนดูวิแรกต้องรู้ว่าใคร) · ภาพ faceFront 0-1 ใช้ได้เฉพาะช่องย่อย (action/context สื่ออารมณ์/โมเมนต์) · ช่อง circle เลือก faceFront=2 ก่อนเสมอถ้ามี' : ''}${storyRulesOn ? `\n(15) ${_item15Hero} · 🔴ถ้าพูลมีภาพเห็นหน้าเต็ม (ตา+จมูก+ปาก) ของตัวเอก ห้ามเลือกภาพหน้าถูกหมวก/หน้ากาก/มือบังเป็น hero เด็ดขาด · 🔴ทุกช่องห้ามช็อตซ้ำประเภทกับ hero ของคนเดิม (เช่นเซลฟี่หมวก/หน้ากากซ้ำ 2 ช่อง) ช่องย่อยต้องเล่าคนละฉาก/คนละโมเมนต์ · ช่องย่อย reaction/action/context เรียงไทม์ไลน์เล่าเรื่อง: บริบท/ที่มา → โมเมนต์หัวใจ → ผลลัพธ์/จุดพลิก (อ่านจาก note/emotion) · ${_item15Circle} · ภาพที่ note บ่งชี้ฉากรายการทีวี/สตูดิโอ/ฉากหลังจอ-ไฟเวที/ตัวอักษรใหญ่ฉากหลัง = เลี่ยงทุกช่องย่อย เว้นแต่ไม่มีตัวเลือกอื่นจริงๆ · ช่องย่อย (ไม่ใช่ hero) เลี่ยงภาพมืด/เบลอ/คุณภาพต่ำ (สังเกตจาก note) เว้นแต่ไม่มีตัวเลือกอื่นจริงๆ · 🔴มืด/กลางคืนเกิน 1 ช่อง=ผิดกติกาทันที เลือกภาพสว่างชัดก่อนเสมอ` : ''}
ตอบ JSON เท่านั้น:
{"slots":{"hero":{"id":"...","reason":"สั้นๆ","backups":["id","id"]},"reaction":{...},"action":{...},"context":{...},"circle":{...}},"note":"ข้อสังเกตรวม 1 ประโยค"}`;
  ...
  // ★ ข้อ 6 (27 ก.ค. 69 — เจ้าของจับได้จากล็อกจริง: สมองเห็นแค่ 39-58/77-80 ใบ เพราะงบเดิม 18000 ยุค opus-4-8
  //   คับแคบเกินไปเมื่อป้าย/กติกาที่เพิ่มวันนี้ทำให้แต่ละใบยาวขึ้น ⇒ ภาพดีถูกตัดก่อนสมองเห็นด้วยซ้ำ): งบมาจากพารามิเตอร์
  //   imgMetaBudget (caller/megaAdapters.js อ่าน env MEGA_IMG_META_BUDGET + clamp แล้วส่งมา — ไฟล์นี้ไม่อ่าน process.env
  //   เอง แพทเทิร์นเดียวกับ storyRulesOn) · ไม่ส่งพารามิเตอร์ = default 45000 (เดิม 18000 → เผื่อพูล ~80-100 ใบเข้าครบ)
  const IMG_META_BUDGET = imgMetaBudget;
  ...
  // ★ ข้อ 6: log ทุกรอบไม่ว่าตัดหรือไม่ (เดิม log เฉพาะตอนตัด — ไม่มีทางเฝ้าเห็นว่า "ครบ" จริงหรือบังเอิญไม่ตัดพอดี)
  console.log(`[MEGA S6] 📋 meta ส่ง ${_included}/${_lines.length} ใบ (งบ ${IMG_META_BUDGET})${_included < _lines.length ? ' — ใบท้ายคิว (คะแนนต่ำสุด) ถูกตัด' : ''}`);
...
[${_lines.slice(0, _included).join(',\n')}]`;
  ...
  if (_semSlots) {
    ...
    // ★ MEGA_RULES_V3 (28 ก.ค. 69): เหมือน _item15Hero/_item15Circle (legacy) เป๊ะ แค่อ้าง instance id จริงแทน
    //   คำว่า "hero" ลอยๆ — rulesV3On=false = ข้อความเดิมทุก byte (rollback)
    const _item14Hero = rulesV3On
      ? `${_heroInst.id} เลือกใบหน้านิ่ง มองตรง/เฉียง เห็นหน้าชัดเต็ม มีเรื่องซ่อนในสายตา ใหญ่เต็มช่อง — ห้ามเลือกภาพร้องไห้/น้ำตาคลอเป็น ${_heroInst.id} (ปกแสนไลค์ 18 ใบไม่มีเลยสักใบ น้ำตาให้ไปอยู่ช่องย่อยหรือวงกลมแทน) ไม่ใช่ภาพโพสนิ่ง/แฟชั่น/โปรโมทถ้ามีตัวเลือกจริงให้เลือก`
      : `${_heroInst.id} เลือกใบหน้าที่มีอารมณ์ตรง "อารมณ์หลัก" ในเข็มทิศเสมอ (ร้องไห้/น้ำตาคลอ/ยิ้มกว้าง ตามโทนข่าว) ใหญ่เต็มช่อง — ห้ามภาพโพสนิ่ง/แฟชั่น/โปรโมทถ้ามีตัวเลือกอารมณ์จริงให้เลือก`;
    const _item14Circle = rulesV3On
      ? ` · ช่องวงกลม (${_circleIds.join('/')}) = ตัวที่ 2 ของเรื่อง: ใบหน้าตัวละครที่ 2 / ภาพเก่า-วัยเด็ก / โมเมนต์กอด-กราบ / ขาวดำผู้ล่วงลับ+ป้ายชื่อ — ต้องเล่าสิ่งที่ ${_heroInst.id} ไม่ได้เล่า · ยังต้องมีหน้าคนในภาพ (ด่านกลไกปัจจุบันบังคับ)`
      : ` · ช่องวงกลม (${_circleIds.join('/')}) เลือกช็อตหน้าชัดโฟกัสเดี่ยวของบุคคลที่สอง (ไม่ใช่ภาพหมู่/ไกล)`;
...
${roleLines}
${_pagePlaybookBlock}กฎเหล็ก: (1) ถูกคน 100% เหนือทุกข้อ — ${_heroInst.id} ต้องเป็น "ตัวเอกอันดับหนึ่ง" ของข่าวเท่านั้น และช่องที่ระบุ "คน:" ต้องได้ภาพของคนนั้นจริงตามป้าย person ห้ามคนอื่นเด็ดขาด (2) ทุกช่องคนละภาพ ห้ามซ้ำ และควรคนละฉาก (3) เลือกจาก id ในรายการเท่านั้น (4) quality ต่ำ (<4) ใช้เมื่อจำเป็นจริงๆ (5) ช่องไหนไม่มีภาพเข้าเกณฑ์จริงๆ ให้ id=null พร้อมเหตุผล — ห้ามฝืนยัดภาพผิดคน (6) ภาพ clean=false (มีลายน้ำ/ตัวหนังสือทับ) ห้ามขึ้นช่องถ้ามีตัวเลือกสะอาด (7) ภาพ newsScene=false = ภาพแฟ้มจากงาน/บริบทอื่น — เลี่ยงเสมอ ใช้เฉพาะไม่มีภาพเหตุการณ์จริง
(8) ★ปกทั้งใบต้องเล่าเรื่องครบ: ทุกช่องรวมกันต้องเห็น "คน → กำลังทำอะไร → หลักฐาน/สถานที่" — ห้ามเป็นพอร์ตเทรตล้วนทุกช่อง (9) ★ฉากห้ามซ้ำข้ามช่อง (เทียบจาก note — เฟรมคลิปเดียวกัน/เวทีเดิม = ฉากเดียวกัน) (10) ★${_heroInst.id} เลือกภาพ "หน้าเดี่ยว" (faces=1) หน้าใหญ่คมชัด แนวตั้ง/จัตุรัส (orient=tall/sq) ก่อนเสมอ — ห้ามภาพแนวนอนกว้าง/แบนเนอร์ถ้ามีตัวเลือกอื่นของตัวเอก${_circleIds.length ? ` (11) ★ช่องวงกลม (${_circleIds.join('/')}) ควรเป็นคนละคนกับ ${_heroInst.id} เมื่อช่องนั้นไม่ได้ระบุ "คน:" ไว้` : ''} (12) ★ภาพคนหันหลัง/ก้มกราบ/เห็นแต่แผ่นหลัง ใช้ได้เฉพาะช่องฉากกว้างเท่านั้น${_hasFaceH ? ` (13) ★faceH = สัดส่วนความสูงหน้าเทียบเฟรม (0-1) — ${_heroInst.id} เลือกภาพ faceH สูง (หน้าใหญ่เด่น) ก่อนเสมอ หลีกเลี่ยงภาพหน้าเล็กในช็อตกว้าง/มี backdrop เยอะ` : ''}${storyRulesOn ? ` (14) ★สูตรเล่าเรื่องตามเพจ (${rulesV3On ? 'วิจัยปกจริง 478 ใบ' : 'วิจัยปกยอด ≥2 หมื่นไลค์'}): ${_item14Hero} · 🔴ถ้าพูลมีภาพเห็นหน้าเต็ม (ตา+จมูก+ปาก) ของ ${_heroInst.id} ห้ามเลือกภาพหน้าถูกหมวก/หน้ากาก/มือบังเป็น ${_heroInst.id} เด็ดขาด · 🔴ทุกช่องห้ามช็อตซ้ำประเภทกับ ${_heroInst.id} ของคนเดิม (เช่นเซลฟี่หมวก/หน้ากากซ้ำ) ช่องย่อยต้องเล่าคนละฉาก/คนละโมเมนต์ เรียงไทม์ไลน์เล่าเรื่อง: บริบท/ที่มา → โมเมนต์หัวใจ → ผลลัพธ์/จุดพลิก (อ่านจาก note/emotion)${_circleIds.length ? _item14Circle : ''} · ภาพที่ note บ่งชี้ฉากรายการทีวี/สตูดิโอ/ฉากหลังจอ-ไฟเวที/ตัวอักษรใหญ่ฉากหลัง = เลี่ยงทุกช่องย่อย เว้นแต่ไม่มีตัวเลือกอื่นจริงๆ · ช่องย่อย (ไม่ใช่ ${_heroInst.id}) เลี่ยงภาพมืด/เบลอ/คุณภาพต่ำ (สังเกตจาก note) เว้นแต่ไม่มีตัวเลือกอื่นจริงๆ · 🔴มืด/กลางคืนเกิน 1 ช่อง=ผิดกติกาทันที เลือกภาพสว่างชัดก่อนเสมอ` : ''}
ตอบ JSON เท่านั้น:
${schemaSem}`;
  }
