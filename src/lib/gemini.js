...
// เลือกชุดคีย์ตาม 2 flag อิสระกัน (fileTagOn เดิม × busyOn ใหม่) — ไม่กระทบ path เดิมที่ busyOn=false เสมอ
function classifierItemKeys(fileTagOn, busyOn) {
...
function readClassifierItem(raw, requiredKeys, fileTagOn, busyOn) {
...
function validateClassifierItemsV1(raw, expectedCount, fileTagOn, busyOn) {
  ...
  const requiredKeys = classifierItemKeys(fileTagOn, busyOn);
  const seen = new Set();
  const out = [];
  for (const rawItem of itemsArr) {
    const it = readClassifierItem(rawItem, requiredKeys, fileTagOn, busyOn);
...
async function runClassifierStrict({ url, body, pin, onRetry, cost, expectedCount, fileTagOn, busyOn, externalSignal }) {
  ...
  try {
    ...
    const schemaResult = validateClassifierItemsV1(parse.value, expectedCount, fileTagOn, busyOn);
...
export function sanitizeStrictClassifierItem(it, fileTagOn, busyOn) {
  const requiredKeys = classifierItemKeys(fileTagOn === true, busyOn === true);
  const out = readClassifierItem(it, requiredKeys, fileTagOn === true, busyOn === true);
