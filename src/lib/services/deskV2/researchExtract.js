...
// 🔒 P2.1 (17 ก.ค. 69 ผู้ใช้สั่ง): ท่อถอดคลิปรับเฉพาะ "ลิงก์วิดีโอจริง" เท่านั้น — YouTube (watch/shorts/live/youtu.be),
 ...
 * @param {object} lead - ใช้ lead.url เท่านั้น — channel/sourceHost เชื่อไม่ได้
 *   (บทเรียน 17 ก.ค.: channel 'facebook' + host-based พาโพสต์กลุ่ม/โพสต์รูปเข้าคิวถอดคลิป → yt-dlp พัง retry วน 30+ รอบ)
 * @returns {'article'|'clip'}
 */
export function classifyExtractRoute(lead) {
  const url = String(lead?.url || '').toLowerCase();
  return CLIP_URL_RE.test(url) ? 'clip' : 'article';
}
