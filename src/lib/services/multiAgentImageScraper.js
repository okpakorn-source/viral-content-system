...
- 🔴🔴 QUALITY FIRST (กฎเหนือทุกข้อ — บทเรียน CASE-228 เฟรมเบลอดันขึ้นฮีโร่): ภาพ "เบลอ / โฟกัสหลุด / ความละเอียดต่ำ-เม็ดหยาบ / มืดเกิน / หน้าไม่คมชัด" → score ≤ 4 เสมอ **ห้ามเป็นฮีโร่ ห้ามเข้าวงกลม** แม้จะเป็นเฟรมข่าว/แคนดิดก็ตาม — ปกข่าวต้อง "คมชัด" มาก่อนเป็นอันดับ 1 · **เฟรมคลิปที่เบลอ = แย่กว่า ภาพสตูดิโอที่คมชัด** (อย่าเลือกเฟรมเบลอเพราะมันเป็น documentary)
- ลำดับความชอบที่ถูกต้อง: แคนดิด/เฟรมข่าว"ที่คม" > สตูดิโอ"ที่คม" > ... > เบลอทุกชนิด (ทิ้ง)
- ✅ UPRANK (+1 ถึง +2) เฉพาะภาพที่ "คมชัดจริง": ภาพ "แคนดิด / เฟรมจากคลิปสัมภาษณ์ / ข่าว / รายการทีวี / ถ่ายสดเหตุการณ์" — สีหน้า/แสง/ฉากเป็นธรรมชาติจริง = ตรงสไตล์ปกข่าวไวรัลที่ปัง (เหมือนดูข่าว)
...
★★★ 🦸 HERO ต้อง "หน้าคนชัด รู้ว่าใครทันที" — ฮีโร่ไม่จำเป็นต้องมาจากคลิปต้นทาง (28 มิ.ย. ผู้ใช้+GPT สั่ง · บทเรียน CASE-231 เฟรมคลิปเป็นฉาก ไม่เห็นหน้า → ฮีโร่ไม่เด่น) ★★★
- ✅ HERO_FACE ให้เฉพาะภาพที่: เห็น "หน้าชัด หน้าตรง/เกือบตรง" · รู้ทันทีว่าใคร · เด่นดึงสายตา · crop แน่นพอดี (Facebook ดันรีชภาพ "ใบหน้าเด่น+บุคคลชัด")
- ⛔ ห้ามให้ HERO_FACE + กด score ≤5 (เป็นได้แค่ CONTEXT_SCENE) กับภาพ: เบลอ/มัว/แตก · หันข้างมาก/หันหลัง · ไมค์บังหน้า · แว่นดำบังตา · เฟรมไกล/ตัวเล็ก · โล่งเกินไป/รกเกินไป · ดูไม่ออกว่าใคร
- ★ เฟรมจาก "คลิปต้นทาง": ใช้ได้ แต่ "ถ้าไม่เห็นหน้าชัด = CONTEXT (ภาพบริบท) ไม่ใช่ HERO" — อย่าฝืนดันเฟรมคลิปเป็นฮีโร่ · ถ้าเป็นคนดัง/ดารา/บุคคลสาธารณะ → ภาพหน้าชัดจาก research (Google/ข่าว) ควรเป็นฮีโร่แทน · เฟรมคลิปเก็บไว้เล่า "สถานการณ์/หน้างาน/บริบท" เท่านั้น และเลือกเฉพาะเฟรมที่ "เห็นหน้ามากที่สุด"
- ★ ลำดับฮีโร่: (1) หน้าชัดสุด (2) ระบุตัวได้ทันที (3) องค์ประกอบแน่นพอดีไม่โล่ง (4) เด่นเหมาะไวรัล FB
- ★ วงกลม: หน้าเด่นชัดมีพลัง หรือบริบทเด่นเล่าเรื่องได้จริง — ⛔ ห้ามเบลอ/มัว/มองไม่ออกว่าอะไร

★★★ 🗂️ เข้าใจ "ประเภทข่าว" ก่อนเลือกภาพ — อย่าใช้กฎเดียวกับทุกข่าว (28 มิ.ย. ผู้ใช้+GPT · ข่าว success/ตอบแทนทีม) ★★★
- ดูแก่นข่าวก่อน แล้วเลือกภาพ "ให้ตรงแก่น" (ไม่ใช่เอาแต่ภาพสวยแฟชั่น):
  · ข่าว success/ธุรกิจ/ตอบแทนทีม/น้ำใจ → ภาพ "ทีมงาน/กลุ่มคน/ฉลอง/มอบรางวัล/ครอบครัว/ทริป" = ON-STORY คะแนนสูง (สื่อ "ไม่ลืมคนเบื้องหลัง/แบ่งปันความสำเร็จ")
  · ข่าวครอบครัว/อบอุ่น → ภาพคู่/ครอบครัว/โมเมนต์อบอุ่น = ON-STORY
  · ข่าวดราม่า/กระแส → ภาพเหตุการณ์/สีหน้าจริง · ข่าวสู้ชีวิต → ภาพครุ่นคิด/สถานการณ์จริง
  · ข่าวแฟชั่น/ลุคใหม่/ความสวย → ภาพสวย/ลุคเด่น = ON-STORY (เฉพาะข่าวแนวนี้เท่านั้น)
- ⛔ ภาพสวยแฟชั่นที่ "ไม่ตรงแก่นข่าว" → ใช้เป็น CONTEXT รองได้ ไม่ใช่จุดขาย · hero ยังต้อง identity หน้าชัด
- เป้าหมาย: ปกต้อง "เล่าเรื่องของข่าว" (story clarity) ไม่ใช่แค่ "โชว์ว่าใครสวย" — ภาพรองทุกใบต้องมีหน้าที่ (ทีม/บริบท/อารมณ์/เหตุการณ์) ห้ามภาพสุ่มที่ไม่ช่วยเล่าเรื่อง
...
- ★ CONTEXT_SCENE = ฉาก/สถานที่/ฝูงชนมุมกว้าง ที่ "ไม่มีคนแปลกหน้าคนใดเด่นเป็นตัวเอกของภาพ" — ถ้าเป็นพอร์ตเทรต/ครึ่งตัวของคนแปลกหน้า 1 คน → ไม่ใช่ CONTEXT_SCENE → REJECT
- ✅ ข้อยกเว้น (คงไว้ กันตัดภาพคนในข่าวทิ้ง): ภาพ "โคลสอัพ/พอร์ตเทรตเดี่ยว" ที่อาจเป็น ${mainChar} แต่ไม่ชัวร์ 100% → ยังเก็บได้ — กฎนี้เข้มเฉพาะ "ภาพที่มีคนอื่นเด่นชัดว่าไม่ใช่คนในข่าว"
→ เมื่อต้องเลือกภาพคน ให้เลือก "${mainChar} เดี่ยว" หรือ "${mainChar} + ครอบครัว/ตัวรองที่ข่าวระบุ" เสมอ — ห้ามเอาคนนอกเข้ามาเติมช่อง
...
=== ★★★★ FACE RECOGNIZABILITY — เกณฑ์สำคัญสุดสำหรับ "ภาพคน" (ทุกภาพต้องจำได้ว่าใคร!) ===
หลักการ: ปกที่ดี = ทุกช่อง "คน" คนอ่านต้องดูออกทันทีว่าเป็นใคร ถ้าจำหน้าไม่ได้ = ภาพนั้นทำให้ข่าวดูไม่รู้เรื่อง
...
function processJudgeResults(parsed, validCandidates, allowFallback = true) {
  ...
  // ★ 28 มิ.ย. (#1 ผู้ใช้สั่งห้ามภาพขยะ — path นี้คือ Claude fallback ที่ใช้จริง CASE-231 มี 1 REJECT หลุด): ตัด role=REJECT ออกแม้ score≥3
  const accepted = parsed.filter(s => s.score >= 3 && s.role !== 'REJECT');
  const nearMiss = parsed.filter(s => s.score === 2 && s.role !== 'REJECT');
  const rejected = parsed.filter(s => s.score < 2 || s.role === 'REJECT');

  console.log(`[Judge] AI scores: ${parsed.map(s => `#${s.index}=${s.score}(${s.role})`).join(', ')}`);
  console.log(`[Judge] 📊 Accepted(≥3): ${accepted.length}, Near-miss(2): ${nearMiss.length}, Rejected: ${rejected.length}`);
  // ★ 28 มิ.ย. (#2 debug ต่อใบ): โชว์ score/role/แหล่ง ต่อภาพ → เห็นว่าทำไมเลือก/ตัด
  try {
    parsed.slice(0, 30).forEach(s => {
      let host = '?'; try { host = new URL(String(validCandidates[s.index] || '')).hostname.replace(/^www\./, ''); } catch {}
      const verdict = (s.score >= 3 && s.role !== 'REJECT') ? '✅รับ ' : (s.score === 2 ? '🟡near' : '❌ตัด ');
      console.log(`   🔍 #${s.index} ${verdict} score=${s.score} role=${s.role} src=${host}`);
    });
  } catch {}
