 ...
 */
export function extractIdentityAnchors({ newsTitle = '', newsBody = '', breakdownData = null, resolvedIdentity = null }) {
  const text = `${newsTitle}\n${newsBody}`;
  const names = new Set();
  const anchors = new Set();

  // ★ Context-Resolve (11 มิ.ย.): ตัวตนที่ยืนยันจากการค้นรอบระบุตัวตน — เพิ่มชื่อจริง/ฉายา/จุดยึดพิเศษ
  //   ทำให้ข่าว "ชื่อเล่นล้วน" (เช่น 'กาฟิว') มีจุดยึดพอให้ผล research รอดตัวกรองแทนการโดนทิ้งหมด
  if (resolvedIdentity?.fullName) {
    names.add(String(resolvedIdentity.fullName).trim());
    (resolvedIdentity.aliases || []).forEach(a => { if (String(a).trim().length >= 2) names.add(String(a).trim()); });
    (resolvedIdentity.extraAnchors || []).forEach(a => { if (String(a).trim().length >= 3) anchors.add(String(a).trim()); });
  }
...
export async function verifyResearchItems({ newsTitle, newsBody, breakdownData, items, getText, resolvedIdentity = null }) {
  ...
  const { names, anchors } = extractIdentityAnchors({ newsTitle, newsBody, breakdownData, resolvedIdentity });
  const report = [];
