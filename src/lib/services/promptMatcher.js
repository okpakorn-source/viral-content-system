...
/**
 * ★ 25 ก.ค. 69 — กันหยิบพร้อมท์ซ้ำ (เจ้าของสั่ง: "ให้ใช้ศักยภาพคลังพร้อมท์ได้สูงสุด")
 * ─────────────────────────────────────────────────────────────
 * ปัญหาเดิม: สูตรให้คะแนนดูแต่ "ป้าย" ใบที่ป้ายกว้างที่สุดจึงชนะทุกข่าว
 *   → prompt เดียวถูกใช้ 411 ครั้ง ขณะที่ 6 ใบไม่เคยถูกใช้เลย (วัดจาก data/prompt-library.json)
 * วิธีแก้: หักคะแนนใบที่ใช้ถี่เกินค่ากลาง + ใบที่เพิ่งใช้ไปหมาดๆ · ให้แต้มใบที่ยังไม่เคยออกงาน
 * ปิดคืน: env PROMPT_ANTI_REPEAT=off (กลับไปเรียงตามคะแนนดิบเป๊ะ)
 */
export function applyDiversityPenalty(scored, validPrompts, { now = null } = {}) {
  if (process.env.PROMPT_ANTI_REPEAT === 'off') return scored;
  if (!Array.isArray(scored) || !scored.length) return scored;

  const COOLDOWN_HARD_H = Number(process.env.PROMPT_COOLDOWN_HOURS) || 6;
  const COOLDOWN_SOFT_H = COOLDOWN_HARD_H * 4;
  const nowMs = now ? new Date(now).getTime() : Date.now();

  const counts = validPrompts.map(p => Number(p?.usageCount) || 0).sort((a, b) => a - b);
  const median = counts.length ? counts[Math.floor(counts.length / 2)] : 0;

  const out = scored.map(s => {
    const p = validPrompts[s.index] || {};
    const used = Number(p.usageCount) || 0;
    let penalty = 0;
    const notes = [];

    // ใช้ถี่กว่าค่ากลางมาก = ใบป้ายกว้าง ผูกขาดงาน → หักคะแนน (เพดาน -12)
    if (median > 0 && used > median) {
      const pen = Math.min(12, Math.round(((used / median) - 1) * 4));
      if (pen > 0) { penalty += pen; notes.push(`overuse(-${pen})`); }
    }

    // เพิ่งใช้ไปหมาดๆ → พักก่อน ให้ใบอื่นได้ออกงานบ้าง
    const lastMs = p.lastUsedAt ? new Date(p.lastUsedAt).getTime() : 0;
    if (lastMs > 0 && Number.isFinite(lastMs)) {
      const hrs = (nowMs - lastMs) / 3600000;
      if (hrs < COOLDOWN_HARD_H) { penalty += 15; notes.push('cooldown(-15)'); }
      else if (hrs < COOLDOWN_SOFT_H) { penalty += 7; notes.push('cooldown(-7)'); }
    }

    // ยังไม่เคยถูกหยิบเลย → ให้โอกาสนิดหน่อย (ไม่พอจะพลิกใบที่ตรงกว่าชัดๆ)
    if (used === 0) { penalty -= 5; notes.push('fresh(+5)'); }

    if (!penalty) return s;
    return { ...s, score: Math.max(0, s.score - penalty), rawScore: s.score, diversity: notes };
  });
...
/**
 * ★ 25 ก.ค. 69 — คะแนนใกล้กัน = สุ่มถ่วงน้ำหนัก ไม่ใช่หยิบอันดับ 1 ตายตัว
 * เดิม `scoredPrompts[0]` ทำให้ข่าวแนวเดียวกันได้การ์ดใบเดิมทุกครั้ง
 * ปิดคืน: env PROMPT_TIE_RANDOM=off
 */
export function pickAmongTies(scored, { window = 6, pool = 3, rand = Math.random } = {}) {
  if (!Array.isArray(scored) || !scored.length) return null;
  if (process.env.PROMPT_TIE_RANDOM === 'off') return scored[0];

  const top = scored[0].score;
  const ties = scored.slice(0, pool).filter(s => (top - s.score) <= window);
  if (ties.length <= 1) return scored[0];

  const weights = ties.map(t => Math.max(1, (t.score - top + window) + 1));
  const total = weights.reduce((a, b) => a + b, 0);
  let r = rand() * total;
  for (let i = 0; i < ties.length; i++) {
    r -= weights[i];
    if (r <= 0) return ties[i];
  }
  return ties[0];
}
