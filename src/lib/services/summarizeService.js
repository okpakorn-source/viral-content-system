...
function extractKeyNumbers(sourceText) {
  ...
  while ((m = re.exec(s)) !== null && found.length < 8) {
    ...
    // เก็บเฉพาะเลขที่ "เด่นจริง": มี ≥2 หลัก หรือคู่กับหน่วยเงินก้อนใหญ่
...
function flagCrossVersionDuplicates(versions) {
  ...
  for (let i = 0; i < contents.length; i++) {
    for (let j = i + 1; j < contents.length; j++) {
      ...
      if (!a || !b) continue;
      // ประโยคปิด: หาง 35 ตัวอักษรเหมือนกันเป๊ะ = จบซ้ำ
      ...
      // วลียาวซ้ำกลางเรื่อง: หน้าต่าง 28 ตัวอักษร ก้าวทีละ 14
...
export async function performSummarize({
  ...
  const lengthConfig = {
    ...
    long:   { min: 500, max: 1000, paragraphs: '3-5', paraDesc: '3-5 ย่อหน้า', sentences: '4-8' },
  };
  let lenCfg = lengthConfig[contentLength] || lengthConfig.short;
  // ★ 14 ส.ค. 69 (VIRAL_HITS_FORMULA=1): ล็อกฟอร์มโพสต์แสนไลก์จริง — กลุ่มแสนไลก์ของเพจ 27/27 ใบ = 3 ย่อหน้า
  //   ทับเฉพาะ medium/long (short เป็น 3 ย่อหน้าอยู่แล้ว) · กฎข่าวบาง 2 ย่อหน้าด้านล่างยังชนะเสมอ (ทำงานทีหลัง)
...
วิเคราะห์ข่าวนี้ออกมาเป็น JSON Format โดยมีโครงสร้างดังนี้:
{
  ...
  "share_triggers": {
    "triggers": ["ข้อคิด", "เตือนภัย", "สะเทือนใจ", "อยากชื่นชม", "อยากให้กำลังใจ"] // ประเด็นมนุษย์ที่ชวนแชร์ (เพจเล่ามุมบวก — ห้ามมี "อยากด่า")
          ...
          // --- STAGE 2: CLUSTER-BASED HYBRID SCORER & MATCHER ---
          // ★ DNA v3.3 (12 มิ.ย. 69): สูตรให้คะแนนย้ายไปสมองกลาง promptMatcher.js (เดิมก๊อปไว้ 4 ที่ เริ่ม drift)
          //   + แท็กอารมณ์ข่าวถูกขยายเป็น "มุมบวกที่จะเล่า" อัตโนมัติ (ข่าวเศร้า→ซึ้ง/ตื้นตัน, ข่าวโกรธ→เห็นใจ/ชื่นชม)
          //   สอดคล้อง TONE_OVERRIDE ที่บังคับเล่าบวกท้ายน้ำ → พร้อมท์ v3 แท็กบวกแมตช์ได้เต็มเพดาน 25
          ...
          try {
            ...
            top10PromptScores = scoredPrompts.slice(0, 10).map(s => {
              ...
              return {
                ...
                // ★ 1 ส.ค. 69 (Opus P2-D — พอร์ต B5 จากแฝด TEXT): แนบโทน+เนื้อย่อจริง ให้ AI ตัดสินจากเนื้อ ไม่ใช่แค่ชื่อ
            ...
            if (winner) {
              ...
              if (matchType === 'BORROWED') {
                // รายการช้อปปิ้งให้ทีม: หมวดไหนจับคู่ไม่ได้ = หอสมุดยังขาดตัวอย่างแนวนั้น
...
`;
    ...
    //   ตอนนี้ override ด้วยมุมเล่าเฉพาะของ call นี้
    if (focusAngle) {
    ...
    // ★ 4 ส.ค. 69 (ผลแซนด์บ็อกซ์ 10 ข่าว + ศึก 6 นักเขียน — แก้รากคำเฟ้อ "พื้นความยาวไม่สนเนื้อเข้า"):
    //   ข่าวบาง (ต้นฉบับ <500 ตัวอักษร) เคยพองถึง 3.1 เท่า → บังคับ 2 ย่อหน้า เพดาน 160 คำ
    //   default ปิด (เจ้าของสั่ง 14 ส.ค. คืนพฤติกรรมยุค 2 เดือน) — เปิดคืน: THIN_SOURCE_2PARA=1
    const _thinSrcLen = String(actualNewsBody || '').length;
    if (process.env.THIN_SOURCE_2PARA === '1' && _thinSrcLen > 0 && _thinSrcLen < 500) {
      lenCfg = { min: 60, max: 160, paragraphs: '2', paraDesc: '2 ย่อหน้า', sentences: '2-4' };
      console.log(`[Analyze-Service] 📏 ข่าวบาง (ต้นฉบับ ${_thinSrcLen} ตัวอักษร) → บังคับ 2 ย่อหน้า / เพดาน 160 คำ`);
    }
...
`;
    ...
    let multiPrompt = prompt + '\n\n=== คำสั่งสำคัญสำหรับการเขียน ===\n' +
      ...
      `แต่ละเวอร์ชัน:\n` +
      `- ${process.env.LENGTH_BY_CONTENT !== '1' ? `ความยาวบังคับ ${lenCfg.min}-${lenCfg.max} คำ` : `ความยาว: เล่าให้ครบทุกประเด็นสำคัญจากต้นฉบับเป็นหลัก สูงสุดไม่เกิน ${lenCfg.max} คำ — เนื้อน้อยเขียนสั้นได้ ห้ามยืดความ/เติมสิ่งที่ต้นฉบับไม่มีเพื่อให้ยาวขึ้น`} ทุกประโยคต้องให้ข้อมูลใหม่ แบ่ง ${lenCfg.paraDesc} ตามนี้เท่านั้น ห้ามเกิน — ประโยคปิดต้องอยู่ท้ายย่อหน้าสุดท้าย ห้ามแยกออกเป็นย่อหน้าใหม่เด็ดขาด\n` +
      ...
      `- โครงสร้าง ${lenCfg.paragraphs} ย่อหน้า: [ย่อหน้า 1] เปิดแรง hook ดึงอารมณ์ [ย่อหน้าตรงกลาง] เล่ารายละเอียด storytelling [ย่อหน้าสุดท้าย] ปิดท้ายด้วยประโยคบรรยายเรียบๆ ที่บอกเล่าความจริงโดยไม่ตีความหมายเพิ่มเติม — จำนวนย่อหน้าต้องตรงตามที่กำหนด ห้ามเกิน ประโยคปิดต้องอยู่ท้ายย่อหน้าสุดท้าย ห้ามแยกออกมาเป็นย่อหน้าใหม่เด็ดขาด\n` +
      ...
      '{\n' +
      '  "versions": [\n' +
      `    {"style": "ชื่อแนว", "title": "พาดหัว", "content": "${process.env.LENGTH_BY_CONTENT !== '1' ? `เนื้อหายาว ${lenCfg.min}-${lenCfg.max} คำ` : `เนื้อหาครบทุกประเด็นสำคัญ ไม่เกิน ${lenCfg.max} คำ`} แบ่ง ${lenCfg.paraDesc} คั่นด้วย \\n\\n", "hook": "ประโยคเปิด", "closing": "ประโยคปิดกระตุ้น", "tone": "โทนเสียง", "target": "กลุ่มเป้าหมาย"}\n` +
      '  ],\n' +
          ...
          data: {
            usedPreset: promptSource?.startsWith('library')
              ? {
                  ...
                  isBorrowed: smartPrompt._isBorrowed || false,
                  borrowReason: smartPrompt._borrowReason || null,
                  // ★ 30 มิ.ย.: ฟิลด์ตรวจสอบย้อนหลัง (เพิ่มอย่างเดียว ไม่แตะตรรกะแมท/เจน) — บอกว่าใช้พร้อมท์ตัวไหน/ใกล้แค่ไหน
...
5. BRIDGES — ประโยคเชื่อมระหว่างประเด็น (3-5 ประโยค)
   ...
   เช่น: "แล้วเรื่องที่ไม่ค่อยมีใครรู้คือ..." / "ย้อนกลับไปตอนนั้น..."
  ...
  if (mode === 'mix') {
    ...
    try {
      ...
      const mixPrompt = fullCtx + researchCtx + factPoolCtx + smartPromptCtx + '\n\n' +
        ...
        `แต่ละเวอร์ชัน:\n` +
        `- ${process.env.LENGTH_BY_CONTENT !== '1' ? `ต้องยาวอย่างน้อย ${lenCfg.min} คำ ถึง ${lenCfg.max} คำ` : `ความยาวตามเนื้อจริง สูงสุดไม่เกิน ${lenCfg.max} คำ — ครบทุกประเด็นสำคัญ ห้ามยืดความ`} / ${lenCfg.paraDesc}\n` +
...
ส่งคืนผลลัพธ์เป็น JSON ล้วนๆ ห้ามมี Markdown ตามโครงสร้างนี้:
{
  ...
  try {
    const prompt = preset.prompt
      ...
      .replace('{custom_instruction}', customPrompt ? `คำสั่งเพิ่มเติม: "${customPrompt}"` : '');
    // ★ 1 ส.ค. 69 (Sol รอบ 2 — parity legacy สาย URL): แนบกติกาห้ามยัดข้อคิด เว้นแต่เปิด FORCE_LESSON_ANGLE=1
...
ตอบเป็น JSON เท่านั้นในรูปแบบนี้:
{
  ...
  // ★ DNA v3.3: ใช้สมองจับคู่กลาง promptMatcher.js (สูตรเดียวกับ STAGE 2 + ขยายแท็กอารมณ์มุมบวก)
  // เส้นทางนี้คง mismatchPenalty -50 ไว้ตามพฤติกรรมเดิม (เลือก per-angle ต้องเข้มกว่า)
  const { scoreLibraryPrompts } = await import('@/lib/services/promptMatcher');
...
ตอบ JSON: { "picks": [เลข 14 ตัว] }`;
      ...
      let _catPick;
      try {
        // ★ 15 ส.ค. 69 (ขั้น 4 sonnet-5 — เจ้าของอนุมัติ · แฝดกับ summarizeServiceText.js ต้องแก้คู่กันเสมอ):
        //   โมเดลขึ้นต้น claude- → callClaude คิดเบา + แคชสารบัญ · ไม่ตั้ง env = luna เส้นเดิมทุกไบต์
        // ★ 15 ส.ค. 69 ดึก (เจ้าของสั่ง "เปิดใช้ sonnet5"): default → claude-sonnet-5 · ถอยกลับ: CARD_PICKER_MODEL=gpt-5.6-luna
        //   ของเดิม: const _pickerModelA = process.env.CARD_PICKER_MODEL || MODEL_FAST; // gpt-5.6-luna
        const _pickerModelA = process.env.CARD_PICKER_MODEL || MODEL_FAST; // ★ 15 ส.ค. เย็น เจ้าของสั่งถอย: sonnet-5 ทำข่าวบิดเบือนเยอะ → กลับ luna (เปิด sonnet-5 คืน: CARD_PICKER_MODEL=claude-sonnet-5)
...
ตอบ JSON: { "picks": [เลข 14 ตัว] }`;
  ...
  if (process.env.CARD_PICKER_AI !== '0' && topPrompts.length > 1) {
    ...
    // ★ 15 ส.ค. 69 ดึก (เจ้าของสั่ง "เปิดใช้ sonnet5"): default → claude-sonnet-5 (ของเดิม: || MODEL_FAST = luna)
    const _pickerModelB = process.env.CARD_PICKER_MODEL_B || process.env.CARD_PICKER_MODEL || MODEL_FAST; // ★ 15 ส.ค. เย็น เจ้าของสั่งถอยกลับ luna
    try {
...
ตอบเป็น JSON: { "selectedIndex": <1-${_aiCands.length}>, "reason": "..." }`;
      ...
      // ★ 15 ส.ค. 69 (ขั้น 4 sonnet-5 — เจ้าของอนุมัติ · แฝดกับ summarizeServiceText.js ต้องแก้คู่กันเสมอ):
      //   ซ่อมบั๊ก race ไม่ตัดสาย HTTP จริง (จ่ายเงินฟรีหลัง timeout)
