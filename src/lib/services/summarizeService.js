...
export async function performSummarize({
  ...
  if (process.env.VIRAL_HITS_FORMULA === '1' && (contentLength === 'medium' || contentLength === 'long')) {
...
`;
    ...
    //   ปิดได้: THIN_SOURCE_2PARA=0 (กลับไปใช้ lenCfg เดิม)
    const _thinSrcLen = String(actualNewsBody || '').length;
    if (process.env.THIN_SOURCE_2PARA !== '0' && _thinSrcLen > 0 && _thinSrcLen < 500) {
...
`;
    ...
    let multiPrompt = prompt + '\n\n=== คำสั่งสำคัญสำหรับการเขียน ===\n' +
      ...
      `แต่ละเวอร์ชัน:\n` +
      `- ${process.env.LENGTH_BY_CONTENT === '0' ? `ความยาวบังคับ ${lenCfg.min}-${lenCfg.max} คำ` : `ความยาว: เล่าให้ครบทุกประเด็นสำคัญจากต้นฉบับเป็นหลัก สูงสุดไม่เกิน ${lenCfg.max} คำ — เนื้อน้อยเขียนสั้นได้ ห้ามยืดความ/เติมสิ่งที่ต้นฉบับไม่มีเพื่อให้ยาวขึ้น`} ทุกประโยคต้องให้ข้อมูลใหม่ แบ่ง ${lenCfg.paraDesc} ตามนี้เท่านั้น ห้ามเกิน — ประโยคปิดต้องอยู่ท้ายย่อหน้าสุดท้าย ห้ามแยกออกเป็นย่อหน้าใหม่เด็ดขาด\n` +
      '- ★ แต่ละประเด็นเล่าได้ครั้งเดียว ห้ามเล่าประเด็นเดิมซ้ำด้วยสำนวนอื่นเพื่อถ่วงความยาว — เนื้อหมดให้จบเลย (สำนวนแต่ง/ภาพเปรียบใส่ได้เต็มที่ แต่ครั้งเดียวต่อประเด็น)\n' +
      ...
      '{\n' +
      '  "versions": [\n' +
      `    {"style": "ชื่อแนว", "title": "พาดหัว", "content": "${process.env.LENGTH_BY_CONTENT === '0' ? `เนื้อหายาว ${lenCfg.min}-${lenCfg.max} คำ` : `เนื้อหาครบทุกประเด็นสำคัญ ไม่เกิน ${lenCfg.max} คำ`} แบ่ง ${lenCfg.paraDesc} คั่นด้วย \\n\\n", "hook": "ประโยคเปิด", "closing": "ประโยคปิดกระตุ้น", "tone": "โทนเสียง", "target": "กลุ่มเป้าหมาย"}\n` +
      '  ],\n' +
  ...
  if (mode === 'mix') {
    ...
    try {
      ...
      const mixPrompt = fullCtx + researchCtx + factPoolCtx + smartPromptCtx + '\n\n' +
        ...
        `แต่ละเวอร์ชัน:\n` +
        `- ${process.env.LENGTH_BY_CONTENT === '0' ? `ต้องยาวอย่างน้อย ${lenCfg.min} คำ ถึง ${lenCfg.max} คำ` : `ความยาวตามเนื้อจริง สูงสุดไม่เกิน ${lenCfg.max} คำ — ครบทุกประเด็นสำคัญ ห้ามยืดความ`} / ${lenCfg.paraDesc}\n` +
        '- ★ แต่ละประเด็นเล่าได้ครั้งเดียว ห้ามเล่าประเด็นเดิมซ้ำด้วยสำนวนอื่นเพื่อถ่วงความยาว — เนื้อหมดให้จบเลย\n' +
