...
export async function performSummarize({
  ...
  if (mode === 'breakdown') {
    ...
    try {
      // ★ primary → fallback ให้เหมือนฝั่ง URL (summarizeService.js) — กัน breakdown พังทั้ง pipeline
      let result;
      try {
        result = await callAI({ prompt, model: MODEL_NEWS_ANALYSIS, temperature: 0.4, maxTokens: 8000 });
      } catch (primaryErr) {
        console.warn(`[Breakdown-Service] ⚠️ ${MODEL_NEWS_ANALYSIS} failed: "${primaryErr.message}" — retrying with ${MODEL_HEAVY_FALLBACK} fallback...`);
