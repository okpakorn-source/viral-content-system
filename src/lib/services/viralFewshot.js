...
const VIRAL_STYLE_PACK_HITS =
  ...
  // ★ 16 ส.ค. 69 (วิธี D — เจ้าของสั่งทดลอง): กฎเดิมบังคับ "เอาตัวเลขขึ้นเป็นคำแรก" → ภาษาฝืน
  //   หลักฐานหักล้างกฎเดิม: โพสต์ไวรัลจริงในหอสมุด 71 ใบ **เปิดด้วยตัวเลขเป็นคำแรกแค่ 1 ใบ (1%)**
  //   ของจริงเปิดด้วย "คน" แล้ววางตัวเลขกลางประโยค เช่น "พี่ป๋อ กับ พี่เอ๋ อยู่กินกันมา 18 ปี..."
  //   เปิดกฎเดิมกลับ: HOOK_STYLE_MODE ไม่ใช่ D
  (process.env.HOOK_STYLE_MODE !== 'off'
    ? '1. เปิดโพสต์ด้วย "คนหรือการกระทำจริง" แล้ววางตัวเลขไว้กลางประโยค — นี่คือท่าเปิดของโพสต์แสนไลก์ตัวจริงในหอสมุด (71 ใบ เปิดด้วยตัวเลขเป็นคำแรกแค่ 1 ใบ) รูปประโยคแบบ "พี่ป๋อ กับ พี่เอ๋ อยู่กินกันมา 18 ปี..." / "พวกเรารู้กันดีว่าพี่กุ้งรักษาน้องวิไปเกิน 10 ล้าน..." 🔴 ห้ามขึ้นต้นประโยคด้วยตัวเลขหรือหน่วยวัดลอยๆ ("เกรด 4.00 มาตลอด แต่..." / "ยอดขายเดือนละ 30 ล้าน คือ...") เพราะไม่ใช่ภาษาที่คนไทยพูดจริง — ให้บอกก่อนว่าใคร แล้วค่อยตามด้วยตัวเลข · ตัวเลขต้องมีจริงในข่าวนี้เท่านั้น ห้ามแต่ง\n'
...
export async function getViralFewshotBlock({ category = '', emotionalTags = [], archetype = '', newsTitle = '', newsBrief = null, noHistory = false } = {}) {
  ...
  // ★ 16 ส.ค. 69 (เจ้าของสั่งทดลอง): ปิดสูตรบังคับทั้ง 2 ชุด — เหลือแค่ "ตัวอย่างโพสต์ไวรัลจริง" เป็นไกด์
  //   เหตุผลเจ้าของ: "ให้นักเขียนใช้การ์ดที่จับคู่กับหอสมุดไวรัลเป็นไกด์อยู่แล้ว
  //   นักเขียนจะได้ใช้ศักยภาพตัวเองในการเขียน ไม่ใช่โดนแพทเทิร์นเดิมๆ บังคับจนข่าวเสีย"
  //   หลักฐานที่นำมาสู่การทดลองนี้ (16 ส.ค. เทส 2 ข่าว × 2 สูตร):
  //     สูตร v2 บังคับ "เปิดด้วยตัวเลขขัดแย้ง" → ได้ประโยคฝืนภาษา "เกรด 4.00 อยู่ในมือมาตลอด"
  //     สูตร v1 เปิดเรื่องเป็นธรรมชาติกว่า แต่พาสำนวนบอกความรู้สึกแทนคนอ่านกลับมา (ใจฟู/ขอนับถือใจ) 3-4 จุด/ชุด
  //   เปิดสูตรกลับ: VIRAL_STYLE_PACK=1 (ค่าเริ่มต้น = เปิด · ตั้ง 0 = ไม่ส่งสูตรบังคับเลย)
