...
// ★ 20 ส.ค. 69 (R3 ข้อ 1 — เจ้าของเคาะเอง): วลีลายเซ็นข้อ 3
//   หลักฐาน: "ขอนับถือใจ" = 0/31 โพสต์แสนไลก์ · 0/221 โพสต์ไวรัล · 0.8% โพสต์ไม่ปัง — แต่ระบบเจนใช้ 45%
//   "ไม่แปลกใจเลยที่" = แชมป์ใช้จริง 3/31 · "ใครจะคิดว่า" = 2/31 ⇒ เก็บ 2 ตัวนี้ไว้ และเลิกบังคับให้มีทุกโพสต์
//   ถอย STYLE_PACK_V2=0 → บรรทัดข้อ 3 กลับเป็นไบต์เดิมเป๊ะ
const STYLE_PACK_RULE3_OLD =
  '3. วลีลายเซ็นชวนแชร์ 1-2 จุดต่อโพสต์ (เลือกที่เข้ากับเรื่อง): "ไม่แปลกใจเลยที่...", "ขอนับถือใจ...", "ดีใจแทน...", "ใครจะคิดว่า..."';
const STYLE_PACK_RULE3_V2 =
  '3. วลีลายเซ็นชวนแชร์: ใช้เมื่อเข้ากับเรื่องเท่านั้น ไม่ต้องมีทุกโพสต์ — ถ้าใช้ เลือกจาก "ไม่แปลกใจเลยที่..." หรือ "ใครจะคิดว่า..."';

// ★ 20 ส.ค. 69 (R3 ข้อ 2 — เจ้าของเคาะ "เอา ก ก่อน" = ฝั่งสัจธรรม)
//   ENDING_MODE=truth (ค่าตั้งต้น) = ข้อ 5 เดิม + หางกันความเสี่ยง "คำคมลอย"
//   ENDING_MODE=plain          = ปิดข้อ 5 ทิ้งทั้งบรรทัด (ใช้กลไกเดียวกับ CARD_AUTH_R8 แล้วไล่เลขข้อใหม่)
const STYLE_PACK_RULE5_OLD =
  '5. จบด้วยประโยคสัจธรรมสั้นๆ ที่คนอยากก๊อปไปโพสต์ต่อ ("ไม่มีวันไหนยากไปกว่าวันที่...", "คนกตัญญูไม่มีวันล้มจม") — ห้ามจบด้วยคำถาม';
const STYLE_PACK_RULE5_TRUTH =
  `${STYLE_PACK_RULE5_OLD} — สัจธรรมต้องสรุปจากข้อเท็จจริงในเรื่องนี้เท่านั้น ห้ามคำคมลอยที่ไม่ผูกกับเรื่อง`;

function getViralStylePack() {
  const removeHookRule = isCardAuthorityR7Enabled();
  const endingPlain = isEndingPlain();
  const removeClosingRule = isCardAuthorityR8Enabled() || endingPlain;

  let stylePack = VIRAL_STYLE_PACK;
  if (isStylePackV2Enabled()) stylePack = stylePack.replace(STYLE_PACK_RULE3_OLD, STYLE_PACK_RULE3_V2);
  if (!endingPlain) stylePack = stylePack.replace(STYLE_PACK_RULE5_OLD, STYLE_PACK_RULE5_TRUTH);

  // Keep the legacy prompt byte-for-byte intact when both card-authority switches are off.
  if (!removeHookRule && !removeClosingRule) return stylePack;
...
export async function getViralFewshotBlock({ category = '', emotionalTags = [], archetype = '', newsTitle = '', newsBrief = null, noHistory = false, teacherGuideEligible = false, cardEssence = '', conflictTags = [], humanAngles = [] } = {}) {
  ...
  try {
    ...
    if (mode === 'ai' || mode === 'score') {
      ...
      const mKey = newsTitle ? `${String(newsTitle).slice(0, 80)}|${libCat ?? ''}|${mode}` : ''; // 🗂️ V2 คืน null → คีย์ว่าง ไม่ใช่ข้อความ "null" (สวิตช์ปิด libCat ไม่มีทางเป็น null)
    ...
    if (!picks.length) {
      ...
      crossCat = (shortlistOn && !pool.length) || noShelf; // 🗂️ noShelf: หัวบล็อก/log ห้ามประกาศหมวดที่ไม่มี
