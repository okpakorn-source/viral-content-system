...
export function currentInsightPromptRev() {
  ...
  return swOn('CLIP_FLOW') ? `${withAlive}-flow1` : withAlive; // ★ 10 ส.ค. เฟส B: จัดรูปย่อหน้า
}
...
function buildVideoInsightPrompt(opts = {}) {
  ...
  return applyGrayAlive(applySmartOrder(composed)); // ปิดสวิตช์ = คืนสตริงเดิมทุกตัวอักษร
}
...
export async function extractClipInsight({ url, platform, rawText = '', sourceMeta = null }) {
  ...
  const prompt = applyGrayAlive(applySmartOrder(`คุณเป็นบรรณาธิการข่าว อ่าน "บทถอดเสียงจากคลิป" ด้านล่าง แล้วถอดประเด็นข่าวออกมาเป็น "ข้อมูลดิบ"
