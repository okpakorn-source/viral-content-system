...
- ถ้าคลิปนี้คุย "หลายประเด็นใหญ่ที่แยกเป็นข่าวเดี่ยวได้" (รายการ/สัมภาษณ์/ทอล์ก/หลายเรื่องในคลิปเดียว):
  → เลือกเฉพาะประเด็น "ที่ดีพอเป็นข่าวได้จริง" แล้วทำเนื้อดิบ "เจาะลึกแยกทีละประเด็น"
  ...
  → จำนวน "ยืดหยุ่นตามคลิป" (ปกติ 2-6 อัน) — มีประเด็นดีจริงกี่อันใส่ตามนั้น ไม่ต้องฝืนให้ครบ ไม่จำกัดเพดาน
  → ข้ามช่วงที่ไม่ใช่ประเด็นข่าว (โฆษณา/สปอนเซอร์/พาชมเฉยๆ ที่ไม่มีแก่นข่าว)
- ⛔ ถ้าคลิปเป็น "เรื่องเดียวต่อเนื่องทั้งคลิป" → subStories = [] (เว้นว่าง ไม่ต้องแยก)`;
...
export async function extractClipInsight({ url, platform, rawText = '' }) {
  ...
  if (platform === 'youtube') {
    ...
    // ★ 21 มิ.ย.: 8000→16000 · ★ 25 มิ.ย.: 16000→24000 (เพิ่ม subStories เนื้อดิบแยกประเด็น output ยาวขึ้น)
    const r = await callGeminiVideo({ prompt: VIDEO_INSIGHT_PROMPT, youtubeUrl: url, maxTokens: 24000 });
...
export async function extractInsightFromVideoBuffer(videoBuffer, mimeType = 'video/mp4') {
  ...
  const r = await callGeminiVideoFile({ prompt: VIDEO_INSIGHT_PROMPT, videoBuffer, mimeType, maxTokens: 24000 });
  return normalizeInsight(r, 'gemini-video');
}
