...
const TRANSCRIPT_QUOTES_SCHEMA = `ตอบ JSON เท่านั้น (ห้าม markdown) — เรียงลำดับฟิลด์ตามนี้เป๊ะ (enrichedRaw = ตัวหลัก ต้องเขียนก่อน transcript เสมอ):
{
  ...
  "enrichedRaw": "★★ ตัวหลัก (เขียนช่องนี้ก่อน) — เนื้อดิบมีมิติ: เล่าข่าวนี้ให้ 'ครบทั้งเรื่อง ครอบคลุมทุกประเด็น' ที่คลิปพูดถึง (ทั้งเรื่องหลักและประเด็นย่อยทุกอัน ไล่ตั้งแต่ต้นจนจบ ห้ามตกประเด็น) เป็นหลายย่อหน้าอ่านลื่น โดยถักทอข้อเท็จจริงเข้ากับ 'คำพูดจริง verbatim' ของคนในคลิป — ต้องมีคำพูดจริงในเครื่องหมายคำพูดแทรกอยู่ในเนื้อ 'ทุกช่วงสำคัญ' (คลิปหลายประเด็นปกติ 5-10 จุด กระจายทุกประเด็น) ในจังหวะที่คำพูดเสริมพลังเรื่อง (เช่น ...เจ้าตัวเปิดใจว่า \\"...\\" ...) — ไม่ใช่เล่าใจความล้วน ไม่ใช่แปะคำพูดกองท้าย · ลึกและครบพอหยิบไปเขียนข่าวได้ทันทีโดยไม่ต้องดูคลิป · คำพูดตรงคำจากคลิปจริง ห้ามเรียบเรียงใหม่",
...
ขั้น 5 — เขียน "เนื้อดิบมีมิติ" (enrichedRaw = ตัวหลัก เขียนช่องนี้ก่อนช่องอื่น): เล่าเรื่องข่าวให้ 🔴 **ครบทั้งคลิป ครอบคลุมทั้งเรื่องหลักและ "ทุกประเด็นย่อย"** ตั้งแต่ต้นจนจบ (ห้ามตกประเด็น) เป็นหลายย่อหน้าอ่านลื่น มีมิติ คุณภาพสูง โดย 🔴 **ต้องยก "คำพูดจริง" (verbatim ในเครื่องหมายคำพูด) มาแทรกในเนื้อทุกช่วงสำคัญ — คลิปหลายประเด็นปกติ 5-10 จุด กระจายให้ครบทุกประเด็น** ในจังหวะที่คำพูดทำให้เรื่องมีพลัง/เห็นภาพ/สะเทือนใจ (เช่น ...เจ้าตัวเปิดใจว่า "…" ก่อนจะ...) — ⛔ ไม่ใช่เล่าใจความล้วน (ต้อง "เห็นคำพูดจริง" ในเนื้อ) ไม่ใช่แปะกองท้าย · ลึกและครบพอหยิบไปเขียนข่าวได้ทันที · คำพูดตรงคำจากคลิป ห้ามเรียบเรียงใหม่
...
export async function extractTranscriptQuotes({ url }) {
  const { callGeminiVideo } = await import('@/lib/ai/geminiClient');
  const r = await callGeminiVideo({ prompt: TRANSCRIPT_QUOTES_PROMPT, youtubeUrl: url, maxTokens: 32000 });
...
export async function extractTranscriptQuotesFromVideoBuffer(videoBuffer, mimeType = 'video/mp4') {
  const { callGeminiVideoFile } = await import('@/lib/ai/geminiClient');
  const r = await callGeminiVideoFile({ prompt: TRANSCRIPT_QUOTES_PROMPT, videoBuffer, mimeType, maxTokens: 32000 });
  return normalizeTranscriptQuotes(r);
}
