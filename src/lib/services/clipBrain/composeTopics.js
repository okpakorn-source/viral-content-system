...
ความครอบคลุม (งานจริง 14 ก.ย.: เรื่องน้อยกว่าประเด็น → ระบบตีตก "ของหาย-ประเด็น"): หลักฐานชนิด timeline (id ขึ้นต้น l) คือประเด็นที่ตาเห็นทั้งคลิป — ทุกประเด็น timeline ที่มี transcript/substory/quote/keypoint รองรับ ต้องถูกเล่าอย่างน้อยในเรื่องหนึ่ง (เป็นเรื่องของตัวเอง หรือแทรกเป็นตอนในเรื่องที่เกี่ยว) ห้ามทิ้งเพราะเนื้อสั้น; ช่วงที่หลักฐานมี "(...)" หรือถูกตัดทอน ให้เล่าเท่าที่หลักฐานมี ไม่ข้ามทั้งประเด็น
ยกเว้นช่วงโปรโมตของรายการ (ตัวอย่างช่วงต่อไป ไฮไลท์เปิด/ปิดรายการ โฆษณา คำชวนติดตาม): ไม่ใช่เนื้อข่าว ห้ามแต่งเป็นเรื่องหรือแทรกในเรื่องใด แม้ transcript จะพูดถึง (เจ้าของสั่ง 14 ก.ย.)
...
ห้ามใส่ชื่อบุคคลจากความรู้ทั่วไปหรือความจำของโมเดล แม้จะจำหน้า/จำรายการได้ (พิธีกรรายการ ผู้ประกาศ ดารา แขกรับเชิญ) — ถ้าหลักฐานไม่เอ่ยชื่อ ให้ใช้คำแทนบทบาท เช่น "พิธีกร" "พิธีกรหญิง" "ผู้ประกาศ" (งานจริง 14 ก.ย.: ชื่อพิธิกร 3 คนจากความรู้นอกคลิปถูกระบบตีตก "ของงอก-ชื่อ" ทั้งหมด)
...
- ห้ามคมกว่าหลักฐาน (วงตรวจ 14 ก.ย.: จุดพลาดที่พบบ่อยสุด): (ก) ห้ามใส่คำประเมิน/คุณศัพท์ที่หลักฐานไม่ได้ใช้ เช่น มุ่งมั่น ทุ่มเท คุณภาพดี ป่วยหนัก อย่างหนัก ยาวนาน (ข) ห้ามตัดคำกำกวมของหลักฐาน: "ประมาณ 13 ปี" ต้องคง "ประมาณ" · "คนแรกๆ" ห้ามกลายเป็น "คนแรก" · "วันละประมาณ 60 ก้อน" คง "ประมาณ" (ค) ห้ามเติมรายละเอียดจำเพาะที่หลักฐานไม่ระบุ: ชนิดของสิ่งของ ("กีตาร์" ≠ "กีตาร์ไฟฟ้า") สถานที่ ("ไปตรวจ" ≠ "ไปตรวจที่โรงพยาบาล") ผู้กระทำ ("ตรวจพบ" ≠ "แพทย์ตรวจพบ") ลำดับเวลา ("เวลาต่อมา") และเหตุ-ผล ("การฝึกอย่างหนักช่วยให้…") — หลักฐานบอกเท่าไหร่ เขียนเท่านั้น
...
${list(evidencePack?.clipMeta?.promoSkipped).length ? `ช่วงโปรโมตของรายการที่ถูกตัดออกจากหลักฐานแล้ว (ห้ามเขียนถึง): ${list(evidencePack.clipMeta.promoSkipped).map((p) => `${text(p?.time)} ${text(p?.topic)}`.trim()).join(' · ')}\n` : ''}คำกำกับเพิ่มเติม (เปลี่ยนเฉพาะสำนวน ไม่ลดเกณฑ์): ${JSON.stringify({ audience: text(spec?.audience), style: text(spec?.style) })}
