...
หากไม่มีเนื้อหาเพิ่มเติม ให้ผลลัพธ์เป็น: newsTitle="คลิป TikTok (${domain})", newsBody="ไม่สามารถดึงเนื้อหาอัตโนมัติได้ กรุณาวางลิงก์แบบเต็มหรือพิมพ์เนื้อหาเอง"`.trim();
  ...
  const generationTasks = anglesToUse.map((angleObj, index) => {
    return withTimeout((async () => {
      ...
      //   เคสจริง: การ์ดสั่ง "เข้าเรื่องทันทีด้วยตัวเลขผูกประโยค" → นักเขียนได้ "เกรด 4.00 อยู่ในมือมาตลอด"
      //   (86 จาก 201 ใบในคลัง = 43% มีคำสั่งนี้ · โผล่ซ้ำทุกรอบไม่ว่าจะปิดสูตร v1/v2 หรือไม่)
      //   HOOK_STYLE_MODE: off(ค่าเริ่มต้น = เดิมเป๊ะ) · A · B · C
      //     A = เติมคำกำกับ "ถ้าเอาตัวเลขขึ้นต้นแล้วภาษาฝืน ให้ใช้ภาพการกระทำแทน" (แก้จุดเดียว ครอบทุกใบ)
      //     B = เขียนถ้อยคำ hookStyle ใหม่ในหน่วยความจำ (เท่ากับแก้การ์ด 86 ใบ แต่ไม่แตะไฟล์คลังจริง = ย้อนได้ทันที)
      //     C = ลดน้ำหนักจาก "บังคับ" เป็น "แนวทาง" (ให้นักเขียนเลือกเอง)
      const _hookMode = String(process.env.HOOK_STYLE_MODE || 'on').trim().toUpperCase();
      const _NUM_HOOK_RE = /ตัวเลขผูกประโยค/;
      let _hookText = _promptHook;
      if (_hookText && _hookMode !== 'OFF' && _NUM_HOOK_RE.test(_hookText)) {
        // B: แทนคำสั่งที่บีบให้เอาตัวเลขเป็นประธานประโยค ด้วยคำสั่งที่เปิดทางให้ภาษาเป็นธรรมชาติ
        _hookText = _hookText.replace(_NUM_HOOK_RE, 'ตัวเลขที่วางไว้ในประโยคอย่างเป็นธรรมชาติ (ห้ามยกตัวเลขขึ้นเป็นประธานจนภาษาฝืน)');
      }
      const _guardA = (_hookMode === 'A' && _hookText && _NUM_HOOK_RE.test(_hookText))
        ? ' (⚠️ ถ้าเอาตัวเลขขึ้นต้นประโยคแล้วภาษาฝืน ไม่เป็นภาษาที่คนไทยพูดจริง — ให้เปลี่ยนไปเปิดด้วยภาพการกระทำจริงแทนทันที ความเป็นธรรมชาติของภาษาสำคัญกว่าการเอาตัวเลขขึ้นก่อน)'
        : '';
