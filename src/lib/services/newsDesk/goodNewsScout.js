...
const CELEB_HIGHLIGHT_BROAD = [
  ...
  // ★ 3 ก.ค. (ผู้ใช้ชี้ตัวอย่าง: คลิปแฉ "กว่าจะมีลูก พยายาม 2 ปี"): เส้นทางมีลูกยาก-สมหวัง = เปิดใจที่คนอิน
  'ดารา เปิดใจ กว่าจะมีลูก ซึ้ง', 'ดารา มีบุตรยาก พยายามมีลูก สัมภาษณ์',
  // ★ 3 ก.ค. (ผู้ใช้ชี้ตัวอย่าง: แฉ พีท ทองเจือ): พ่อแม่ดาราต้นแบบ — สัมภาษณ์แนวคิดเลี้ยงลูก (มีแก่น ไม่ใช่ไลฟ์สไตล์ลอยๆ)
  'พ่อแม่ดารา เปิดใจ เลี้ยงลูก สนับสนุนความฝัน', 'ดารา สอนลูก แนวคิดเลี้ยงลูก น่าชื่นชม',
];
...
// ════════════════════════════════════════════════════════════════════
// ★★★ 4 ก.ค. (ผู้ใช้สั่ง — วิเคราะห์ CSV เพจซ้ำ): "น้ำดีลึกทุกวงการ" แนวปัญญาปันสุข
//   ปัญหา: โต๊ะมี human-interest ลึกแค่ 4% (คำค้นเดิมยิงดาราบันเทิง ~50 คำ vs ชาวบ้าน 2 คำ)
//   CSV เพจพิสูจน์: เด็กลำบาก-เก่ง-กตัญญู 497 โพสต์ · คนแก่สู้ชีวิต 858 · พลเมืองดี 265 · เครื่องแบบ/พระ med 11k
//   → คลังนี้ = เรื่อง "คนจริง" ทุกวงการ ที่กินใจ (เด็กลำบาก/เก่ง/ได้รับการช่วยเหลือ, คนแก่สู้ชีวิต,
//     พลเมืองดี, เครื่องแบบ/พระเสียสละ, กระแสโซเชียลน้ำดี) — ยิงเยอะทุกรอบ ให้แนวนี้ครองโต๊ะ
// ════════════════════════════════════════════════════════════════════
const DEEP_GOOD_POOL = [
  // 👶 เด็กลำบาก + เก่ง/กตัญญู (แนวแชมป์ 497 โพสต์)
  'เด็กยากจน เรียนเก่ง สอบติด ไวรัล', 'นักเรียนยากจน เกรดดี ทุนการศึกษา', 'เด็กกำพร้า อยู่กับยาย เรียนดี',
  'เด็กทำงานส่งตัวเองเรียน สู้ชีวิต', 'เด็กเก็บขยะ ขายของ ช่วยครอบครัว', 'นักเรียนกตัญญู ดูแลพ่อแม่ป่วย',
  'เด็กบ้านนอก สอบติดหมอ วิศวะ', 'เณรน้อย ยอดกตัญญู เรื่องราวซึ้ง', 'เด็กเดินเท้าไกลไปเรียน มุ่งมั่น',
  'ลูกชาวนา เรียนจบ ป้ายแดง ครอบครัวภูมิใจ',
  // 🤲 เด็ก/คนลำบาก "ได้รับการช่วยเหลือ" (ปัญญาปันสุขแท้)
  'ปัญญาปันสุข ช่วยเหลือ เด็ก', 'ช่วยเหลือเด็กยากไร้ มอบทุน สร้างบ้าน', 'พลังน้ำใจ ช่วยครอบครัวยากจน',
  'ระดมทุน ช่วยเด็กป่วย ผ่าตัด', 'มอบบ้านหลังใหม่ ครอบครัวยากจน ไวรัล', 'สังคมยื่นมือช่วย ครอบครัวลำบาก',
  // 🧓 คนแก่/ผู้สูงวัย สู้ชีวิต (858 โพสต์)
  'ตายาย เก็บขยะ เลี้ยงหลาน สู้ชีวิต', 'ยายวัย 80 ขายของ หาเลี้ยงครอบครัว', 'คุณตา สู้ชีวิต อาชีพน่าทึ่ง ไวรัล',
  'ผู้สูงวัย ไม่ยอมแพ้ หาเลี้ยงชีพ ซึ้ง', 'ลุงป้า สู้ชีวิต น่าเคารพ ชาวเน็ตชื่นชม',
  // 🤝 พลเมืองดี/น้ำใจ (265 โพสต์)
  'พลเมืองดี ช่วยชีวิต ไวรัล ล่าสุด', 'เก็บเงินได้ คืนเจ้าของ ซื่อสัตย์', 'วินมอเตอร์ไซค์ แท็กซี่ คืนของ น้ำใจ',
  'ชาวบ้าน รวมน้ำใจ ช่วยเพื่อนบ้าน', 'คนไทยไม่ทิ้งกัน ช่วยเหลือ ไวรัล', 'ช่วยคนตกทุกข์ กลางถนน ชาวเน็ตชื่นชม',
  // 💪 สู้ชีวิต/พลิกชีวิต ชาวบ้าน (178 โพสต์)
  'คนพิการ สู้ชีวิต ไม่ยอมแพ้ ไวรัล', 'จากยากจน พลิกชีวิต สำเร็จ เรื่องจริง', 'แม่เลี้ยงเดี่ยว สู้ชีวิต เพื่อลูก',
  'หนุ่มสาว ลาออก ทำตามฝัน สำเร็จ', 'ล้มแล้วลุก ตั้งตัวใหม่ จากศูนย์',
  // 🎖️ เครื่องแบบ/พระ เสียสละ (med 11k สูงสุด)
  'ตำรวจ ทหาร น้ำใจ ช่วยประชาชน ไวรัล', 'กู้ภัย เสียสละ ช่วยชีวิต น่าเคารพ', 'ทหาร เรียบง่าย ติดดิน น่าเคารพ',
  'พระ หลวงพ่อ เมตตา ช่วยคนยาก ไวรัล', 'ครู เสียสละ ทุ่มเทเพื่อเด็ก โรงเรียนไกล', 'หมอ พยาบาล น้ำใจ ช่วยผู้ป่วยยากไร้',
  // 🔥 กระแสโซเชียลน้ำดี (เคยปังแล้วเป็นน้ำดี)
  'ชาวเน็ตแห่ชื่นชม เรื่องราวดีๆ ล่าสุด', 'คลิปน้ำใจ ประทับใจ แชร์สนั่น', 'เรื่องซึ้ง กินใจ สังคมยกย่อง ไวรัล',
  'ทำดีได้ดี เรื่องจริง ประทับใจ', 'ต่างชาติ น้ำใจ ช่วยคนไทย ซึ้ง',
];

/** ★★★ น้ำดีลึกทุกวงการ (4 ก.ค.) — เรื่องคนจริงกินใจแนวปัญญาปันสุข หมุนคลัง 45 แนว count คำ/รอบ */
export function generateDeepGoodQueries(count = 16) {
  const slot = Math.floor(Date.now() / 3600e3); // หมุนทุก ชม. → ครอบคลังใน ~3 รอบ
  const out = [];
  for (let i = 0; i < count; i++) out.push(DEEP_GOOD_POOL[(slot * count + i) % DEEP_GOOD_POOL.length]);
  return [...new Set(out)].map(q => ({ q, genre: 'น้ำดีลึก' }));
}
