...
const BANNED_PATTERNS = [
  ...
  // ★ 2 ก.ค. เพิ่ม คาสิโน|สล็อต — เทสจริงเจอสแปม "happyluke คาสิโนเงินจริง" หลุดประตู (กฎทีม: พนันไม่เอาทุกรูปแบบ)
  /การพนัน|เล่นพนัน|บ่อนพนัน|เว็บพนัน|พนันออนไลน์|บาคาร่า|แทงบอล|เปิดบ่อน|คาสิโน|สล็อต/,
...
const OFF_BRAND_PATTERNS = [
  ...
  // ★ 4 ก.ค. (ผู้ใช้ "เจอแต่บอลไม่เอา"): ตัดข่าวบอลล้วน (ผล/ย้ายทีม/นัดชิง/ลีก) — ไม่โดน "นักกีฬากตัญญู/สู้ชีวิต/น้ำใจ"
  //   จับเฉพาะสำนวนข่าวแมตช์/ตลาดซื้อขาย ที่ไม่มีคำน้ำดี (กตัญญู/ช่วยเหลือ/สู้ชีวิต/บริจาค = ปล่อยผ่านเสมอ)
  /(ปิดดีล|คว้าตัว|ย้ายซบ|เปิดตัวนักเตะ|ประเดิมสนาม|บุกเยือน|เปิดรัง|ดวลแข้ง|ประตูชัย|ยิงจุดโทษ|ซิวแชมป์|จ่าฝูง|บุนเดสลีกา|พรีเมียร์ลีก|ลาลีกา|เอฟเอคัพ|ยูฟ่า|เจลีก|ไทยลีก.{0,6}(นัด|เกม|ประตู))/,
...
const FIT_WEIGHTS = {
  ...
  // ★★ 2 ก.ค. (ลีกผลจริง มิ.ย. 949 โพสต์): กระแสรายวัน (ซากากระแสใหญ่ median 21k = แชมป์) เดิมตกไป default 6 — ยกขึ้น
  //   ความรัก/แต่งงาน (median 8.2k กลางตาราง) เดิม default 6 — ยกเป็นระดับกลาง
  'กระแสรายวัน': 14, 'ความรัก/แต่งงาน': 10,
};
...
- ★★★ storyIntensity (2 ก.ค. — เกณฑ์จากผลโพสต์จริง 949 โพสต์): "ความเข้มของสตอรี่" 0-3
  · 3 = มีทั้ง contrast สุดขั้ว + ตัวเลขรูปธรรม (เช่น "เกรด 4.00 แต่บ้านมีเงินวันละร้อย" / "วิ่ง 2 กม.ทุกเที่ยงเอาข้าวให้แม่รถเข็น" / "แม่ทัพเกษียณสู่พระกวาดลานวัด อุทิศทหาร 42 นาย")
...
// ★★★ 2 ก.ค. 69 — คลังทองชุดใหม่จากข้อมูลจริง: วิเคราะห์โพสต์เพจทั้งเดือน มิ.ย. 949 โพสต์ (Meta export)
//   ตัวเลขในวงเล็บ = รีแอกชันจริง · คัดครอบ 5 สูตรที่พิสูจน์แล้ว:
//   A ชาวบ้าน+contrastสุดขั้ว · B เกาะซากากระแสใหญ่ · C เครื่องแบบ/ผ้าเหลืองเสียสละ · D ดาราให้แบบมีตัวเลข+ผู้รับตัวจริง · E สมถะ/กตัญญู
//   บทเรียนเชิงโครง (วัดจาก 949 โพสต์): หัวเรื่องมีโครงหักมุม "แม้/ทั้งที่/แต่กลับ" median 11.2k vs 6.6k (+70%)
//   · มีตัวเลขรูปธรรม (เงิน/อายุ/ระยะทาง/เกรด) median 8.2k vs 4.4k (~2 เท่า)
const GOLD_EXAMPLES = [
  'เกรดเฉลี่ย 4.00 เต็ม แต่บ้านมีเงินวันละร้อยบาท น้องพลอยกำลังขึ้น ม.4 (243,401) — ชาวบ้านนิรนาม+contrastสุดขั้ว+ตัวเลข [สู้ชีวิต]',
  'พี่หนุ่ม กรรชัย มอบ 20,000 กลางรายการโหนกระแสให้พี่ขยัน หลังฟังเรื่องชีวิต (167,701) — คนดังให้เงิน+เกาะกระแสใหญ่ที่คนตามอยู่ [น้ำใจ/ช่วยเหลือ]',
  'บิ๊กกุ้ง วัย 60 จากอดีตแม่ทัพภาค 2 สู่พระกวาดลานวัด อุทิศแด่ทหารกล้า 42 นาย (164,362) — เครื่องแบบใหญ่สละสิ่งที่ใหญ่กว่า [สู้ชีวิต]',
  'เด็กหญิง 7 ขวบ วิ่งไปกลับ 2 กม.ทุกพักเที่ยง เอาข้าวครึ่งกล่องไปให้แม่นั่งรถเข็น (155,256) — เด็กกตัญญู+ตัวเลขเห็นภาพ [กตัญญู/ครอบครัวอบอุ่น]',
  'ก้อง ห้วยไร่ รับน้องพั้นช์มาดูแล เด็กไม่เคยเอ่ยปากขอเงินสักบาท (126,686) — ดาราอุปการะ+รายละเอียดนิสัยจริง [คนดังทำดี/ติดดิน]',
  'เงินทิปหน้าเวทีของปุ้ย L.กฮ. กลายเป็นเครื่องฟอกไต 450,000 บาท มอบให้ รพ.ทุ่งสง (123,857) — เส้นทางเงินชัด+ผู้รับตัวจริง [คนดังทำดี/ติดดิน]',
  'พี่ปอนด์กู้ภัยขอขมาแม่ ทำทุกทางแต่พา 2 คนติดเหมืองออกมาไม่ได้ มอบ 45,000 แทนกำลังใจ (119,563) — ฮีโร่ในซากากระแสใหญ่+อารมณ์จริง [น้ำใจ/ช่วยเหลือ]',
  'หมอโบว์ดูแลแม่ป่วยอัลไซเมอร์ 9 ปี หมดเงินเกือบ 10 ล้าน "เงินหาใหม่ได้แต่แม่มีคนเดียว" (109,311) — กตัญญู+ตัวเลข+คำพูดเจาะใจ [กตัญญู/ครอบครัวอบอุ่น]',
  'ยายติ๊กรับจ้างได้วันละร้อย เงินก้อนเล็กกลายเป็นค่าข้าวค่าเรียนของหลานสาว (102,987) — คนแก่สู้ชีวิต+contrast [สู้ชีวิต]',
  'เงินทิปแฟนเพลง อ้ายมนต์แคนไม่เก็บสักบาท ต่อเป็นบ้านทั้งหลังให้ผู้พิการทางสายตา (96,224) — ดาราลูกทุ่งให้แบบเป็นรูปธรรม [คนดังทำดี/ติดดิน]',
  'ต่าย อรทัย 24 ปีในวงการ ไม่ซื้อแบรนด์เนม อยู่บ้านไม้หลังเดิม 36 ปี เก็บเงินสร้างบ้านให้แม่กับยาย (66,270) — ดาราสมถะ+ตัวเลขปี [คนดังทำดี/ติดดิน]',
  'นักเรียนหญิง 3 คนพาลุงพิการข้ามถนน ย่อตัวไหว้รถที่จอดให้ (65,504 — เรื่องเดียวกันปังซ้ำ 5 โพสต์) — พลเมืองดีเด็ก ภาพเล่าเอง [น้ำใจ/ช่วยเหลือ]',
];

// ★ 2 ก.ค. 69 — คลังตัวอย่างแป้กจริง (จากโซน <500 รีแอกชันของเดือนเดียวกัน): สอน บก.AI ว่าอะไร "ดูดีแต่ไม่ปัง"
const FLOP_EXAMPLES = [
  'มอบจักรยาน/ครุภัณฑ์การแพทย์ให้โรงเรียน-รพ. แบบข่าวแจก PR ไม่มีปม-ไม่มีตัวละครเด่น (~130 รีแอกชัน)',
  'MrBeast สร้างบ้าน 100 หลังให้คนต่างประเทศ — เรื่องดีแต่ไม่เกี่ยวคนไทยเลย (311) · ต่างชาติจะปังต่อเมื่อ "ทำเพื่อคนไทย/ผูกกับไทย" เท่านั้น',
  'ดาราพาลูกฉลองวันเกิด/เลี้ยงลูกน่ารัก ไลฟ์สไตล์เรียบๆ ไม่มี contrast ไม่มีปม (329)',
  'เรื่องการเมือง/นโยบายรัฐ แม้เล่าเชิงบวกก็เงียบ (131)',
...
★★★ เกณฑ์พิสูจน์จากผลจริง 949 โพสต์ เดือน มิ.ย. 69 (2 ก.ค. — ผู้ใช้ยืนยันด้วยข้อมูล Meta):
① "ความเข้มของสตอรี่" ชี้ขาดกว่าหมวด — เรื่องที่มี contrast สุดขั้ว (เกรด 4.00 แต่บ้านมีเงินวันละร้อย) + ตัวเลขรูปธรรม (จำนวนเงิน/อายุ/ระยะทาง/กี่ปี) ปังกว่าเรื่องหมวดเดียวกันที่เล่าเรียบๆ ~2 เท่า → ข่าวที่มีทั้ง contrast+ตัวเลข = ดัน 8-10 · หมวดถูกแต่เล่าเรียบไม่มีปม = ไม่เกิน 5
② ชาวบ้านนิรนาม "สตอรี่เข้มจัด" ให้คะแนนสูงได้เต็มที่ (โพสต์อันดับ 1 ของเดือน 243k คือนักเรียนโนเนม) — ห้ามกดคะแนนเพราะไม่ดังอย่างเดียว · แต่ชาวบ้านเรื่องเรียบๆ ไม่มี contrast = ต่ำตามจริง
③ เกาะซากากระแสใหญ่ระดับชาติที่ยังไม่จบ = แนวที่ปังสุด (median 21k) — ข่าวที่เป็น "ตัวละครข้างเคียง/มุมน้ำใจ/ฮีโร่" ของเหตุการณ์ใหญ่ที่คนทั้งประเทศตามอยู่ ให้ 8-10 และเสนอมุมตามตัวละครรายคน
④ ทหาร/ตำรวจ/กู้ภัย/พระ-ผ้าเหลือง ที่ "เสียสละเกินตำแหน่ง" (แม่ทัพบวชกวาดลานวัด/ตำรวจเก่าควักเงินประกันให้ ตชด.) = วงการที่แรงจริง ให้คะแนนแบบเดียวกับดาราดัง
⑤ ราชวงศ์/สถาบัน "เชิงบวก-อาลัย-ชื่นชม" = ปังแรง (median 20k) ให้สูงได้ — เฉพาะแง่ดีเท่านั้น (แง่ลบถูกตัดที่ประตูแล้ว)
⑥ ต่างชาติ: ผ่านเฉพาะ "ทำเพื่อคนไทย/ผูกกับไทยชัดๆ" (กษัตริย์ภูฏานกับไทย/ดาราเกาหลีช่วยนักศึกษาไทย) — ต่างชาติล้วนไม่เกี่ยวไทยต่อให้ซึ้ง = 0-3
⑦ กดต่ำ 0-4: ข่าวแจก-มอบสิ่งของแนว PR ไม่มีปม · ไลฟ์สไตล์/เลี้ยงลูกดาราแบบไม่มี contrast · การเมือง-นโยบาย
...
export function finalScore(item, perfBoost = null) {
  ...
  //   (โพสต์อันดับ 1 ของเดือน 243k = นักเรียนโนเนม) → ไม่ดังแต่ intensity≥2 ได้ boost ใกล้เคียง famous
  const _si = Number(item.storyIntensity) || 0;
  const fameBoost = item.notability === 'famous' ? 12 : (_si >= 2 ? 10 : 0);
  const intensityBoost = _si >= 3 ? 6 : _si === 2 ? 3 : 0; // ความเข้มสตอรี่ = ตัวทำนายที่วัดแล้ว (~2 เท่า)
  const imgBoost = item.imageUrl ? 4 : 0;
  return Math.max(0, Math.min(100, Math.round(fit + fresh + judge - toxPenalty + fameBoost + intensityBoost + imgBoost)));
}
