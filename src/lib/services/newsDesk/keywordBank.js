...
// ════════════════════════════════════════════════════════════════════
// ★★★ 4 ก.ค. (ผู้ใช้: "ข่าวชาวบ้าน/พลเมืองดี/สังคมด้านดี เจอน้อย — ข่าวหลักๆ ที่ผ่านตาแล้ว")
//   วิเคราะห์: 83% ของโต๊ะมาจาก Google News (สำนักข่าวใหญ่) · ข่าวชาวบ้านตัวจริงอยู่ FB/TikTok
//   ทางออก (เทสแล้วได้จริง): ค้น site:tiktok.com / site:facebook.com ด้วย "คำชาวบ้านน้ำดี" → เจอเพียบ
//   = "เลื่อนฟีดโซเชียลอัตโนมัติ" เวอร์ชันที่ทำได้จริง (FYP scroll จริงทำไม่ได้ — แพลตฟอร์มไม่เปิด API)
// ════════════════════════════════════════════════════════════════════
const SOCIAL_GOOD_KEYWORDS = [
  // 🤝 พลเมืองดี/น้ำใจ (ชาวบ้านช่วยกัน)
  'พลเมืองดี ช่วยเหลือ', 'พลเมืองดี ช่วยชีวิต', 'น้ำใจคนไทย ช่วยเหลือ', 'ชาวบ้าน น้ำใจ ช่วยกัน',
  'คนไทยไม่ทิ้งกัน ช่วยเหลือ', 'ช่วยคนตกทุกข์ ประทับใจ', 'คืนเงิน คืนของ ซื่อสัตย์', 'เก็บเงินได้ ตามหาเจ้าของ',
  // 🧓👶 ช่วยคนแก่/เด็ก/คนลำบาก
  'ช่วยเหลือคนแก่ คนชรา', 'ช่วยเหลือเด็กยากจน', 'ช่วยคนพิการ น้ำใจ', 'แบ่งปัน ช่วยคนจน',
  'มอบเงิน มอบของ ช่วยเหลือ', 'ช่วยคนไร้บ้าน คนเร่ร่อน', 'พาคนหลงทาง กลับบ้าน',
  // 💪 คนสู้ชีวิต ชาวบ้าน
  'คนสู้ชีวิต ยากจน', 'ชาวบ้าน สู้ชีวิต น่าทึ่ง', 'อาชีพสุจริต สู้ชีวิต', 'แม่ค้า พ่อค้า สู้ชีวิต',
  'คนพิการ สู้ชีวิต ไม่ยอมแพ้', 'ตายาย เก็บของเก่า เลี้ยงหลาน',
  // 🚑 กู้ภัย/จิตอาสา/ฮีโร่ชาวบ้าน
  'กู้ภัย ช่วยชีวิต นาทีชีวิต', 'จิตอาสา ช่วยเหลือ สังคม', 'ฮีโร่ชาวบ้าน ช่วยคน', 'ตำรวจ ทหาร น้ำใจ ชาวบ้าน',
  // 🐶 รักสัตว์/สังคมด้านดี
  'ช่วยหมาแมวจร รักสัตว์', 'เรื่องดีๆในสังคม ประทับใจ', 'คนดีมีอยู่จริง ไวรัล', 'สังคมด้านดี น้ำใจ ไวรัล',
];

/** ★★★ ล่าโซเชียลน้ำดี (4 ก.ค.) — ค้น TikTok/FB ด้วยคำชาวบ้าน (แตะข่าวที่ Google News ไม่มี) perPlatform คีย์/แพลตฟอร์ม
 *  คืน {q, platform, category, lane:'clip'} — platform tiktok/facebook เท่านั้น (ที่ข่าวชาวบ้านอยู่จริง) */
export function generateSocialGoodQueries(perPlatform = 6) {
  const slot = Math.floor(Date.now() / 3600e3);
  const platforms = ['tiktok', 'facebook'];
  const out = [];
  platforms.forEach((platform, pi) => {
    for (let i = 0; i < perPlatform; i++) {
      const q = SOCIAL_GOOD_KEYWORDS[(slot * perPlatform + i + pi * 5) % SOCIAL_GOOD_KEYWORDS.length];
      out.push({ q, platform, category: '', lane: 'clip' });
    }
  });
  return out;
