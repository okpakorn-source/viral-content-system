...
/**
 * ★ หลอมรวมข้อเท็จจริงรีเสิร์ชเข้ากับ "เนื้อหาดิบเดิม" ให้อ่านลื่นเป็นเนื้อเดียว
 *   ครอบส่วนที่แทรกใหม่ด้วย ⟦n⟧...⟦/⟧ (n = เลขแหล่ง) เพื่อให้รู้ว่าท่อนไหนมาจากรีเสิร์ช + โยงไปแหล่งล่าง
 *   คงเนื้อเดิมครบ (โดยเฉพาะฉากเปิด+ผลปิด) · แทรกเฉพาะ facts ที่มีแหล่ง · ไม่แต่งเพิ่ม
 */
export async function weaveEnrichment({ rawContent, facts }) {
  const useFacts = (facts || []).filter(f => f.text && f.sourceUrl);
  if (!rawContent || !useFacts.length) return { ok: false };
  const factList = useFacts.map((f, i) => `[${i + 1}] ${f.text}`).join('\n');
  const prompt = `เนื้อข่าวดิบเดิม (แกนเดียว เล่าเหตุการณ์ตรงๆ):
"""${rawContent}"""

ข้อเท็จจริงเสริมจากการรีเสิร์ช (แทรกได้เฉพาะที่ระบุนี้ มีเลขแหล่งกำกับ):
${factList}

งาน: แทรกข้อเท็จจริงเสริมเข้าไปใน "จุดที่เข้ากับเนื้อเรื่อง" ของเนื้อข่าวดิบ ให้อ่านลื่นเป็นเนื้อเดียวกัน (ไม่ใช่แปะต่อท้ายเป็นก้อน)
กฎเหล็ก:
- คงเนื้อเดิม "ทั้งหมด" ห้ามตัด/ห้ามย่อ โดยเฉพาะย่อหน้าเปิดฉาก และย่อหน้าปิด/จุดพีค
- แทรกเฉพาะข้อเท็จจริงเสริมที่ให้ไว้ข้างบน ห้ามแต่งเพิ่ม ห้ามเปลี่ยนตัวเลข/ชื่อ
- ครอบ "เฉพาะข้อความส่วนที่แทรกใหม่" ด้วยเครื่องหมาย ⟦n⟧ ... ⟦/⟧ โดย n = เลขแหล่งของข้อเท็จจริงนั้น (เช่น ⟦1⟧ข้อความเสริม⟦/⟧) เพื่อให้รู้ว่าท่อนไหนมาจากรีเสิร์ช
...
ตอบ JSON: {"merged":"เนื้อที่หลอมรวมแล้ว พร้อมเครื่องหมาย ⟦n⟧...⟦/⟧ ตรงส่วนที่เสริม"}`;
  try {
    const r = await callRawJSON({ prompt, model: MODEL_PRIMARY, temperature: 0.3, maxTokens: 2200 });
