...
export class MasterAgent {
  ...
  compileContext(options = {}) {
    ...
    // 7. คำสั่งเหล็ก — ห้ามตั้งคำถามปิดท้าย
    // 🗑️ 17 ส.ค. 69: ตัดเฉพาะวรรค "เขียนยาวอย่างน้อย 250 คำ..." ออก — ส่วนห้ามแต่งเพิ่ม/ห้ามตัดข้อมูลสำคัญยังอยู่
    //    ⚠️ แก้คำอธิบาย 17 ส.ค. 69 (เอเจนท์ "โอปุส5" ในทีมพอดีจับได้ ผมยืนยันซ้ำแล้ว):
    //    compileContext() ถูกเรียกที่เดียวคือ summarizeServiceText.js:1962 และ summarizeService.js:1816
    //    ซึ่ง **ทั้งคู่อยู่ในบล็อก `if (mode === 'mix')`** ⇒ เส้นนี้คือโหมด mix (ปุ่มผสมมุมที่คนกดเอง)
    //    ไม่ใช่เส้นเขียนข่าวหลักตามที่ผมเคยเข้าใจผิด — เส้นหลักคือ mode === 'analyze' ซึ่งไม่ผ่านตรงนี้
    ctx += '⚠️ คำสั่งเหล็ก: ใช้ข้อมูลทั้งหมดข้างต้น ห้ามแต่งเพิ่ม ห้ามตัดข้อมูลสำคัญ' + legacyLengthRule('masterAgentIron') + '\n';
    ctx += 'โครงสร้าง: [ย่อหน้า 1] เปิดแรง hook → [ย่อหน้า 2] storytelling เนื้อหาเยอะ → [ย่อหน้า 3] ปิดด้วยประโยคบรรยายทรงพลัง สรุปแก่นอารมณ์\n';
    ctx += '⚠️ ห้ามตั้งคำถามปิดท้ายเด็ดขาด ห้ามจบด้วยคำถามใดๆ — เน้นบรรยายตาม prompt\n';
