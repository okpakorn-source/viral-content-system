...
// ★ W3 (18 ก.ค.): guard ตัด "ชื่อ subject ขยะ" ที่ขั้นวิเคราะห์สกัดพลาดเป็นเศษประโยค/วลี ก่อนสร้างคำค้น
//   บทเรียนจริง (คลังข่าว ครูปลา): key_people สกัดได้ "ครู คุณแม่ก็เป็นมะเร็ง และจากไปในที่สุด", "ครูติด ซึ่งเธอสอบได้ที่",
//   "ครูอนุบาล\n\nส่วนกรณีที่เด็ก" → ถ้าปล่อยเป็น subject = คำค้นขยะ = ภาพผิด + เปลืองโควตา SerpApi/ตา
//   ตัดเฉพาะสัญญาณ "ไม่ใช่ชื่อคน/วัตถุแน่ๆ" (ขึ้นบรรทัดใหม่ / ยาวผิดปกติ / คำเชื่อม-กริยาในประโยค) — precision สูงไว้ก่อน
//   ห้ามหลุดตัดชื่อจริง (ชื่อ+นามสกุล/ราชสกุลยาวยังผ่าน) · kill-switch: SUBJECT_SANITY=0 = คืนพฤติกรรมเดิม
const SUBJECT_CLAUSE_MARKER = /เล่า|บอกว่า|กล่าว|ซึ่ง|เพราะ|ทำให้|ส่วนกรณี|สิ่งที่|โดยเธอ|โดยเขา|เผยความ|คือการ|ย้อนคิด|และจากไป|ในที่สุด|ที่สุด|ได้มากขนาด|ความตั้งใจ|เป็นแบบ/;
export function isSaneSubjectName(name) {
  const n = String(name || '').trim();
  if (!n) return false;
  if (/[\n\r]/.test(n)) return false;            // ชื่อไม่มีทางมีขึ้นบรรทัดใหม่ = เศษข้อความที่ถูกตัดมา
  if (n.length > 50) return false;               // ชื่อคน/ฉายา/วัตถุ/ราชสกุล ไม่ยาวเกินนี้ = วลี/ประโยค
  if (n.split(/\s+/).length > 5) return false;   // เกิน 5 คำ (คั่นด้วยเว้นวรรค) = วลี/ประโยค
  if (SUBJECT_CLAUSE_MARKER.test(n)) return false; // มีคำเชื่อม/กริยาในประโยค = เศษข้อความ
  return true;
}
...
export function buildQueries(keywords, maxQueries) {
  ...
  const rawSubjects = keywords.subjects || [];
  const _sane = process.env.SUBJECT_SANITY !== '0' ? rawSubjects.filter((s) => isSaneSubjectName(s?.name)) : rawSubjects;
  const subjects = _sane.length ? _sane : rawSubjects;
