...
function createSupabaseModel(tableName) {
  return {
    async create({ data }) {
      ...
      const row = toSnake({ ...data, id, updatedAt: new Date().toISOString() });
    ...
    async update({ where, data }) {
      const sb = getSupabase();
      const row = toSnake({ ...data, updatedAt: new Date().toISOString() });
...
if (isSupabaseReady()) {
  ...
  prismaInstance = {
    ...
    source: createSupabaseModel('contents'), // sources ใช้ร่วมกับ contents (simplified)
