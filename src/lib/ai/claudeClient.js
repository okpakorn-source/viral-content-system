...
export async function callClaude({ prompt, systemPrompt, model = DEFAULT_WRITE_MODEL, temperature = 0.7, maxTokens = 8000, signal }) {
  ...
  const HOUSE_CARD_FIRST = `คุณเป็น AI assistant ที่เชี่ยวชาญการเขียนภาษาไทยสำหรับ Facebook
...
- 🔴 สถานะบุคคล "ยังมีชีวิต/เสียชีวิตแล้ว" ต้องตรงต้นฉบับ 100% — ถ้าต้นฉบับมีผู้เสียชีวิต ต้องบอกการจากไปให้ชัดอย่างน้อย 1 ครั้ง ("เสียชีวิต"/"จากไป" ใช้ได้ตรงๆ) ห้ามเล่าฉากอดีตแบบละคำบอกการจากไปจนคนอ่านเข้าใจว่ายังมีชีวิตอยู่
- ห้ามคาดเดาเพศจากชื่อ — ข่าวไม่ระบุเพศ ให้เรียกด้วยชื่อหรือ "เจ้าตัว"
- ลักษณนามถูกประเภท: พระสงฆ์ = "รูป", คนทั่วไป = "คน" — ห้ามใช้ลักษณนามสิ่งของกับคน
...
=== จบ SAFETY RULES ===`;
  const HOUSE_LEGACY = `คุณเป็น AI assistant ที่เชี่ยวชาญการเขียนภาษาไทยสำหรับ Facebook
ตอบเป็น JSON เท่านั้น ใช้ key names ตามที่ระบุใน prompt
...
คงไว้ข้อเดียว: สถานะเป็น/ตายของบุคคลต้องตรงต้นฉบับ 100% และห้ามใส่ข้อมูลที่ไม่มีในข่าว
=== จบ SAFETY RULES ===`;
  // 🔴 ★ 25 ก.ค. 69 (เจ้าของสั่ง: "เอากลับมา HUMAN WRITING DNA"):
  //    ตอนเช้าตัดออกพร้อมกฎกลางทั้งก้อน แล้วเจ้าของแจ้งว่า "ความลื่นของเนื้อหาหายไป"
  //    เอากลับเฉพาะส่วน "ความเป็นมนุษย์ของสำนวน+จังหวะ" — ไม่เอากฎที่ชนกับการ์ด
  //    (ไม่เอา: บังคับ 250 คำ/3 ย่อหน้า, ลิสต์ห้ามใช้คำ, กฎตัวเลขใน hook, ห้ามตั้งคำถามปิดท้าย)
  //    ปิดคืน: env HUMAN_DNA=off
...
=== จบ HUMAN WRITING DNA V2 ===`;
  ...
  const systemMsg = systemPrompt || _houseRules;
